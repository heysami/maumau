...
const extensionRestartMetadataFiles = new Set(["maumau.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.MAUMAU_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.MAUMAU_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[maumau] ${message}\n`);
};

const runMaumau = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["maumau.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
