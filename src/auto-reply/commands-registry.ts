...
export function isCommandEnabled(cfg: MaumauConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: MaumauConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: MaumauConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: MaumauConfig) {
