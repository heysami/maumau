...
Maumau ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/maumau`, override with
  `MAUMAU_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
maumau update --tag maumau@2026.3.22
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `maumau update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Maumau prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `maumau update` would do without making changes:

```bash
maumau update --dry-run
maumau update --channel beta --dry-run
maumau update --tag 2026.3.22 --dry-run
maumau update --dry-run --json
```
...
When you switch channels with `maumau update`, Maumau also syncs plugin
sources:
