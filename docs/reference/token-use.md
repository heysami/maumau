---
summary: "How Maumau builds prompt context and reports token usage + costs"
read_when:
...
Maumau tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Maumau assembles its own system prompt on every run. It includes:
...
For images, Maumau downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Maumau session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `maumau status --usage` and `maumau channels list` show
...
`cacheWrite`. If pricing is missing, Maumau shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Maumau can
...
Anthropic's 1M context window is currently beta-gated. Maumau can inject the
...
Maumau skips the `context-1m-*` beta header because Anthropic currently
