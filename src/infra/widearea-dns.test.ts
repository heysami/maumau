...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "maumau.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (Maumau)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "maumau.internal", expected: "maumau.internal." },
    { value: "maumau.internal.", expected: "maumau.internal." },
    { value: "  maumau.internal  ", expected: "maumau.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { MAUMAU_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { MAUMAU_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("maumau.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "maumau.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      ...
      instanceLabel: "studio-london",
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/maumau",
    });

    expect(txt).toContain(`$ORIGIN maumau.internal.`);
    ...
    expect(txt).toContain(`_maumau-gw._tcp IN PTR studio-london._maumau-gw._tcp`);
    expect(txt).toContain(`studio-london._maumau-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Maumau)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/maumau`);
  });
  ...
  it("includes gateway TLS TXT fields and trims display metadata", () => {
    ...
    expect(txt).toContain(`$ORIGIN maumau.internal.`);
    expect(txt).toContain(`studio-london IN A 100.123.224.76`);
    expect(txt).toContain(`studio-london._maumau-gw._tcp IN TXT`);
    expect(txt).toContain(`displayName=Mac Studio (Maumau)`);
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("maumau.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("maumau.internal."),
