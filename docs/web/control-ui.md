...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/maumau`)
...
If the page fails to load, start the Gateway first: `maumau gateway`.
...
created. Re-run `maumau devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `maumau devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.maumau/maumau.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop maumau`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Maumau
...
Maumau **blocks** Control UI connections without device identity.
