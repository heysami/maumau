...
let configOverride: MaumauConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: MaumauConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/maumau/maumau/issues/31298
    previousFastTestEnv = process.env.MAUMAU_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.MAUMAU_TEST_FAST;
      return;
    }
    process.env.MAUMAU_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // MAUMAU_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Maumau runtime context (internal):");
