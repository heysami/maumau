...
describe("logAcceptedEnvOption", () => {
  it("logs accepted env options once with redaction and formatting", () => {
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith("env: MAUMAU_TEST_ENV=<redacted> (test option)");
  });
    ...
    withEnv(
      {
        VITEST: "1",
        NODE_ENV: "development",
        MAUMAU_BLANK_ENV: "value",
      },
      () => {
        logAcceptedEnvOption({
          key: "MAUMAU_BLANK_ENV",
