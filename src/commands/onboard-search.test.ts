...
function createPerplexityConfig(apiKey: string, enabled?: boolean): MaumauConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: MaumauConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): MaumauConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: MaumauConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: MaumauConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
