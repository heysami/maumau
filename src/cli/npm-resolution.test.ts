...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@maumau/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@maumau/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@maumau/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@maumau/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @maumau/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@maumau/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@maumau/plugin-alpha@1.2.3",
        installPath: "/tmp/maumau/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@maumau/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @maumau/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@maumau/plugin-alpha@latest",
      true,
      "/tmp/maumau/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@maumau/plugin-alpha@latest",
      installPath: "/tmp/maumau/extensions/alpha",
      version: "1.2.3",
