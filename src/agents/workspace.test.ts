...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MAUMAU_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MAUMAU_HOME: "/srv/maumau-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/maumau-home"), ".maumau", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".maumau", "workspace-state.json"] as const;
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-workspace-hardlink-"));
    try {
