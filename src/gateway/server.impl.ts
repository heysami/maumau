...
import { ensureMaumauCliOnPath } from "../infra/path-env.js";
import {
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("maumau doctor")}" to repair, then retry.`
    : "";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.MAUMAU_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.MAUMAU_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "MAUMAU_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "MAUMAU_RAW_STREAM_PATH",
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `maumau config set gateway.auth.mode token` and `maumau config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  if (matrixInstallPathIssue) {
    const lines = formatPluginInstallPathIssue({
      ...
      pluginLabel: "Matrix",
      defaultInstallCommand: "maumau plugins install @maumau/matrix",
      repoInstallCommand: "maumau plugins install ./extensions/matrix",
  ...
  const transcriptUnsub = minimalTestGateway
    ...
    : onSessionTranscriptUpdate((update) => {
        ...
        const sessionSnapshot = sessionRow
          ...
          : {};
        const message = attachMaumauTranscriptMeta(update.message, {
