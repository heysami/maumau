...
function normalizeBaseUrl(baseUrl?: string): string {
  const envValue =
    process.env.MAUMAU_CLAWHUB_URL?.trim() ||
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    process.env.MAUMAU_CLAWHUB_CONFIG_PATH?.trim() ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    process.env.MAUMAU_CLAWHUB_TOKEN?.trim() ||
...
export async function downloadClawHubPackageArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-clawhub-package-"));
...
export async function downloadClawHubSkillArchive(params: {
  ...
  const bytes = new Uint8Array(await response.arrayBuffer());
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-clawhub-skill-"));
