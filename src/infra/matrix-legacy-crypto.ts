    ...
    if (!stat.isDirectory()) {
      return {
        ...
        warning:
          ...
          "Maumau skipped automatic crypto migration for that path.",
      };
    }
  } catch (err) {
    return {
...
function resolveMatrixAccountIds(cfg: MaumauConfig): string[] {
    ...
    if (!plan.deviceId) {
      warnings.push(
        `Legacy Matrix encrypted state detected at ${plan.legacyCryptoPath}, but no device ID was found for account "${plan.accountId}". ` +
          `Maumau will continue, but old encrypted history cannot be recovered automatically.`,
      );
      continue;
    }
    ...
    if (!summary.decryptionKeyBase64 && (summary.roomKeyCounts?.backedUp ?? 0) > 0) {
      warnings.push(
        `Legacy Matrix encrypted state for account "${plan.accountId}" has backed-up room keys, but no local backup decryption key was found. ` +
          `Ask the operator to run "maumau matrix verify backup restore --recovery-key <key>" after upgrade if they have the recovery key.`,
      );
    }
