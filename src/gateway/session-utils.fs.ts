...
export function attachMaumauTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__maumau && typeof record.__maumau === "object" && !Array.isArray(record.__maumau)
      ? (record.__maumau as Record<string, unknown>)
      : {};
  return {
    ...record,
    __maumau: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachMaumauTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "maumau" && model === "delivery-mirror";
