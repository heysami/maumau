---
summary: "Run Maumau with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Maumau against a local SGLang server
...
Maumau can connect to SGLang using the `openai-completions` API.

Maumau can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Maumau will query:
