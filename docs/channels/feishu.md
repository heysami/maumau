...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Maumau to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Maumau releases, so no separate plugin install
is required.
...
```bash
maumau plugins install @maumau/feishu
```
...
If you just installed Maumau, run onboarding:
...
2. Configuring app credentials in Maumau
...
1. You already ran `maumau channels add` for Feishu
2. The gateway is running (`maumau gateway status`)
...
```bash
maumau pairing approve feishu <CODE>
```
...
2. Run `maumau logs --follow` and look for `chat_id`
...
2. Run `maumau logs --follow` and look for `open_id`
...
| Command                  | Description                   |
| ------------------------ | ----------------------------- |
| `maumau gateway status`  | Show gateway status           |
| `maumau gateway install` | Install/start gateway service |
| `maumau gateway stop`    | Stop gateway service          |
| `maumau gateway restart` | Restart gateway service       |
| `maumau logs --follow`   | Tail gateway logs             |
...
4. Check logs: `maumau logs --follow`
...
5. Ensure the gateway is running: `maumau gateway status`
6. Check logs: `maumau logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.maumau/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.maumau/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
