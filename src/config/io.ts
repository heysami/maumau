...
import type { MaumauConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  maumau config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  maumau config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: MaumauConfig,
  pathSegments: string[],
): { changed: boolean; next: MaumauConfig } {
...
function coerceConfig(value: unknown): MaumauConfig {
...
function stampConfigVersion(cfg: MaumauConfig): MaumauConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: MaumauConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer Maumau (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as MaumauConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): MaumauConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as MaumauConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: MaumauConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as MaumauConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.MAUMAU_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.MAUMAU_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      argv: process.argv.slice(0, 8),
      execArgv: process.execArgv.slice(0, 8),
      watchMode: deps.env.MAUMAU_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.MAUMAU_WATCH_SESSION === "string" &&
        deps.env.MAUMAU_WATCH_SESSION.trim().length > 0
          ? deps.env.MAUMAU_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.MAUMAU_WATCH_COMMAND === "string" &&
        deps.env.MAUMAU_WATCH_COMMAND.trim().length > 0
          ? deps.env.MAUMAU_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `MAUMAU_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: MaumauConfig | null = null;
let runtimeConfigSourceSnapshot: MaumauConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MAUMAU_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.MAUMAU_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function setRuntimeConfigSnapshot(config: MaumauConfig, sourceConfig?: MaumauConfig): void {
...
export function getRuntimeConfigSnapshot(): MaumauConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): MaumauConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: MaumauConfig;
...
export function projectConfigOntoRuntimeSourceSnapshot(config: MaumauConfig): MaumauConfig {
...
export function loadConfig(): MaumauConfig {
...
export async function readBestEffortConfig(): Promise<MaumauConfig> {
...
export async function writeConfigFile(
  ...
  // edits to maumau.json remain visible (no stale snapshot).
}
