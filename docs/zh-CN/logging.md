...
你可以在 `~/.maumau/maumau.json` 中覆盖此设置：
...
所有日志配置都在 `~/.maumau/maumau.json` 的 `logging` 下。
...
- Maumau 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `maumau plugins enable diagnostics-otel` 启用插件。
...
- `maumau.tokens`（计数器，属性：`maumau.token`、`maumau.channel`、`maumau.provider`、`maumau.model`）
- `maumau.cost.usd`（计数器，属性：`maumau.channel`、`maumau.provider`、`maumau.model`）
- `maumau.run.duration_ms`（直方图，属性：`maumau.channel`、`maumau.provider`、`maumau.model`）
- `maumau.context.tokens`（直方图，属性：`maumau.context`、`maumau.channel`、`maumau.provider`、`maumau.model`）

消息流：

- `maumau.webhook.received`（计数器，属性：`maumau.channel`、`maumau.webhook`）
- `maumau.webhook.error`（计数器，属性：`maumau.channel`、`maumau.webhook`）
- `maumau.webhook.duration_ms`（直方图，属性：`maumau.channel`、`maumau.webhook`）
- `maumau.message.queued`（计数器，属性：`maumau.channel`、`maumau.source`）
- `maumau.message.processed`（计数器，属性：`maumau.channel`、`maumau.outcome`）
- `maumau.message.duration_ms`（直方图，属性：`maumau.channel`、`maumau.outcome`）

队列 + 会话：

- `maumau.queue.lane.enqueue`（计数器，属性：`maumau.lane`）
- `maumau.queue.lane.dequeue`（计数器，属性：`maumau.lane`）
- `maumau.queue.depth`（直方图，属性：`maumau.lane` 或 `maumau.channel=heartbeat`）
- `maumau.queue.wait_ms`（直方图，属性：`maumau.lane`）
- `maumau.session.state`（计数器，属性：`maumau.state`、`maumau.reason`）
- `maumau.session.stuck`（计数器，属性：`maumau.state`）
- `maumau.session.stuck_age_ms`（直方图，属性：`maumau.state`）
- `maumau.run.attempt`（计数器，属性：`maumau.attempt`）
...
- `maumau.webhook.error`
  - `maumau.channel`、`maumau.webhook`、`maumau.chatId`、`maumau.error`
- `maumau.message.processed`
  - `maumau.channel`、`maumau.outcome`、`maumau.chatId`、`maumau.messageId`、`maumau.sessionKey`、`maumau.sessionId`、`maumau.reason`
- `maumau.session.stuck`
  - `maumau.state`、`maumau.ageMs`、`maumau.queueDepth`、`maumau.sessionKey`、`maumau.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `maumau doctor`。
