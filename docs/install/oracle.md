---
summary: "Host Maumau on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Maumau on Oracle Cloud
  - Looking for free VPS hosting for Maumau
...
Run a persistent Maumau Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@maumau`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=maumau --reset` to re-authenticate.

**Gateway will not start** -- Run `maumau doctor --non-interactive` and check logs with `journalctl --user -u maumau-gateway -n 50`.
...
- [Updating](/install/updating) -- keep Maumau up to date
