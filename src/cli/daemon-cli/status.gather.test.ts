...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.MAUMAU_STATE_DIR ?? "/tmp/maumau-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.MAUMAU_CONFIG_PATH ?? `${stateDir}/maumau.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/maumau-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.MAUMAU_STATE_DIR = "/tmp/maumau-cli";
    process.env.MAUMAU_CONFIG_PATH = "/tmp/maumau-cli/maumau.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.MAUMAU_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.MAUMAU_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.MAUMAU_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "maumau-gateway" }],
        hints: [],
      },
      healthy: false,
