...
test("exec sets MAUMAU_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-maumau-shell", {
    command: "node -e \"process.stdout.write(process.env.MAUMAU_SHELL || '')\"",
    pty: true,
  });
