...
function collectConfigEnvVarsByTarget(cfg?: MaumauConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: MaumauConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: MaumauConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: MaumauConfig): Record<string, string> {
...
export function applyConfigEnvVars(cfg: MaumauConfig, env: NodeJS.ProcessEnv = process.env): void {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.MAUMAU_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
