...
read_when:
  - Setting up Maumau for the first time
...
Maumau reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.maumau/maumau.json`.

If the file is missing, Maumau uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `maumau onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    maumau onboard       # full onboarding flow
    maumau configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    maumau config get agents.defaults.workspace
    maumau config set agents.defaults.heartbeat.every "2h"
    maumau config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.maumau/maumau.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Maumau only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`maumau doctor`, `maumau logs`, `maumau health`, `maumau status`)
- Run `maumau doctor` to see exact issues
- Run `maumau doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `maumau.json`.
    ...
    ```bash
    maumau config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `MAUMAU_APNS_RELAY_BASE_URL` and `MAUMAU_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `MAUMAU_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.maumau/maumau.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `maumau config set` for single keys.
    </Warning>
    ...
    ```bash
    maumau gateway call config.get --params '{}'  # capture payload.hash
    maumau gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.maumau/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    maumau gateway call config.patch --params '{
...
Maumau reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.maumau/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Maumau runs your login shell and imports only the missing keys:
...
Env var equivalent: `MAUMAU_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${MAUMAU_GATEWAY_TOKEN}" } },
