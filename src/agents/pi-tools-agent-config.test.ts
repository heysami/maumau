...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: MaumauConfig) {
    return createMaumauCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<MaumauConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<MaumauConfig["agents"]>["list"]>[number]["tools"];
  }): MaumauConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createMaumauCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createMaumauCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createMaumauCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createMaumauCodingTools({
      config: cfg,
    ...
    const bobTools = createMaumauCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createMaumauCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createMaumauCodingTools({
      config: cfg,
