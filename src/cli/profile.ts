...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.maumau${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.MAUMAU_PROFILE = profile;

  const stateDir = env.MAUMAU_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.MAUMAU_STATE_DIR?.trim()) {
    env.MAUMAU_STATE_DIR = stateDir;
  }

  if (!env.MAUMAU_CONFIG_PATH?.trim()) {
    env.MAUMAU_CONFIG_PATH = path.join(stateDir, "maumau.json");
  }

  if (profile === "dev" && !env.MAUMAU_GATEWAY_PORT?.trim()) {
    env.MAUMAU_GATEWAY_PORT = "19001";
  }
}
