...
import { VERSION } from "maumau/plugin-sdk/cli-runtime";
import { danger, success } from "maumau/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "maumau/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "maumau/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("maumau channels login")}`,
              ),
            );
          }
        }
