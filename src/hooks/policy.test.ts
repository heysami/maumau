...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "maumau-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "maumau-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "maumau-bundled");
      const plugin = makeHookEntry("shared", "maumau-plugin");
      const managed = makeHookEntry("shared", "maumau-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("maumau-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "maumau-managed");
      const workspace = makeHookEntry("shared", "maumau-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "maumau-workspace");
      const second = makeHookEntry("shared", "maumau-workspace");
      second.hook.handlerPath = "/tmp/maumau-workspace/shared/handler-2.js";
