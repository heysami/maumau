...
vi.mock("maumau/plugin-sdk/config-runtime", async () => {
  ...
  return {
    ...actual,
    loadConfig: () => config,
    resolveStorePath: vi.fn(() => "/tmp/maumau-sessions.json"),
...
vi.mock("maumau/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("maumau/plugin-sdk/security-runtime")>(
    "maumau/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("maumau/plugin-sdk/reply-runtime"),
      import("maumau/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
