...
vi.mock("../../tts/tts.js", () => ({
  ...
  resolveTtsConfig: (cfg: MaumauConfig) => ttsMocks.resolveTtsConfig(cfg),
}));
...
const emptyConfig = {} as MaumauConfig;
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("routes media-only tool results when summaries are suppressed", async () => {
    ...
    const replyResolver = async (_ctx: MsgContext, opts?: GetReplyOptions, _cfg?: MaumauConfig) => {
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as MaumauConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
  ...
  it("routes plugin-owned bindings to the owning plugin before generic inbound claim broadcast", async () => {
    ...
    hookMocks.registry.plugins = [{ id: "maumau-codex-app-server", status: "loaded" }];
    hookMocks.runner.runInboundClaimForPluginOutcome.mockResolvedValue({
      status: "handled",
      result: { handled: true },
    });
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        ...
        pluginRoot: "/Users/huntharo/github/maumau-app-server",
      },
  ...
  it("falls back to Maumau once per startup when a bound plugin is missing", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const replyResolver = vi.fn(async () => ({ text: "maumau fallback" }) satisfies ReplyPayload);
    ...
    const firstNotice = (firstDispatcher.sendToolResult as ReturnType<typeof vi.fn>).mock
      .calls[0]?.[0] as ReplyPayload | undefined;
    expect(firstNotice?.text).toContain("Routing this message to Maumau instead.");
  ...
  it("falls back to Maumau when the bound plugin is loaded but has no inbound_claim handler", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const notice = (dispatcher.sendToolResult as ReturnType<typeof vi.fn>).mock.calls[0]?.[0] as
      | ReplyPayload
      | undefined;
    expect(notice?.text).toContain("Routing this message to Maumau instead.");
    expect(replyResolver).toHaveBeenCalledTimes(1);
    expect(hookMocks.runner.runInboundClaim).not.toHaveBeenCalled();
  });
  ...
  it("passes configOverride to replyResolver when provided", async () => {
    ...
    let receivedCfg: MaumauConfig | undefined;
  ...
  it("does not pass cfg as implicit configOverride when configOverride is not provided", async () => {
    setNoAbort();
    const cfg = { agents: { defaults: { userTimezone: "UTC" } } } as MaumauConfig;
    const dispatcher = createDispatcher();
