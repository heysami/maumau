...
const defaultTools = createMaumauCodingTools({ senderIsOwner: true });
...
describe("createMaumauCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: MaumauConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createMaumauCodingTools({
    ...
    const anthropicTools = createMaumauCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: MaumauConfig = {
      tools: {
        exec: {
    ...
    const allowed = createMaumauCodingTools({
    ...
    const denied = createMaumauCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createMaumauCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
