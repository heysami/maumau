---
read_when:
  - 设置 Maumau 与 Synology Chat
  - 调试 Synology Chat webhook 路由
summary: Synology Chat webhook 设置与 Maumau 配置
title: Synology Chat
x-i18n:
...
1. 安装并启用 Synology Chat 插件。
   - `maumau onboard` 现在会在与 `maumau channels add` 相同的渠道设置列表中显示 Synology Chat。
   - 非交互式设置：`maumau channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. 在 Synology Chat 集成中：
   - 创建一个入站 webhook 并复制其 URL。
   - 使用你的 secret token 创建一个出站 webhook。
3. 将出站 webhook URL 指向你的 Maumau Gateway 网关：
   - 默认是 `https://gateway-host/webhook/synology`。
   - 或者使用你自定义的 `channels.synology-chat.webhookPath`。
4. 在 Maumau 中完成设置。
   - 引导式：`maumau onboard`
   - 直接设置：`maumau channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
