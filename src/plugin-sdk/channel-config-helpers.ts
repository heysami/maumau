...
type ChannelConfigAccessorParams<Config extends MaumauConfig = MaumauConfig> = {
  cfg: Config;
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends MaumauConfig = MaumauConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends MaumauConfig = MaumauConfig>(
...
> {
  const base = {
    resolveAllowFrom: ({ cfg, accountId }: { cfg: MaumauConfig; accountId?: string | null }) =>
      mapAllowFromEntries(
...
function setTopLevelChannelEnabledInConfigSection<Config extends MaumauConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends MaumauConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends MaumauConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
