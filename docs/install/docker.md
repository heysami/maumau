---
summary: "Optional Docker-based setup and onboarding for Maumau"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Maumau on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export MAUMAU_IMAGE="ghcr.io/maumau/maumau:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/maumau/maumau/pkgs/container/maumau).
  ...
  <Step title="Complete onboarding">
    ...
    `maumau-gateway` directly. `maumau-cli` is for commands you run after
...
docker compose run --rm --no-deps --entrypoint node maumau-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `MAUMAU_EXTRA_MOUNTS`
or `MAUMAU_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `maumau-cli` shares `maumau-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d maumau-gateway`, run onboarding
and setup-time config writes through `maumau-gateway` with
...
| Variable                     | Purpose                                                          |
| ---------------------------- | ---------------------------------------------------------------- |
| `MAUMAU_IMAGE`               | Use a remote image instead of building locally                   |
| `MAUMAU_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `MAUMAU_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `MAUMAU_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `MAUMAU_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `MAUMAU_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `MAUMAU_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
`scripts/docker/setup.sh` defaults `MAUMAU_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `MAUMAU_CONFIG_DIR` to `/home/node/.maumau` and
`MAUMAU_WORKSPACE_DIR` to `/home/node/.maumau/workspace`, so those paths
...
and rolling file logs under `/tmp/maumau/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/maumau/maumau/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Then use `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Run
`clawdock-help` for all commands.
See the [`ClawDock` Helper README](https://github.com/maumau/maumau/blob/main/scripts/shell-helpers/README.md).

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export MAUMAU_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export MAUMAU_SANDBOX=1
    export MAUMAU_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm maumau-cli gateway probe
    docker compose run -T --rm maumau-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `maumau-cli` uses `network_mode: "service:maumau-gateway"` so CLI
    ...
    `no-new-privileges` on `maumau-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.maumau`, make sure your host bind mounts are owned by uid 1000:
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export MAUMAU_HOME_VOLUME="maumau_home"`
    2. **Bake system deps**: `export MAUMAU_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `MAUMAU_HOME_VOLUME` or `MAUMAU_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/maumau/maumau/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Maumau runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm maumau-cli config set gateway.mode local
    docker compose run --rm maumau-cli config set gateway.bind lan
    docker compose run --rm maumau-cli devices list --url ws://127.0.0.1:18789
    ```
