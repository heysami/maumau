...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.maumau?.extensions)
    ? packageJson.maumau.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.maumau?.setupEntry === "string" &&
    packageJson.maumau.setupEntry.trim().length > 0
      ? packageJson.maumau.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasPackageJson, sourceEntries } of entries) {
    artifacts.add(`dist/extensions/${id}/maumau.plugin.json`);
    if (hasPackageJson) {
