...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Maumau handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `Maumau docs: ${docsPath}`,
    "Mirror: https://docs.maumau.ai",
    "Source: https://github.com/maumau/maumau",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.com",
    "For Maumau behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `maumau status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Maumau process",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside Maumau.";
  }

  const lines = [
    "You are a personal assistant running inside Maumau.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control Maumau's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "If unsure, ask the user to run `maumau help` (or `maumau gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Maumau Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Maumau pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by Maumau and included below in Project Context.",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'Maumau treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
