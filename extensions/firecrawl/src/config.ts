...
import { normalizeSecretInput } from "maumau/plugin-sdk/provider-auth";
...
type WebSearchConfig = NonNullable<MaumauConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<MaumauConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: MaumauConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: MaumauConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: MaumauConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: MaumauConfig): FirecrawlFetchConfig {
...
export function resolveFirecrawlApiKey(cfg?: MaumauConfig): string | undefined {
...
export function resolveFirecrawlBaseUrl(cfg?: MaumauConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: MaumauConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: MaumauConfig, override?: number): number {
