...
type PromptDefaultModelResult = { model?: string; config?: MaumauConfig };
...
function resolveConfiguredModelRaw(cfg: MaumauConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: MaumauConfig): string[] {
...
export function applyModelAllowlist(cfg: MaumauConfig, models: string[]): MaumauConfig {
