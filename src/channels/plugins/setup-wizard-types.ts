...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: MaumauConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: MaumauConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: MaumauConfig, policy: DmPolicy, accountId?: string) => MaumauConfig;
