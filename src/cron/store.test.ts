...
const { makeStorePath } = createCronStoreHarness({ prefix: "maumau-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses MAUMAU_HOME for tilde expansion", () => {
    vi.stubEnv("MAUMAU_HOME", "/srv/maumau-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/maumau-home", "cron", "jobs.json"));
  });
});
