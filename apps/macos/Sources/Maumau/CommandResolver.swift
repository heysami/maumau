...
enum CommandResolver {
    private static let projectRootDefaultsKey = "maumau.gatewayProjectRootPath"
    private static let helperName = "maumau"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let maumauEntry = root.appendingPathComponent("maumau.mjs").path
        if FileManager().isReadableFile(atPath: maumauEntry) { return maumauEntry }
        let binEntry = root.appendingPathComponent("bin/maumau.js").path
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + maumauPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func maumauManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".maumau"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func maumauExecutable(searchPaths: [String]? = nil) -> String? {
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("maumau.mjs").path,
            root.appendingPathComponent("bin/maumau.js").path,
        ]
    ...
    static func hasAnyMaumauInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.maumauExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let maumauPath = self.projectMaumauExecutable(projectRoot: root) {
            return [maumauPath, subcommand] + extraArgs
        }
        if let maumauPath = self.maumauExecutable(searchPaths: searchPaths) {
            return [maumauPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "maumau", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            maumau entrypoint missing (looked for dist/index.js or maumau.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real maumau CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/maumau"
        ...
        \(cliSection)
        \(projectSection)
        if command -v maumau >/dev/null 2>&1; then
          CLI="$(command -v maumau)"
          maumau \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/maumau.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/maumau.mjs"
            node "$PRJ/maumau.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/maumau.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/maumau.js"
            node "$PRJ/bin/maumau.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent maumau"
          pnpm --silent maumau \(quotedArgs);
        else
          echo "maumau CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? MaumauConfigFile.loadDict()
