...
read_when:
  - You want to understand how Maumau assembles model context
...
A **context engine** controls how Maumau builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Maumau ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.maumau/maumau.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Maumau plugin. Install
...
maumau plugins install -l ./my-context-engine
```
...
Every time Maumau runs a model prompt, the context engine participates at
...
Maumau currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Maumau
...
The built-in `legacy` engine preserves Maumau's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Maumau uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Maumau disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Maumau automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `maumau/plugin-sdk/core` to use
  Maumau's built-in compaction behavior.
...
Maumau resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Maumau's built-in summarization. Plugin engines can implement
...
- Use `maumau doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Maumau
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `maumau plugins install -l ./my-engine` to link a
