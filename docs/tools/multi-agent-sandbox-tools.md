...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `maumau sandbox explain`.
...
Legacy `agent.*` configs are migrated by `maumau doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=maumau-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${MAUMAU_STATE_DIR:-$HOME/.maumau}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
