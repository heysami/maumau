...
This is the full reference for `maumau onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.maumau/maumau.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `maumau doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.maumau/credentials/oauth.json`; auth profiles live in `~/.maumau/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.maumau/credentials/oauth.json` (or `$MAUMAU_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.maumau/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `maumau pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `maumau configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `maumau health`.
    - Tip: `maumau status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export MAUMAU_GATEWAY_TOKEN="your-token"
maumau onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env MAUMAU_GATEWAY_TOKEN
```
...
- Stores it under `~/.maumau/tools/signal-cli/<version>/`.
...
Typical fields in `~/.maumau/maumau.json`:
...
`maumau agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.maumau/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.maumau/agents/<agentId>/sessions/`.
