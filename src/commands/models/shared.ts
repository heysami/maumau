...
import { type MaumauConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function loadValidConfigOrThrow(): Promise<MaumauConfig> {
...
export async function updateConfig(
  mutator: (cfg: MaumauConfig) => MaumauConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: MaumauConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: MaumauConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("maumau agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
