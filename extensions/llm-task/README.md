...
`maumau.invoke --each`) without adding new Maumau code per workflow.
...
This extension depends on Maumau internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Maumau extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.maumau/extensions` as a standalone plugin directory.
