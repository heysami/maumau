...
const { stopMaumauChromeMock } = vi.hoisted(() => ({
  stopMaumauChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopMaumauChrome: stopMaumauChromeMock,
}));
...
beforeEach(async () => {
  ...
  stopMaumauChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["maumau", "user"]);
    ...
    expect(stopMap.maumau).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopMaumauChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("maumau browser stop failed: Error: oops");
  });
});
