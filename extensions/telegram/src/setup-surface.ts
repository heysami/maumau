...
import type { ChannelSetupDmPolicy, ChannelSetupWizard } from "maumau/plugin-sdk/setup";
...
function ensureTelegramDefaultGroupMentionGate(cfg: MaumauConfig, accountId: string): MaumauConfig {
...
function shouldShowTelegramDmAccessWarning(cfg: MaumauConfig, accountId: string): boolean {
...
function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`maumau config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`maumau config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
