...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("maumau gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.MAUMAU_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.MAUMAU_PROFILE),
      }),
    );
  }
  return hints;
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const container = env.MAUMAU_CONTAINER_HINT?.trim() || env.MAUMAU_CONTAINER?.trim();
  const hints = buildPlatformServiceStartHints({
    installCommand: formatCliCommand("maumau gateway install", env),
    startCommand: formatCliCommand("maumau gateway", env),
...
export function filterContainerGenericHints(
  ...
  if (!(env.MAUMAU_CONTAINER_HINT?.trim() || env.MAUMAU_CONTAINER?.trim())) {
    return hints;
  }
  return hints.filter(
    (hint) =>
