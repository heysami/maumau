...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.MAUMAU_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.MAUMAU_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.MAUMAU_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.MAUMAU_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers MAUMAU_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.MAUMAU_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.MAUMAU_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.MAUMAU_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.MAUMAU_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getHandshakeTimeoutMs()).toBe(75);
        process.env.MAUMAU_HANDSHAKE_TIMEOUT_MS = "";
        expect(getHandshakeTimeoutMs()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.MAUMAU_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.MAUMAU_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.MAUMAU_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.MAUMAU_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            MAUMAU_VERSION: " ",
            MAUMAU_SERVICE_VERSION: "2.4.6-service",
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `maumau-test-device-${randomUUID()}.json`),
        nonce,
      });
