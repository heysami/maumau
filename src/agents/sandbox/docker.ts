...
import { markMaumauExecEnv } from "../../infra/maumau-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "maumau.sandbox=1");
  args.push("--label", `maumau.sessionKey=${params.scopeKey}`);
  args.push("--label", `maumau.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `maumau.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markMaumauExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "maumau.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`maumau sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`maumau sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("maumau sandbox recreate --all");
}
