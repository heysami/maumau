...
function cfg(input: Partial<MaumauConfig>): MaumauConfig {
  return input as MaumauConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: MaumauConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  MAUMAU_GATEWAY_TOKEN: "env-token",
  MAUMAU_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when MAUMAU env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when MAUMAU env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when MAUMAU env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
