...
import { resolveMaumauPackageRootSync } from "../infra/maumau-root.js";
...
function hasTrustedMaumauRootIndicator(params: {
  packageRoot: string;
  ...
  const hasMaumauBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("maumau")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.maumau === "string");
  const hasMaumauEntrypoint = fs.existsSync(path.join(params.packageRoot, "maumau.mjs"));
  return hasCliEntryExport || hasMaumauBin || hasMaumauEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedMaumauRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedMaumauRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveMaumauPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedMaumauRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveMaumauPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedMaumauRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
      ? resolveMaumauPackageRootSync({
          cwd,
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({ modulePath, argv1: params.argv1 })) {
    ...
    for (const kind of orderedKinds) {
      ...
      if (fs.existsSync(candidate)) {
        aliasMap[`maumau/plugin-sdk/${subpath}`] = candidate;
        aliasMap[`maumau/plugin-sdk/${subpath}`] = candidate;
        break;
      }
    }
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias
      ? {
          "maumau/extension-api": extensionApiAlias,
        }
      : {}),
    ...(pluginSdkAlias
      ? {
          "maumau/plugin-sdk": pluginSdkAlias,
        }
      : {}),
