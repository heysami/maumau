...
private const val NOTIFICATION_CHANNEL_BASE_ID = "maumau.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Maumau Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Maumau Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Maumau Active")
      }
