...
async function runPrepare(cfg: MaumauConfig, accountId: string) {
...
async function runFinalize(cfg: MaumauConfig, accountId: string) {
...
function expectPreparedResult(
...
): { cfg: MaumauConfig } & Exclude<Awaited<ReturnType<typeof runPrepare>>, void | undefined> {
  ...
  return prepared as { cfg: MaumauConfig } & Exclude<
...
describe("telegramSetupWizard.finalize", () => {
  it("shows global config commands for the default account", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining('maumau config set channels.telegram.dmPolicy "allowlist"'),
      "Telegram DM access warning",
    );
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(`maumau config set channels.telegram.allowFrom '["YOUR_USER_ID"]'`),
      "Telegram DM access warning",
    );
  });
  ...
  it("shows account-scoped config commands for named accounts", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(
        'maumau config set channels.telegram.accounts.alerts.dmPolicy "allowlist"',
      ),
      "Telegram DM access warning",
    );
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(
        `maumau config set channels.telegram.accounts.alerts.allowFrom '["YOUR_USER_ID"]'`,
      ),
      "Telegram DM access warning",
    );
  });
