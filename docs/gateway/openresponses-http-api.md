...
Maumau’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`maumau agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "maumau"`, `model: "maumau/default"`, `model: "maumau/<agentId>"`, or `x-maumau-agent-id`
- use `x-maumau-model` when you want to override the selected agent's backend model
- use `x-maumau-session-key` for explicit session routing
- use `x-maumau-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Maumau reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
