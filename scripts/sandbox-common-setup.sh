...
BASE_IMAGE="${BASE_IMAGE:-maumau-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-maumau-sandbox-common:bookworm-slim}"
...
MAUMAU_DOCKER_BUILD_USE_BUILDX="${MAUMAU_DOCKER_BUILD_USE_BUILDX:-0}"
MAUMAU_DOCKER_BUILD_CACHE_FROM="${MAUMAU_DOCKER_BUILD_CACHE_FROM:-}"
MAUMAU_DOCKER_BUILD_CACHE_TO="${MAUMAU_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${MAUMAU_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${MAUMAU_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${MAUMAU_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${MAUMAU_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${MAUMAU_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=maumau.sandbox=1)
NOTE
