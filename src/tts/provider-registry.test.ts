...
vi.mock("../plugins/loader.js", () => ({
  loadMaumauPlugins: (...args: Parameters<typeof loadMaumauPluginsMock>) =>
    loadMaumauPluginsMock(...args),
}));
...
describe("speech provider registry", () => {
  beforeEach(async () => {
    ...
    loadMaumauPluginsMock.mockReturnValue(createEmptyPluginRegistry());
  ...
  it("loads speech providers from plugins when config is provided", () => {
    loadMaumauPluginsMock.mockReturnValue({
      ...createEmptyPluginRegistry(),
      speechProviders: [
        {
    ...
    expect(loadMaumauPluginsMock).toHaveBeenCalledWith({ config: cfg });
  });
