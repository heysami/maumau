...
import type { MaumauConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: MaumauConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: MaumauConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  ...
  const validated = MaumauSchema.safeParse(normalizedRaw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as MaumauConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as MaumauConfig);
  ...
  return {
    ok: true,
    config: validated.data as MaumauConfig,
  };
}
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; Maumau mapped it automatically for compatibility.",
  }));
  ...
  let compatConfig: MaumauConfig | null | undefined;

  const ensureCompatConfig = (): MaumauConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Maumau config schema.
