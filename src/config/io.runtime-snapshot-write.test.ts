...
function createSourceConfig(): MaumauConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): MaumauConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("maumau-config-runtime-source-", async () => {
  ...
  it("skips source projection for non-runtime-derived configs", async () => {
    await withTempHome("maumau-config-runtime-projection-shape-", async () => {
      ...
      const independentConfig: MaumauConfig = {
        models: {
          providers: {
            openai: {
              baseUrl: "https://api.openai.com/v1",
  ...
  it("preserves source secret refs when writeConfigFile receives runtime-resolved config", async () => {
    await withTempHome("maumau-config-runtime-write-", async (home) => {
  ...
  it("refreshes the runtime snapshot after writes so follow-up reads see persisted changes", async () => {
    await withTempHome("maumau-config-runtime-write-refresh-", async (home) => {
      const configPath = path.join(home, ".maumau", "maumau.json");
      const sourceConfig: MaumauConfig = {
        models: {
          providers: {
      ...
      const nextRuntimeConfig: MaumauConfig = {
        ...runtimeConfig,
  ...
  it("keeps the last-known-good runtime snapshot active while a specialized refresh is pending", async () => {
    await withTempHome("maumau-config-runtime-refresh-pending-", async (home) => {
      const configPath = path.join(home, ".maumau", "maumau.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
      const nextRuntimeConfig: MaumauConfig = {
