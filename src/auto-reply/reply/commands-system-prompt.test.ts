...
const { createMaumauCodingToolsMock } = vi.hoisted(() => ({
  createMaumauCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    createMaumauCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createMaumauCodingToolsMock).toHaveBeenCalledWith(
