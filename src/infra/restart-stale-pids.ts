 ...
 * returning control to the caller (typically `triggerMaumauRestart` →
...
/**
 * Parse maumau gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("maumau")) {
 ...
 * Returns only PIDs that belong to maumau gateway processes (not the current process).
 */
