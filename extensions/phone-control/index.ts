...
function normalizeDenyList(cfg: MaumauPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: MaumauPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: MaumauPluginApi) {
    ...
    const timerService: MaumauPluginService = {
