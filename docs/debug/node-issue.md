...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Maumau
...
Running Maumau via Node with `tsx` fails at startup with:

```
[maumau] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `maumau status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch maumau.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node maumau.mjs status` works on Node 25.
