...
Read-only diff viewer plugin for **Maumau** agents.
...
Set plugin-wide defaults in `~/.maumau/maumau.json`:
...
Before:
Maumau supports plugins.

After:
Maumau supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/maumau-diffs`).
