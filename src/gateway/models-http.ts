...
import { MAUMAU_DEFAULT_MODEL_ID, MAUMAU_MODEL_ID, resolveAgentIdFromModel } from "./http-utils.js";
...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([MAUMAU_MODEL_ID, MAUMAU_DEFAULT_MODEL_ID]);
  ids.add(`maumau/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`maumau/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== MAUMAU_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
