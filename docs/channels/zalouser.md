...
read_when:
  - Setting up Zalo Personal for Maumau
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Maumau.
...
- Install via CLI: `maumau plugins install @maumau/zalouser`
- Or from a source checkout: `maumau plugins install ./extensions/zalouser`
...
- `maumau pairing list zalouser`
- `maumau pairing approve zalouser <code>`
...
- On startup, Maumau resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Maumau stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Maumau state. Example:
...
- Maumau sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Maumau sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `maumau channels logout --channel zalouser && maumau channels login --channel zalouser`
...
- The channel now runs fully in Maumau without external CLI binaries.
