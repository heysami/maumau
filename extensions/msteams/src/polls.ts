...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["maumauPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["maumau", "pollId"]) ??
    readNestedString(value, ["maumau", "poll", "id"]) ??
    readNestedString(value, ["data", "maumauPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "maumau", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "maumau poll vote",
          displayText: "Vote recorded",
          value: { maumauPollId: pollId, pollId },
        },
      },
    ],
  };
