...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("maumau doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with MAUMAU_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with MAUMAU_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.MAUMAU_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://maumau-gateway.ai:18789",
      onConnectError,
    });
