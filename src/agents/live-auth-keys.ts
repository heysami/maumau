...
const GOOGLE_LIVE_SINGLE_KEY = "MAUMAU_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "MAUMAU_LIVE_ANTHROPIC_KEY",
    listVar: "MAUMAU_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "MAUMAU_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `MAUMAU_LIVE_${base}_KEY`;
