---
summary: "Deploy Maumau Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Maumau on a Kubernetes cluster
  - You want to test Maumau in a Kubernetes environment
title: "Kubernetes"
---

# Maumau on Kubernetes

A minimal starting point for running Maumau on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Maumau is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/maumau 18789:18789 -n maumau
...
```bash
kubectl get secret maumau-secrets -n maumau -o jsonpath='{.data.MAUMAU_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: maumau (configurable via MAUMAU_NAMESPACE)
├── Deployment/maumau        # Single pod, init container + gateway
├── Service/maumau           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/maumau-config  # maumau.json + AGENTS.md
└── Secret/maumau-secrets    # Gateway token + API keys
```
...
Edit `maumau.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret maumau-secrets -n maumau \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/maumau -n maumau
```

### Custom namespace

```bash
MAUMAU_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/maumau/maumau:latest # or pin to a specific version from https://github.com/maumau/maumau/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # maumau.json + AGENTS.md
