...
function formatPluginLine(plugin: PluginRecord, verbose = false): string {
  ...
  const format = plugin.format ?? "maumau";
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.maumau.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action((id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "maumau"}`);
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: MaumauConfig = enableResult.config;
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.maumau.ai/plugin");
      lines.push("");
