...
Maumau can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Maumau can send audio; Telegram gets a round voice-note bubble.
...
Maumau defaults to Microsoft (unless disabled via
...
TTS config lives under `messages.tts` in `maumau.json`.
...
- If `provider` is **unset**, Maumau prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `microsoft`.
...
`~/.maumau/settings/tts.json`, override with `MAUMAU_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Maumau retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Maumau registers
...
reply delivery. When the result is Telegram-compatible, Maumau marks it for
