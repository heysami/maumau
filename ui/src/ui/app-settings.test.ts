...
const createHost = (tab: Tab): SettingsHost => ({
  ...
  theme: "dash" as unknown as ThemeName & ThemeMode,
  themeMode: "light",
  themeResolved: "dash-light",
...
describe("applySettingsFromUrl", () => {
  ...
  it("hydrates query token params and strips them from the URL", () => {
    ...
    host.settings.gatewayUrl = "wss://control.example/maumau";
  ...
  it("keeps query token params pending when a gatewayUrl confirmation is required", () => {
    setTestWindowUrl(
      "https://control.example/ui/overview?gatewayUrl=wss://other-gateway.example/maumau&token=abc123",
    );
    const host = createHost("overview");
    host.settings.gatewayUrl = "wss://control.example/maumau";
    ...
    expect(host.pendingGatewayUrl).toBe("wss://other-gateway.example/maumau");
