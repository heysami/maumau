...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.MAUMAU_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    MAUMAU_PROFILE: profile,
    MAUMAU_GATEWAY_PORT: String(port),
    MAUMAU_LAUNCHD_LABEL: resolvedLaunchdLabel,
    MAUMAU_SYSTEMD_UNIT: systemdUnit,
    MAUMAU_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    MAUMAU_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    MAUMAU_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = env.MAUMAU_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    MAUMAU_GATEWAY_TOKEN: gatewayToken,
    MAUMAU_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    MAUMAU_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    MAUMAU_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    MAUMAU_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    MAUMAU_LOG_PREFIX: "node",
    MAUMAU_SERVICE_MARKER: NODE_SERVICE_MARKER,
    MAUMAU_SERVICE_KIND: NODE_SERVICE_KIND,
    MAUMAU_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    MAUMAU_STATE_DIR: sharedEnv.stateDir,
    MAUMAU_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.MAUMAU_STATE_DIR;
  const configPath = env.MAUMAU_CONFIG_PATH;
