...
import { formatCliCommand } from "maumau/plugin-sdk/setup-tools";
...
export async function refreshQwenPortalCredentials(
  ...
  if (!response.ok) {
    ...
    if (response.status === 400) {
      throw new Error(
        `Qwen OAuth refresh token expired or invalid. Re-authenticate with \`${formatCliCommand("maumau models auth login --provider qwen-portal")}\`.`,
      );
    }
