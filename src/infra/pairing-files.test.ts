...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/maumau-state", "devices")).toEqual({
      dir: path.join("/tmp/maumau-state", "devices"),
      pendingPath: path.join("/tmp/maumau-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/maumau-state", "devices", "paired.json"),
    });
  });
