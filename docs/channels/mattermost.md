---
summary: "Mattermost bot setup and Maumau config"
read_when:
...
```bash
maumau plugins install @maumau/mattermost
```

Local checkout (when running from a git repo):

```bash
maumau plugins install ./extensions/mattermost
```
...
4. Configure Maumau and start the gateway.
...
Native slash commands are opt-in. When enabled, Maumau registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Maumau derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Maumau.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Maumau.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Maumau, not `404`.
...
- Approve via:
  - `maumau pairing list mattermost`
  - `maumau pairing approve mattermost <CODE>`
...
Use these target formats with `maumau message send` or cron/webhooks:
...
Maumau resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Maumau sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Maumau sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Maumau derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Maumau run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="maumau-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"maumau-mattermost-interactions",
...
`maumau message send` and cron/webhook deliveries.
