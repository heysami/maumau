...
import type { MaumauPackageManifest } from "./manifest.js";
...
function makeTempDir() {
  return makeTrackedTempDir("maumau-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "maumau.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createPluginCandidate(params: {
  ...
  format?: "maumau" | "bundle";
  bundleFormat?: "codex" | "claude" | "cursor";
  packageManifest?: MaumauPackageManifest;
  packageDir?: string;
}): PluginCandidate {
  return {
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "maumau.plugin.json");
  const linkedManifest = path.join(rootDir, "maumau.plugin.json");
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("skips plugins whose minHostVersion is newer than the current host", () => {
    ...
    expect(
      registry.diagnostics.some((diag) =>
        diag.message.includes("plugin requires Maumau >=2026.3.22, but this host is 2026.3.21"),
      ),
    ).toBe(true);
  });
  ...
  it("rejects invalid minHostVersion metadata", () => {
    ...
    expect(
      registry.diagnostics.some((diag) =>
        diag.message.includes("plugin manifest invalid | maumau.install.minHostVersion must use"),
      ),
    ).toBe(true);
  });
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        ...
        MAUMAU_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: hermeticEnv({
        HOME: homeB,
        MAUMAU_HOME: undefined,
        MAUMAU_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
