...
export function createStatusScanSharedMocks(configPathLabel: string) {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/maumau-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = MaumauConfig>(
  overrides: T = {} as T,
): MaumauConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): MaumauConfig {
