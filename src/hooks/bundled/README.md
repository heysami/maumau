...
This directory contains hooks that ship with Maumau. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
- **Managed hooks**: `~/.maumau/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.maumau/maumau.json`:
...
2. Restart gateway: `pkill -9 -f 'maumau.*gateway' && pnpm maumau gateway`
3. Enable the hook: `maumau hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.maumau.ai/automation/hooks
