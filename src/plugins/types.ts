 ...
 */
export type MaumauPluginConfigSchema = {
...
export type MaumauPluginToolContext = {
  config?: MaumauConfig;
  workspaceDir?: string;
  agentDir?: string;
  agentId?: string;
...
export type MaumauPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type MaumauPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<MaumauConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthContext = {
  ...
  /**
   * Onboarding secret input preference.
   *
   * Interactive wizard flows set this when the caller explicitly requested
   * paste-now or env/file/exec ref input. Ad-hoc `models auth login` flows
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Maumau uses this to expose
 ...
 *
 * Runs after Maumau resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Maumau auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Maumau wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Maumau builds its generic
...
/**
 * Provider-owned stream wrapper hook after Maumau applies its generic
 ...
 *
 * Return `true` or `false` to override Maumau's built-in provider cache TTL
 ...
 *
 * Runs only after Maumau exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after Maumau loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * Maumau calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic Maumau wrappers.
   *
   ...
   *
   * Called after Maumau resolves the raw configured credential but before the
   ...
   * hint than Maumau's generic auth-store guidance.
   */
   ...
   * `errorMessage` when Maumau should surface a provider-specific hint for
   ...
   * merging. Maumau deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Maumau uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `maumau doctor`.
   *
   ...
   *
   * Maumau calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Maumau's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
  ...
  getConfiguredCredentialValue?: (config?: MaumauConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: MaumauConfig, value: unknown) => void;
  applySelectionConfig?: (config: MaumauConfig) => MaumauConfig;
...
export type MaumauPluginGatewayMethod = {
  method: string;
 ...
 */
export type MaumauPluginCommandDefinition = {
...
export type MaumauPluginHttpRouteAuth = "gateway" | "plugin";
export type MaumauPluginHttpRouteMatch = "exact" | "prefix";

export type MaumauPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type MaumauPluginHttpRouteParams = {
  path: string;
  handler: MaumauPluginHttpRouteHandler;
  auth: MaumauPluginHttpRouteAuth;
  match?: MaumauPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type MaumauPluginCliContext = {
  program: Command;
  config: MaumauConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type MaumauPluginCliRegistrar = (ctx: MaumauPluginCliContext) => void | Promise<void>;

/** Context passed to long-lived plugin services. */
export type MaumauPluginServiceContext = {
  config: MaumauConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type MaumauPluginService = {
  id: string;
  start: (ctx: MaumauPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: MaumauPluginServiceContext) => void | Promise<void>;
};

export type MaumauPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type MaumauPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  kind?: PluginKind;
  configSchema?: MaumauPluginConfigSchema;
  register?: (api: MaumauPluginApi) => void | Promise<void>;
  activate?: (api: MaumauPluginApi) => void | Promise<void>;
};

export type MaumauPluginModule =
  | MaumauPluginDefinition
  | ((api: MaumauPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

/** Main registration API injected into native plugin entry files. */
export type MaumauPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | MaumauPluginToolFactory,
    opts?: MaumauPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: MaumauPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: MaumauPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: MaumauPluginChannelRegistration | ChannelPlugin) => void;
  registerGatewayMethod: (method: string, handler: GatewayRequestHandler) => void;
  registerCli: (registrar: MaumauPluginCliRegistrar, opts?: { commands?: string[] }) => void;
  registerService: (service: MaumauPluginService) => void;
   ...
   */
  registerCommand: (command: MaumauPluginCommandDefinition) => void;
...
export type PluginFormat = "maumau" | "bundle";
