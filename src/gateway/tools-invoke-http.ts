...
import { createMaumauTools } from "../agents/maumau-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-maumau-message-channel") ?? "");
  const accountId = getHeader(req, "x-maumau-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-maumau-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-maumau-thread-id")?.trim() || undefined;
  ...
  const allTools = createMaumauTools({
