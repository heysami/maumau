...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("maumau/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("maumau/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced maumau/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced maumau/plugin-sdk/<subpath> must exist in the public package exports.",
  );
