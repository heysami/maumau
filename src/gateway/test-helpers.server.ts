...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "MAUMAU_SKIP_PROVIDERS",
  "MAUMAU_SKIP_CRON",
  "MAUMAU_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.MAUMAU_CONFIG_PATH) {
    configPaths.add(process.env.MAUMAU_CONFIG_PATH);
  }
  if (process.env.MAUMAU_STATE_DIR) {
    configPaths.add(path.join(process.env.MAUMAU_STATE_DIR, "maumau.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.MAUMAU_SKIP_PROVIDERS = "1";
  process.env.MAUMAU_SKIP_CRON = "1";
  process.env.MAUMAU_TEST_MINIMAL_GATEWAY = "1";
  process.env.MAUMAU_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "maumau-test-no-bundled-extensions")
    : "maumau-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".maumau-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".maumau-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".maumau-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>maumau-test-control-ui</title>\n",
    "utf-8",
  );
...
const CONNECT_CHALLENGE_NONCE_KEY = "__maumauTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__maumauTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["MAUMAU_GATEWAY_TOKEN"]);
  const prev = process.env.MAUMAU_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MAUMAU_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.MAUMAU_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.MAUMAU_GATEWAY_PASSWORD;
