...
summary: 在 Maumau 中使用 Qwen OAuth（免费层）
title: Qwen
x-i18n:
...
```bash
maumau plugins enable qwen-portal-auth
```
...
```bash
maumau models set qwen-portal/coder-model
```

## 复用 Qwen Code CLI 登录

如果你已经使用 Qwen Code CLI 登录，Maumau 会在加载认证存储时从 `~/.qwen/oauth_creds.json` 同步凭证。你仍然需要一个 `models.providers.qwen-portal` 条目（使用上面的登录命令创建一个）。
