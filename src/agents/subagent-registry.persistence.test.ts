...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["MAUMAU_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-subagent-"));
  ...
  it("uses isolated temp state when MAUMAU_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "maumau-test-state"));
  });
});
