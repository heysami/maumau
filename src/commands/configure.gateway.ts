...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "MAUMAU_GATEWAY_TOKEN",
          placeholder: "MAUMAU_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like MAUMAU_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Maumau will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Maumau trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.maumau.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
