...
process.env.MAUMAU_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime Maumau modules.
const testEnv = withIsolatedTestHome();

installProcessWarningFilter();

const REGISTRY_STATE = Symbol.for("maumau.pluginRegistryState");
...
const createStubPlugin = (params: {
  ...
  config: {
    ...
    resolveAccount: (cfg: MaumauConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: MaumauConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
