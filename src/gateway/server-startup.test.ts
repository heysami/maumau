...
const ensureMaumauModelsJsonMock = vi.fn<
...
vi.mock("../agents/agent-paths.js", () => ({
  resolveMaumauAgentDir: () => "/tmp/agent",
}));

vi.mock("../agents/models-config.js", () => ({
  ensureMaumauModelsJson: (config: unknown, agentDir: unknown) =>
    ensureMaumauModelsJsonMock(config, agentDir),
}));
...
describe("gateway startup primary model warmup", () => {
  beforeEach(() => {
    ensureMaumauModelsJsonMock.mockClear();
  ...
  it("prewarms an explicit configured primary model", async () => {
    ...
    expect(ensureMaumauModelsJsonMock).toHaveBeenCalledWith(cfg, "/tmp/agent");
  ...
  it("skips warmup when no explicit primary model is configured", async () => {
    ...
    expect(ensureMaumauModelsJsonMock).not.toHaveBeenCalled();
