...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): MaumauConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as MaumauConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): MaumauConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as MaumauConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: MaumauConfig, accountId = "media") {
    await handleTelegramAction(
