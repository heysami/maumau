...
export const POSIX_MAUMAU_TMP_DIR = "/tmp/maumau";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredMaumauTmpDirOptions = {
...
export function resolvePreferredMaumauTmpDir(
  options: ResolvePreferredMaumauTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "maumau" : `maumau-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[maumau] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Maumau temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Maumau temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_MAUMAU_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_MAUMAU_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_MAUMAU_TMP_DIR)) {
      return POSIX_MAUMAU_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_MAUMAU_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_MAUMAU_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_MAUMAU_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_MAUMAU_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_MAUMAU_TMP_DIR;
