...
`maumau agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    maumau agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    maumau agent --agent ops --message "Summarize logs"

    # Target a phone number (derives session key)
    maumau agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    maumau agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    maumau agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    maumau agent --agent ops --message "Generate report" \
...
maumau agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
