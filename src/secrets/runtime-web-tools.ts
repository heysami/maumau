...
type FetchConfig = NonNullable<MaumauConfig["tools"]>["web"] extends infer Web
...
function hasCustomWebSearchPluginRisk(config: MaumauConfig): boolean {
...
function setResolvedFirecrawlApiKey(params: { resolvedConfig: MaumauConfig; value: string }): void {
  const tools = ensureObject(params.resolvedConfig as Record<string, unknown>, "tools");
  const web = ensureObject(tools, "web");
