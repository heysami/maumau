import { resolveDefaultModelForAgent } from "maumau/plugin-sdk/agent-runtime";
import type { MaumauConfig } from "maumau/plugin-sdk/config-runtime";
import { resetInboundDedupe } from "maumau/plugin-sdk/reply-runtime";
import type { MsgContext } from "maumau/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "maumau/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "maumau/plugin-sdk/reply-runtime";
import type { MockFn } from "maumau/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("maumau/plugin-sdk/config-runtime").loadConfig;
type ResolveStorePathFn = typeof import("maumau/plugin-sdk/config-runtime").resolveStorePath;
type TelegramBotRuntimeForTest = NonNullable<
  Parameters<typeof import("./bot.js").setTelegramBotRuntimeForTest>[0]
>;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("maumau/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/maumau-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("maumau/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("maumau/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("maumau/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
...
vi.doMock("maumau/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.doMock("maumau/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: MaumauConfig): {
...
vi.doMock("maumau/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("maumau/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("maumau/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("maumau/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("maumau/plugin-sdk/infra-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: MaumauConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: MaumauConfig) => {
