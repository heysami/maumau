  ...
  describe("resolveApnsRelayConfigFromEnv", () => {
    ...
    it("lets env overrides win and clamps tiny timeout values", () => {
      const resolved = resolveApnsRelayConfigFromEnv(
        {
          MAUMAU_APNS_RELAY_BASE_URL: " https://relay-override.example.com/base/ ",
          MAUMAU_APNS_RELAY_TIMEOUT_MS: "999",
    ...
    it("allows loopback http URLs for alternate truthy env values", () => {
      const resolved = resolveApnsRelayConfigFromEnv({
        MAUMAU_APNS_RELAY_BASE_URL: "http://[::1]:8787",
        MAUMAU_APNS_RELAY_ALLOW_HTTP: "yes",
        MAUMAU_APNS_RELAY_TIMEOUT_MS: "nope",
      } as NodeJS.ProcessEnv);
    ...
    it.each([
      {
        ...
        env: { MAUMAU_APNS_RELAY_BASE_URL: "ftp://relay.example.com" },
        expected: "unsupported protocol",
      },
      {
        name: "http non-loopback host",
        env: {
          MAUMAU_APNS_RELAY_BASE_URL: "http://relay.example.com",
          MAUMAU_APNS_RELAY_ALLOW_HTTP: "true",
        },
        expected: "loopback hosts",
      },
      {
        name: "query string",
        env: { MAUMAU_APNS_RELAY_BASE_URL: "https://relay.example.com/path?debug=1" },
        expected: "query and fragment are not allowed",
      },
      {
        name: "userinfo",
        env: { MAUMAU_APNS_RELAY_BASE_URL: "https://user:pass@relay.example.com/path" },
