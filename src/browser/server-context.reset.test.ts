...
function localMaumauProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "maumau",
...
function createLocalMaumauResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localMaumauProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveMaumauUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-reset-"));
    const profileDir = path.join(tempRoot, "maumau");
    ...
    const ops = createLocalMaumauResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveMaumauUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-reset-no-own-"));
    const profileDir = path.join(tempRoot, "maumau");
    fs.mkdirSync(profileDir, { recursive: true });
