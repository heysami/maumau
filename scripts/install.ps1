# Maumau Installer for Windows (PowerShell)
# Usage: iwr -useb https://maumau.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://maumau.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\maumau",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Install-MaumauGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Maumau from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/maumau/maumau.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\maumau\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\maumau.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Maumau from git to $GitDir" -Level info
        } else {
            Install-MaumauGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Maumau via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-MaumauNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'maumau onboard' to complete setup" -Level info
    }
