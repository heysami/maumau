...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/maumau/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Maumau Re-Auth" -c "After getting key, run: ssh $SERVER '~/maumau/scripts/mobile-reauth.sh'" --id maumau-auth
        ;;
    *)
