  ...
  if (!channel) {
    return {
      warning:
        ...
        'Configure Matrix, then rerun "maumau doctor --fix" or restart the gateway.',
    };
  }
  if (requiresExplicitMatrixDefaultAccount(params.cfg)) {
    return {
      warning:
        ...
        'Set "channels.matrix.defaultAccount" to the intended target account before rerunning "maumau doctor --fix" or restarting the gateway.',
    };
  }
  ...
  if (!target) {
    ...
    return {
      warning:
        ...
        'Start the gateway once with a working Matrix login, or rerun "maumau doctor --fix" after cached credentials are available.',
    };
  }
