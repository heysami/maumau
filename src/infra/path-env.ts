...
function candidateBinDirs(opts: EnsureMaumauPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `maumau` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "maumau");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.MAUMAU_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "maumau"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `maumau` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMaumauCliOnPath(opts: EnsureMaumauPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MAUMAU_PATH_BOOTSTRAPPED)) {
    return;
  }
