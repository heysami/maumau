...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: MaumauWatchNotifyParams) -> MaumauWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [MaumauWatchAction]?,
        kind: String?,
        promptId: String?) -> [MaumauWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> MaumauWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return MaumauWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                MaumauWatchAction(id: "approve", label: "Approve"),
                MaumauWatchAction(id: "decline", label: "Decline", style: "destructive"),
                MaumauWatchAction(id: "open_phone", label: "Open iPhone"),
                MaumauWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            MaumauWatchAction(id: "done", label: "Done"),
            MaumauWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            MaumauWatchAction(id: "open_phone", label: "Open iPhone"),
            MaumauWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: MaumauWatchRisk?,
        priority: MaumauNotificationPriority?) -> MaumauWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: MaumauNotificationPriority?,
        risk: MaumauWatchRisk?) -> MaumauNotificationPriority?
    {
