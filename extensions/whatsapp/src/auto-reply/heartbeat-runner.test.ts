...
vi.mock("maumau/plugin-sdk/agent-runtime", () => ({
...
vi.mock("maumau/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.mock("maumau/plugin-sdk/routing", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/routing")>();
  return {
    ...actual,
...
vi.mock("maumau/plugin-sdk/runtime-env", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/runtime-env")>();
  const logger = {
    child: () => logger,
...
vi.mock("maumau/plugin-sdk/text-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/text-runtime")>();
  return {
    ...actual,
    redactIdentifier,
  };
...
vi.mock("../auth-store.js", () => ({
  WA_WEB_AUTH_DIR: "/tmp/maumau-oauth/whatsapp/default",
  resolveDefaultWebAuthDir: () => "/tmp/maumau-oauth/whatsapp/default",
