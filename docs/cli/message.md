---
summary: "CLI reference for `maumau message` (send + channel actions)"
read_when:
...
```
maumau message <subcommand> [flags]
```
...
- On cache miss, Maumau will attempt a live directory lookup when the provider supports it.
...
- `maumau message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Maumau does not force a `default` account SecretRef scope
...
```
maumau message send --channel discord \
...
```
maumau message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
maumau message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
maumau message poll --channel msteams \
...
```
maumau message react --channel slack \
...
```
maumau message react --channel signal \
...
```
maumau message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
maumau message send --channel telegram --target @mychat \
