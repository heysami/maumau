...
vi.mock("../infra/path-env.js", () => ({
  ensureMaumauCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "maumau", "status"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    await runCli(["node", "maumau", "--help"]);

    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "maumau", "--help"]);
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "maumau", "--container", "demo", "status"]);
