...
Maumau can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `maumau configure --section web` enables the bundled Firecrawl plugin automatically.
...
Maumau always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
