...
const SESSIONS_YIELD_INTERRUPT_CUSTOM_TYPE = "maumau.sessions_yield_interrupt";
const SESSIONS_YIELD_CONTEXT_CUSTOM_TYPE = "maumau.sessions_yield";
const SESSIONS_YIELD_ABORT_SETTLE_TIMEOUT_MS = process.env.MAUMAU_TEST_FAST === "1" ? 250 : 2_000;
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const docsPath = await resolveMaumauDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/maumau/maumau/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("maumau:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/maumau/maumau/issues/8643
