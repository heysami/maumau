...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["MAUMAU_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.MAUMAU_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveMaumauPackageRoot } = await import("./maumau-root.js"));
    ...
    vi.mocked(resolveMaumauPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveMaumauPackageRoot).mockResolvedValue("/opt/maumau");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/maumau/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
