---
summary: "Maumau on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Maumau on DigitalOcean
  - Looking for cheap VPS hosting for Maumau
title: "DigitalOcean (Platform)"
---

# Maumau on DigitalOcean

## Goal

Run a persistent Maumau Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status maumau-gateway.service

# View logs
journalctl --user -u maumau-gateway.service -f
```
...
```bash
maumau config set gateway.bind tailnet
maumau gateway restart
```
...
```bash
maumau channels login whatsapp
# Scan QR code
```
...
- `~/.maumau/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u maumau --no-pager -n 50
```
