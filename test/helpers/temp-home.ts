...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked MAUMAU_HOME.
  delete process.env.MAUMAU_HOME;
  process.env.MAUMAU_STATE_DIR = path.join(base, ".maumau");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "maumau-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".maumau", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
