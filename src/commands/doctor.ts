...
import { resolveMaumauPackageRoot } from "../infra/maumau-root.js";
...
function resolveMode(cfg: MaumauConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveMaumauPackageRoot({
  ...
  let cfg: MaumauConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("maumau configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("maumau config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("maumau setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("maumau config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("maumau config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("maumau doctor --fix")}" to apply changes.`);
  }
