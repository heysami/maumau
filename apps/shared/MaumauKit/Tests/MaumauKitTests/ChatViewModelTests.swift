...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> MaumauChatHistoryPayload
{
    MaumauChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> MaumauChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> MaumauChatSessionEntry
{
    MaumauChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> MaumauChatModelChoice {
    MaumauChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [MaumauChatHistoryPayload],
    sessionsResponses: [MaumauChatSessionsListResponse] = [],
    modelResponses: [[MaumauChatModelChoice]] = [],
    ...
    -> (TestChatTransport, MaumauChatViewModel)
{
...
private func sendUserMessage(_ vm: MaumauChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, MaumauChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [MaumauChatHistoryPayload]
    private let sessionsResponses: [MaumauChatSessionsListResponse]
    private let modelResponses: [[MaumauChatModelChoice]]
    ...
    private let stream: AsyncStream<MaumauChatTransportEvent>
    private let continuation: AsyncStream<MaumauChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<MaumauChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> MaumauChatHistoryPayload {
        ...
        return self.historyResponses.last ?? MaumauChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [MaumauChatAttachmentPayload]) async throws -> MaumauChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return MaumauChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> MaumauChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? MaumauChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: MaumauChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = MaumauChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = MaumauChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: MaumauChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = MaumauChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: MaumauChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(MaumauChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == MaumauChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = MaumauChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: MaumauChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = MaumauChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = MaumauChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = MaumauChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { MaumauChatViewModel(sessionKey: "main", transport: transport) }
