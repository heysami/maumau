...
declare const __MAUMAU_VERSION__: string | undefined;
const CORE_PACKAGE_NAMES = new Set(["maumau"]);
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["MAUMAU_VERSION"],
      runtimeVersion,
      env["MAUMAU_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Maumau version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __MAUMAU_VERSION__ === "string" ? __MAUMAU_VERSION__ : undefined,
  bundledVersion: process.env.MAUMAU_BUNDLED_VERSION,
});
