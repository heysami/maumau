...
@MainActor
public struct MaumauChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    private var visibleMessages: [MaumauChatMessage] {
        let base: [MaumauChatMessage]
    ...
    private func isToolResultMessage(_ message: MaumauChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: MaumauChatMessage) -> Bool {
    ...
    private func primaryText(in message: MaumauChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: MaumauChatMessage) -> Bool {
    ...
    private func toolCalls(in message: MaumauChatMessage) -> [MaumauChatMessageContent] {
    ...
    private func inlineToolResults(in message: MaumauChatMessage) -> [MaumauChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: MaumauChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: MaumauChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(MaumauChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
