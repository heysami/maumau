...
- During heartbeat runs, Maumau treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `maumau sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Maumau skips the heartbeat run entirely (no model call).
...
```bash
maumau system event --text "Check for urgent follow-ups" --mode now
```
