...
const MAUMAU_REPO_URL = "https://github.com/maumau/maumau.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "maumau";
...
export function resolveGitInstallDir(): string {
  const override = process.env.MAUMAU_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "maumau");
  }
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveMaumauPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", MAUMAU_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `MAUMAU_GIT_DIR points at a non-git directory: ${params.dir}. Set MAUMAU_GIT_DIR to an empty folder or an maumau checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`MAUMAU_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "maumau.mjs");
