...
import { resolvePreferredMaumauTmpDir } from "../tmp-maumau-dir.js";
...
const telegramChunkConfig: MaumauConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: MaumauConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("scopes media local roots to the active agent workspace when agentId is provided", async () => {
    ...
    expect(
      sendOpts?.mediaLocalRoots?.some((root) =>
        root.endsWith(path.join(".maumau", "workspace-work")),
      ),
    ).toBe(true);
  });

  it("includes Maumau tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredMaumauTmpDir()]),
      }),
    );
  });

  it("includes Maumau tmp root in signal mediaLocalRoots", async () => {
  ...
  it("forwards audioAsVoice through generic plugin media delivery", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as MaumauConfig,
      channel: "matrix",
  ...
  it("includes Maumau tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes Maumau tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: MaumauConfig = { channels: { signal: { mediaMaxMb: 2 } } };
