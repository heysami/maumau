...
type ConfiguredBinding = NonNullable<MaumauConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<MaumauConfig>,
): MaumauConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: MaumauConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/maumau" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
