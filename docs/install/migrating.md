---
summary: "Move (migrate) an Maumau install from one machine to another"
read_when:
  - You are moving Maumau to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Maumau to a New Machine

This guide moves an Maumau gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.maumau/` by default) and your **workspace**, you preserve:

- **Config** -- `maumau.json` and all gateway settings
...
<Tip>
Run `maumau status` on the old machine to confirm your state directory path.
Custom profiles use `~/.maumau-<profile>/` or a path set via `MAUMAU_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    maumau gateway stop
    cd ~
    tar -czf maumau-state.tgz .maumau
    ```

    If you use multiple profiles (e.g. `~/.maumau-work`), archive each separately.

  </Step>

  <Step title="Install Maumau on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.maumau/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `MAUMAU_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `maumau doctor`.
  </Accordion>

  <Accordion title="Copying only maumau.json">
  ...
  <Accordion title="Remote mode">
    ...
    Migrate the gateway host itself, not your local laptop. See [FAQ](/help/faq#where-does-maumau-store-its-data).
  </Accordion>
...
- [ ] `maumau status` shows the gateway running
