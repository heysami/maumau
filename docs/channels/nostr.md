...
read_when:
  - You want Maumau to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Maumau to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`maumau onboard`) and `maumau channels add` list optional channel plugins.
...
```bash
maumau plugins install @maumau/nostr
```

Use a local checkout (dev workflows):

```bash
maumau plugins install --link <path-to-maumau>/extensions/nostr
```
...
maumau channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "maumau@example.com",
        lud16: "maumau@example.com",
      },
    },
  },
}
