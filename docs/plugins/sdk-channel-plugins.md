...
summary: "Step-by-step guide to building a messaging channel plugin for Maumau"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Maumau to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects Maumau to a
...
Channel plugins do not need their own send/edit/react tools. Maumau keeps one
...
<Steps>
    ...
    ```json package.json
    {
      ...
      "maumau": {
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Maumau to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Add a setup entry">
    ...
    Maumau loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    Maumau. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Maumau.
...
├── package.json              # maumau.channel metadata
├── maumau.plugin.json      # Manifest with config schema
