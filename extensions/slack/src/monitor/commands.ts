import type { SlackSlashCommandConfig } from "maumau/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "maumau");
  const name = normalizedName || "maumau";
  return {
