...
describe("createMaumauCodingTools", () => {
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-alias-"));
    try {
      const tools = createMaumauCodingTools({ workspaceDir: tmpDir });
  ...
  it("accepts broader file/edit alias variants", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-alias-broad-"));
    try {
      const tools = createMaumauCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-structured-write-"));
    try {
      const tools = createMaumauCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-structured-edit-"));
    try {
