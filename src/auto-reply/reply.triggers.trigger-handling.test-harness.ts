...
const piEmbeddedMocks = getSharedMocks("maumau.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("maumau.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("maumau.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("maumau.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    maumauHome: process.env.MAUMAU_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("MAUMAU_HOME", snapshot.maumauHome);
  restoreKey("MAUMAU_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.MAUMAU_STATE_DIR = join(home, ".maumau");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "maumau-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".maumau", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): MaumauConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "maumau"),
