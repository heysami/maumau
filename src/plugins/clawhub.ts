...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "maumau skills install ${pkg.name}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this Maumau runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires Maumau >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
  ...
  try {
    ...
    return {
      ...
      clawhub: {
        source: "clawhub",
        clawhubUrl:
          params.baseUrl?.trim() || process.env.MAUMAU_CLAWHUB_URL?.trim() || "https://clawhub.ai",
