...
async function withIsolatedStateDirAsync<T>(fn: () => Promise<T>): Promise<T> {
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-telegram-state-"));
  return await withEnvAsync({ MAUMAU_STATE_DIR: stateDir }, async () => {
    try {
      return await fn();
    } finally {
...
async function withConfigPathAsync<T>(cfg: unknown, fn: () => Promise<T>): Promise<T> {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-telegram-cfg-"));
  const configPath = path.join(dir, "maumau.json");
  fs.writeFileSync(configPath, JSON.stringify(cfg), "utf-8");
  return await withEnvAsync({ MAUMAU_CONFIG_PATH: configPath }, async () => {
    try {
      return await fn();
    } finally {
...
describe("createTelegramBot", () => {
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    await withIsolatedStateDirAsync(async () => {
      for (const [index, testCase] of cases.entries()) {
        ...
        expect(pairingText, testCase.name).toContain(`maumau pairing approve telegram ${code}`);
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "maumau_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("honors routed group activation from session store", async () => {
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-telegram-"));
