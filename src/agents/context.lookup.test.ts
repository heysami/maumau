...
function mockContextDeps(params: {
  ...
  const ensureMaumauModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveMaumauAgentDir: () => "/tmp/maumau-agent",
  }));
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real maumau startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "maumau", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "maumau", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "maumau", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureMaumauModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureMaumauModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
