---
summary: "Run Maumau on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Maumau
title: "Linux Server"
...
Run the Maumau Gateway on any Linux server or cloud VPS. This page helps you
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/maumau-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `MAUMAU_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  - `MAUMAU_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/maumau-compile-cache`
