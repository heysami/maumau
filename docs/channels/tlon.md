...
Tlon is a decentralized messenger built on Urbit. Maumau connects to your Urbit ship and can
...
```bash
maumau plugins install @maumau/tlon
```

Local checkout (when running from a git repo):

```bash
maumau plugins install ./extensions/tlon
```
...
By default, Maumau blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `maumau message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Maumau replies in-thread.
