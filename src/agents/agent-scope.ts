...
type AgentEntry = NonNullable<NonNullable<MaumauConfig["agents"]>["list"]>[number];
...
type ResolvedAgentConfig = {
  ...
  background?: AgentEntry["background"];
...
export function listAgentEntries(cfg: MaumauConfig): AgentEntry[] {
...
export function listAgentIds(cfg: MaumauConfig): string[] {
...
export function resolveDefaultAgentId(cfg: MaumauConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: MaumauConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentSkillsFilter(cfg: MaumauConfig, agentId: string): string[] | undefined {
...
export function resolveAgentModelPrimary(cfg: MaumauConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: MaumauConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentIdsByWorkspacePath(cfg: MaumauConfig, workspacePath: string): string[] {
