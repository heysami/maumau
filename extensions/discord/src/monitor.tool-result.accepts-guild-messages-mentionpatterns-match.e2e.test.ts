...
const BASE_CFG: Config = {
  agents: {
    defaults: {
      ...
      workspace: "/tmp/maumau",
    },
  },
  messages: {
    inbound: { debounceMs: 0 },
  },
  session: { store: "/tmp/maumau-sessions.json" },
};
...
describe("discord tool result dispatch", () => {
  it("accepts guild messages when mentionPatterns match", async () => {
    const cfg = createMentionRequiredGuildConfig({
      messages: {
        inbound: { debounceMs: 0 },
        groupChat: { mentionPatterns: ["\\bmaumau\\b"] },
      },
    } as Partial<Config>);
    ...
    await handler(createGuildMessageEvent({ messageId: "m2", content: "maumau: hello" }), client);
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "Maumau" },
          },
        },
      }),
      client,
