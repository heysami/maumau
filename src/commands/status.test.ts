...
beforeAll(() => {
  envSnapshot = captureEnv(["MAUMAU_PROFILE"]);
  process.env.MAUMAU_PROFILE = "isolated";
});
...
vi.mock("../infra/maumau-root.js", () => ({
  resolveMaumauPackageRoot: vi.fn().mockResolvedValue("/tmp/maumau"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/maumau/pnpm-lock.yaml",
      markerPath: "/tmp/maumau/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.maumau.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.maumau.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("maumau status --all") ||
          line.includes("maumau --profile isolated status --all"),
      ),
    ).toBe(true);
  });
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("MAUMAU_GATEWAY_TOKEN", "abcd1234", async () => {
