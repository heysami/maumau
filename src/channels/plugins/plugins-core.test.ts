...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@maumau/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-catalog-"));
  ...
  it("preserves plugin ids when they differ from channel ids", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-channel-catalog-state-"));
  ...
  it("uses the provided env for external catalog path resolution", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-catalog-home-"));
        ...
        entries: [
          {
            ...
            maumau: {
              ...
              install: {
                npmSpec: "@maumau/env-demo-channel",
              },
            },
          },
        ],
    ...
    const ids = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        MAUMAU_PLUGIN_CATALOG_PATHS: "~/catalog.json",
        MAUMAU_HOME: home,
        HOME: home,
      },
    }).map((entry) => entry.id);
  ...
  it("uses the provided env for default catalog paths", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-catalog-state-"));
        ...
        entries: [
          {
            ...
            maumau: {
              ...
              install: {
                npmSpec: "@maumau/default-env-demo",
              },
            },
          },
        ],
  ...
  it("includes bundled metadata-only channel entries even when the runtime entrypoint is omitted", () => {
    const packageRoot = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-bundled-catalog-"));
    const bundledDir = path.join(packageRoot, "dist", "extensions", "whatsapp");
    fs.mkdirSync(bundledDir, { recursive: true });
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "maumau" }),
      "utf8",
    );
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(bundledDir, "maumau.plugin.json"),
    ...
    const entry = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        MAUMAU_BUNDLED_PLUGINS_DIR: path.join(packageRoot, "dist", "extensions"),
      },
    }).find((item) => item.id === "whatsapp");

    expect(entry?.install.npmSpec).toBe("@maumau/whatsapp");
    expect(entry?.pluginId).toBe("whatsapp");
  });

  it("includes shipped official channel catalog entries when bundled metadata is omitted", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-official-catalog-"));
  ...
  it("lets external catalogs override shipped fallback channel metadata", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-fallback-catalog-"));
    ...
    const entry = listChannelPluginCatalogEntries({
      ...
      env: {
        ...process.env,
        MAUMAU_BUNDLED_PLUGINS_DIR: path.join(dir, "dist", "extensions"),
      },
    }).find((item) => item.id === "whatsapp");
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: MaumauConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
