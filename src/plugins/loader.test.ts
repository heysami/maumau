...
import { __testing, clearPluginLoaderCache, loadMaumauPlugins } from "./loader.js";
...
type PluginLoadConfig = NonNullable<Parameters<typeof loadMaumauPlugins>[0]>["config"];
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "maumau-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.MAUMAU_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "maumau.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.MAUMAU_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadMaumauPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          maumau: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.MAUMAU_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.MAUMAU_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadMaumauPlugins>) {
...
function useNoBundledPlugins() {
  process.env.MAUMAU_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadMaumauPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadMaumauPlugins>[0], "cache" | "config">,
) {
  return loadMaumauPlugins({
    cache: false,
    ...options,
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "maumau.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
        configSchema: EMPTY_PLUGIN_SCHEMA,
...
function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".maumau", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ MAUMAU_STATE_DIR: stateDir, ...params.env }, () =>
    loadMaumauPlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadMaumauPlugins>,
  pluginId: string,
) {
  expect(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadMaumauPlugins>;
  loadSecond: () => ReturnType<typeof loadMaumauPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadMaumauPlugins>;
  loadVariant: () => ReturnType<typeof loadMaumauPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const maumauHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(maumauHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadMaumauPlugins({
    cache: false,
    config: {
      plugins: {
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.MAUMAU_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".maumau", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ MAUMAU_STATE_DIR: stateDir }, () =>
      loadMaumauPlugins({
        workspaceDir,
    ...
    const cases = [
      {
        ...
        assert: (registry: ReturnType<typeof loadMaumauPlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
      ...
      },
      {
        ...
        run: () => {
          ...
          const full = loadMaumauPlugins(options);
          const scoped = loadMaumauPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
          const scopedAgain = loadMaumauPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadMaumauPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadMaumauPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadMaumauPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadMaumauPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadMaumauPlugins(options);
        ...
        return {
          ...
          loadFirst: () =>
            loadMaumauPlugins({
              ...
              env: {
                ...process.env,
                MAUMAU_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadMaumauPlugins({
              ...
              env: {
                ...process.env,
                MAUMAU_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadMaumauPlugins({
              ...
              env: {
                ...process.env,
                HOME: homeA,
                MAUMAU_HOME: undefined,
                MAUMAU_STATE_DIR: stateDir,
                MAUMAU_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadMaumauPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(maumauHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadMaumauPlugins(options),
          loadVariant: () =>
            loadMaumauPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadMaumauPlugins({
      env: {
        ...
        HOME: homeDir,
        MAUMAU_HOME: undefined,
        MAUMAU_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers MAUMAU_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const maumauHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "maumau-home-demo",
      dir: path.join(maumauHome, "plugins", "maumau-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "maumau-home-demo", register() {} };`,
    });
    ...
    const registry = loadMaumauPlugins({
      ...
      config: {
        plugins: {
          allow: ["maumau-home-demo"],
          entries: {
            "maumau-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/maumau-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "maumau-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadMaumauPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadMaumauPlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "maumau.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const setupRegistry = loadMaumauPlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@maumau/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@maumau/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@maumau/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@maumau/setup-runtime-not-preferred-test",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "maumau.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "maumau.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const stateDir = makeTempDir();
          return withEnv({ MAUMAU_STATE_DIR: stateDir }, () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const workspaceDir = makeTempDir();
          const workspaceExtDir = path.join(workspaceDir, ".maumau", "extensions", "shadowed");
          mkdirSafe(workspaceExtDir);
          writePlugin({
            id: "shadowed",
            body: `module.exports = { id: "shadowed", register() {} };`,
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("maumau/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ MAUMAU_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadMaumauPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__maumauLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("maumau/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("maumau/plugin-sdk");
