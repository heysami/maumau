---
summary: "CLI reference for `maumau secrets` (reload, audit, configure, apply)"
read_when:
...
Use `maumau secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
Scan Maumau state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `maumau.json` refs)
...
```bash
maumau secrets audit
maumau secrets audit --check
maumau secrets audit --json
maumau secrets audit --allow-exec
```
...
```bash
maumau secrets configure
maumau secrets configure --plan-out /tmp/maumau-secrets-plan.json
maumau secrets configure --apply --yes
maumau secrets configure --providers-only
maumau secrets configure --skip-provider-setup
maumau secrets configure --agent ops
maumau secrets configure --json
```
...
- `configure` targets secret-bearing fields in `maumau.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Maumau fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
maumau secrets apply --from /tmp/maumau-secrets-plan.json --json
```
...
- `maumau.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.maumau/.env` known secret keys whose values were migrated
