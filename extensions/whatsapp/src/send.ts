...
import { generateSecureUuid } from "maumau/plugin-sdk/infra-runtime";
...
import { getChildLogger } from "maumau/plugin-sdk/text-runtime";
import { redactIdentifier } from "maumau/plugin-sdk/text-runtime";
import { convertMarkdownTables } from "maumau/plugin-sdk/text-runtime";
import { markdownToWhatsApp } from "maumau/plugin-sdk/text-runtime";
import { toWhatsappJid } from "maumau/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: MaumauConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
