...
const gatewayEnv = {
  MAUMAU_GATEWAY_TOKEN: "env-token", // pragma: allowlist secret
  MAUMAU_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
} as NodeJS.ProcessEnv;

function makeRemoteGatewayConfig(remote: { token?: string; password?: string }): MaumauConfig {
  return {
    gateway: {
      mode: "remote",
      remote,
...
function withGatewayAuthEnv<T>(env: NodeJS.ProcessEnv, fn: () => T): T {
  const keys = ["MAUMAU_GATEWAY_TOKEN", "MAUMAU_GATEWAY_PASSWORD", "MAUMAU_SERVICE_KIND"] as const;
