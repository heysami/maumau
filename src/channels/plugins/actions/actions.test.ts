...
function telegramCfg(): MaumauConfig {
  return { channels: { telegram: { botToken: "tok" } } } as MaumauConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: MaumauConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as MaumauConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as MaumauConfig;
...
function createSignalAccountOverrideCfg(): MaumauConfig {
  return {
    channels: {
      signal: {
...
function createDiscordModerationOverrideCfg(params?: { channelsEnabled?: boolean }): MaumauConfig {
...
describe("discord message actions", () => {
  it("derives discord action listings from channel and moderation gates", () => {
    const cases = [
      {
        name: "defaults",
        cfg: { channels: { discord: { token: "d0" } } } as MaumauConfig,
        expectUploads: true,
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as MaumauConfig,
        accountId: undefined,
        params: {
