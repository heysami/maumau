...
const workspaceHarness = createCommandWorkspaceHarness("maumau-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".maumau", "extensions", params.pluginId);
...
function buildCfg(): MaumauConfig {
  return {
    plugins: {
      enabled: true,
    },
