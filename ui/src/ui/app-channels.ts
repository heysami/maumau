...
export async function handleWhatsAppStart(host: MaumauApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: MaumauApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: MaumauApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: MaumauApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: MaumauApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: MaumauApp): string {
...
function resolveGatewayHttpAuthHeader(host: MaumauApp): string | null {
...
function buildGatewayHttpHeaders(host: MaumauApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: MaumauApp) {
...
export function handleNostrProfileToggleAdvanced(host: MaumauApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: MaumauApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: MaumauApp) {
  const state = host.nostrProfileFormState;
