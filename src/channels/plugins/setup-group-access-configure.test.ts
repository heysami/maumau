...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: MaumauConfig, policy: ChannelAccessPolicy) => MaumauConfig;
  resolveAllowlist: (params: { cfg: MaumauConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: MaumauConfig; resolved: TResolved }) => MaumauConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: MaumauConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: MaumauConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: MaumauConfig, policy: ChannelAccessPolicy): MaumauConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: MaumauConfig, policy: ChannelAccessPolicy): MaumauConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: MaumauConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: MaumauConfig; resolved: string[] }) => {
      calls.push("apply");
