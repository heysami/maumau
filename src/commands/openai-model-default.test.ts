...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: MaumauConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: MaumauConfig) => { changed: boolean; next: MaumauConfig };
  defaultModel: string;
  overrideConfig: MaumauConfig;
  alreadyDefaultConfig: MaumauConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: MaumauConfig) => config,
      applyDefaultConfig: (config: MaumauConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: MaumauConfig = { agents: { defaults: {} } };
