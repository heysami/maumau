...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop maumau!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@maumau_bot", { botUsername: "maumau_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@maumau_bot", { botUsername: "maumau_bot" })).toBe(true);
