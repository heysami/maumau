...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByMaumau ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByMaumau ? "installed · " : "";
  ...
  runtime.log(theme.heading("Maumau status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`maumau devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("maumau devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("maumau devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("maumau security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("maumau security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.maumau.ai/faq");
  runtime.log("Troubleshooting: https://docs.maumau.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("maumau status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("maumau logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("maumau status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("maumau gateway probe")}`);
  }
}
