...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under MAUMAU_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("maumau-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-oauth-sync-"));
    ...
    process.env.MAUMAU_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-oauth-nosync-"));
    process.env.MAUMAU_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside MAUMAU_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-oauth-external-"));
    process.env.MAUMAU_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* MAUMAU_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to MAUMAU_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("maumau-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
