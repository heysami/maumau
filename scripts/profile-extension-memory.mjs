...
const RSS_MARKER = "__MAUMAU_MAX_RSS_KB__=";
...
async function main() {
  ...
  const tmpHome = mkdtempSync(path.join(os.tmpdir(), "maumau-extension-memory-"));
  const hookPath = path.join(tmpHome, "measure-rss.mjs");
  const jsonPath = options.jsonPath ?? path.join(os.tmpdir(), "maumau-extension-memory.json");
