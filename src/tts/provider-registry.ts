...
function resolveSpeechProviderPluginEntries(cfg?: MaumauConfig): SpeechProviderPlugin[] {
  const active = getActivePluginRegistry();
  const registry =
    (active?.speechProviders?.length ?? 0) > 0 || !cfg
      ? active
      : loadMaumauPlugins({ config: cfg });
  return registry?.speechProviders?.map((entry) => entry.provider) ?? [];
}

function buildProviderMaps(cfg?: MaumauConfig): {
...
export function listSpeechProviders(cfg?: MaumauConfig): SpeechProviderPlugin[] {
