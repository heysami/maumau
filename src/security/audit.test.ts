...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: MaumauConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: MaumauConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: MaumauConfig) => string[];
  isConfigured?: (account: unknown, cfg: MaumauConfig) => boolean;
  isEnabled?: (account: unknown, cfg: MaumauConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
async function runInstallMetadataAudit(
  ...
  return runSecurityAudit({
    ...
    includeChannelSecurity: false,
    stateDir,
    configPath: path.join(stateDir, "maumau.json"),
...
describe("security audit", () => {
  ...
  let previousMaumauHome: string | undefined;
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "maumau.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ MAUMAU_STATE_DIR: sharedChannelSecurityStateDir }, () =>
      fn(sharedChannelSecurityStateDir),
    );
  };

  const runSharedExtensionsAudit = async (config: MaumauConfig) => {
    return runSecurityAudit({
      config,
      includeFilesystem: true,
      includeChannelSecurity: false,
      stateDir: sharedExtensionsStateDir,
      configPath: path.join(sharedExtensionsStateDir, "maumau.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  };
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        maumau: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-security-audit-"));
    previousMaumauHome = process.env.MAUMAU_HOME;
    process.env.MAUMAU_HOME = path.join(fixtureRoot, "home");
    await fs.mkdir(process.env.MAUMAU_HOME, { recursive: true, mode: 0o700 });
  ...
  afterAll(async () => {
    if (previousMaumauHome === undefined) {
      delete process.env.MAUMAU_HOME;
    } else {
      process.env.MAUMAU_HOME = previousMaumauHome;
    }
    if (!fixtureRoot) {
      return;
    }
    ...
    const cases = [
          ...
          withEnvAsync(
            {
              ...
              MAUMAU_GATEWAY_PASSWORD: undefined,
            },
            async () =>
              audit({
                gateway: {
      ...
      },
      {
        ...
        run: async () => {
          ...
          const resolvedConfig: MaumauConfig = {
            gateway: {
              bind: "lan",
              auth: {},
            },
  ...
  it("evaluates safeBinTrustedDirs risk findings", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/maumau-safe-bins"];
    const cases = [
      {
    ...
    const cases = [
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("maumau-sbx-browser-old\nmaumau-sbx-browser-missing-hash\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "maumau-sbx-browser-old") {
                return {
              ...
              if (args[0] === "inspect" && args.at(-1) === "maumau-sbx-browser-missing-hash") {
                return {
        ...
        assert: (res: SecurityAuditReport) => {
          ...
          expect(staleEpoch?.detail).toContain("maumau-sbx-browser-old");
        },
      },
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("maumau-sbx-browser-exposed\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "maumau-sbx-browser-exposed") {
                return {
              ...
              if (args[0] === "port" && args.at(-1) === "maumau-sbx-browser-exposed") {
                return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-maumau.json");
    ...
    await Promise.all(
      cases
        ...
        .map(async (testCase) => {
          ...
          const configPath = path.join(fixture.stateDir, "maumau.json");
          await fs.writeFile(configPath, "{}\n", "utf-8");
          if (!isWindows) {
            await fs.chmod(configPath, 0o600);
          }
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): MaumauConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    const cases = [
      ...
      },
      {
        ...
        plugin: (sourceConfig: MaumauConfig) =>
          stubChannelPlugin({
            id: "slack",
            label: "Slack",
            inspectAccount: (cfg) => {
  ...
  it.each([
    {
      ...
      detailIncludes: [
        ...
        "~/.maumau/credentials/discord-allowFrom.json:team.owner",
      ],
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: MaumauConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("evaluates hooks ingress auth and routing findings", async () => {
    ...
    } satisfies NonNullable<MaumauConfig["hooks"]>;
    ...
    const cases = [
      ...
      },
      {
        ...
        env: {
          MAUMAU_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
        },
      ...
      },
      {
        ...
        cfg: { hooks: unrestrictedBaseHooks } satisfies MaumauConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "warn" as const,
      },
      {
        name: "scores unrestricted hooks.allowedAgentIds by remote exposure",
        cfg: { gateway: { bind: "lan" }, hooks: unrestrictedBaseHooks } satisfies MaumauConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "critical" as const,
      },
      {
        name: "scores hooks request sessionKey override by local exposure",
        cfg: { hooks: requestSessionKeyHooks } satisfies MaumauConfig,
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.maumau",
      configPath: "/Users/test/Dropbox/.maumau/maumau.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: MaumauConfig = { logging: { redactSensitive: "off" } };
    ...
    const cases = [
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            path.join(pluginDir, "package.json"),
            JSON.stringify({ name: "@maumau/voice-call", version: "9.9.9" }),
            "utf-8",
          );
          await fs.writeFile(
            path.join(hookDir, "package.json"),
            JSON.stringify({ name: "@maumau/test-hooks", version: "8.8.8" }),
            "utf-8",
          );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.MAUMAU_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.MAUMAU_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
