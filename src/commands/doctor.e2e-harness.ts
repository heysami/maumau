...
export const resolveMaumauPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadMaumauPlugins: () => createEmptyPluginRegistry(),
}));
...
beforeEach(() => {
  ...
  resolveMaumauPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.MAUMAU_STATE_DIR;
  originalUpdateInProgress = process.env.MAUMAU_UPDATE_IN_PROGRESS;
  process.env.MAUMAU_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-doctor-state-"));
