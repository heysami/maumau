...
if [ -f dist/index.mjs ]; then
  MAUMAU_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  MAUMAU_ENTRY="dist/index.js"
else
...
home_dir=$(mktemp -d "/tmp/maumau-plugins-e2e.XXXXXX")
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$MAUMAU_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$MAUMAU_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$MAUMAU_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JS
  cat > "$dir/maumau.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/maumau.plugin.json" "$id"
const fs = require("node:fs");
...
mkdir -p "$HOME/.maumau/extensions/demo-plugin"

cat > "$HOME/.maumau/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.maumau/extensions/demo-plugin/maumau.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$MAUMAU_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/maumau-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@maumau/demo-plugin-tgz",
  "version": "0.0.1",
  "maumau": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/maumau.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$MAUMAU_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$MAUMAU_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/maumau-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@maumau/demo-plugin-dir",
  "version": "0.0.1",
  "maumau": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/maumau.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$MAUMAU_ENTRY" plugins install "$dir_plugin"
node "$MAUMAU_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/maumau-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@maumau/demo-plugin-file",
  "version": "0.0.1",
  "maumau": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/maumau.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$MAUMAU_ENTRY" plugins install "file:$file_pack_dir/package"
node "$MAUMAU_ENTRY" plugins list --json > /tmp/plugins4.json
...
bundle_root="$HOME/.maumau/extensions/claude-bundle-e2e"
...
const configPath = path.join(process.env.HOME, ".maumau", "maumau.json");
...
gateway_log="/tmp/maumau-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/maumau-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@maumau/slash-install-plugin",
  "version": "0.0.1",
  "maumau": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/maumau.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "maumau/fixture-marketplace"
    }
  }
}
JSON

node "$MAUMAU_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$MAUMAU_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$MAUMAU_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$MAUMAU_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$MAUMAU_ENTRY" plugins update marketplace-shortcut --dry-run
node "$MAUMAU_ENTRY" plugins update marketplace-shortcut
node "$MAUMAU_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
