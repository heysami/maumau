...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __maumauDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__maumauDiagnosticEventsState) {
    globalStore.__maumauDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__maumauDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: MaumauConfig): boolean {
