...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Maumau HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `maumau pairing approve slack <code>`.
...
- The interactive callback values are Maumau-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Maumau falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Maumau is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Maumau",
    "description": "Slack connector for Maumau"
  },
  "features": {
    "bot_user": {
      "display_name": "Maumau",
    ...
    "slash_commands": [
      {
        "command": "/maumau",
        "description": "Send a message to Maumau",
        "should_escape": false
      }
    ]
  },
...
Maumau supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Maumau falls back to normal delivery for remaining payloads.
