...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://maumau.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://maumau.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://maumau.ai/abc-123", baseHref)).toBe(
      "blob:https://maumau.ai/abc-123",
    );
  });
