...
read_when:
  ...
  - You want a reference for all registration methods on MaumauPluginApi
...
The `register(api)` callback receives an `MaumauPluginApi` object with these
methods:
...
| `api.config`             | `MaumauConfig`            | Current config snapshot                                   |
...
<Warning>
  Never import your own plugin through `maumau/plugin-sdk/<your-plugin>`
