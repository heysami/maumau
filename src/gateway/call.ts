...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.MAUMAU_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.MAUMAU_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set MAUMAU_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: maumau doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__MAUMAU_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
