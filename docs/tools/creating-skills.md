...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.maumau/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Maumau picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    maumau agent --message "give me a greeting"
    ```
...
| `metadata.maumau.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.maumau.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.maumau.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `maumau agent --message "..."` to test before sharing
...
| `~/.maumau/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Maumau) | Lowest     | Global                |
