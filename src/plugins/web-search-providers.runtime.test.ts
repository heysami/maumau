...
let loadMaumauPluginsMock: ReturnType<typeof vi.fn>;
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadPluginManifestRegistryMock = vi
      ...
      .mockReturnValue({
        plugins: [
          {
            ...
            manifestPath: "/tmp/brave/maumau.plugin.json",
            channels: [],
            providers: [],
            skills: [],
            hooks: [],
          ...
          },
          {
            ...
            manifestPath: "/tmp/noise/maumau.plugin.json",
            channels: [],
            providers: [],
            skills: [],
            hooks: [],
        ...
        ? R
        : never);
    loadMaumauPluginsMock = vi
      .spyOn(loaderModule, "loadMaumauPlugins")
  ...
  it("scopes plugin loading to manifest-declared web-search candidates", () => {
    ...
    expect(loadMaumauPluginsMock).toHaveBeenCalledWith(
  ...
  it("memoizes snapshot provider resolution for the same config and env", () => {
    ...
    const env = { MAUMAU_HOME: "/tmp/maumau-home" } as NodeJS.ProcessEnv;
  ...
  it("invalidates the snapshot cache when config or env contents change in place", () => {
    ...
    env.MAUMAU_HOME = "/tmp/maumau-home-b";
    resolvePluginWebSearchProviders({
      config,
      env,
      bundledAllowlistCompat: true,
    ...
    expect(loadMaumauPluginsMock).toHaveBeenCalledTimes(2);
  });
  ...
  it("skips web-search snapshot memoization when discovery cache ttl is zero", () => {
    ...
    const env = {
      MAUMAU_HOME: "/tmp/maumau-home",
      MAUMAU_PLUGIN_DISCOVERY_CACHE_MS: "0",
    } as NodeJS.ProcessEnv;
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const env = {
      MAUMAU_HOME: "/tmp/maumau-home",
      MAUMAU_PLUGIN_DISCOVERY_CACHE_MS: "5",
      MAUMAU_PLUGIN_MANIFEST_CACHE_MS: "20",
    } as NodeJS.ProcessEnv;
  ...
  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    ...
    const env = {
      MAUMAU_HOME: "/tmp/maumau-home",
      MAUMAU_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    } as NodeJS.ProcessEnv;
    ...
    env.MAUMAU_PLUGIN_DISCOVERY_CACHE_MS = "5";
  ...
  it("prefers the active plugin registry for runtime resolution", () => {
    ...
    expect(loadMaumauPluginsMock).not.toHaveBeenCalled();
  });
});
