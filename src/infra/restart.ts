...
export function triggerMaumauRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(process.env.MAUMAU_SYSTEMD_UNIT, process.env.MAUMAU_PROFILE);
  ...
  const label =
    process.env.MAUMAU_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.MAUMAU_PROFILE);
