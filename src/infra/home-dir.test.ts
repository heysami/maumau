...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers MAUMAU_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for MAUMAU_HOME", () => {
    const result = resolveRequiredHomeDir(
      { MAUMAU_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when MAUMAU_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ MAUMAU_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("resolveOsHomeDir", () => {
  it("ignores MAUMAU_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          MAUMAU_HOME: "/srv/maumau-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { MAUMAU_HOME: "/srv/maumau-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/maumau-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/maumau-home " },
      expected: "/srv/maumau-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/maumau-home/docs"));
  });
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of MAUMAU_HOME", () => {
    expect(
