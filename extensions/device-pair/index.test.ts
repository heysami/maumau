...
import type { MaumauPluginCommandDefinition, PluginCommandContext } from "maumau/plugin-sdk/core";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredMaumauTmpDir: vi.fn(() => path.join(os.tmpdir(), "maumau-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredMaumauTmpDir: pluginApiMocks.resolvePreferredMaumauTmpDir,
...
function createApi(params?: {
  runtime?: MaumauPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as MaumauPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: MaumauPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as MaumauPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredMaumauTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredMaumauTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Maumau iOS app:");
    expect(text).toContain("![Maumau pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Maumau iOS app:");
