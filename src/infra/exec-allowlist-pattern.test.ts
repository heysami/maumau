...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevMaumauHome = process.env.MAUMAU_HOME;
    const prevHome = process.env.HOME;
    process.env.MAUMAU_HOME = "/srv/maumau-home";
    process.env.HOME = "/home/other";
    const maumauHome = path.join(path.resolve("/srv/maumau-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", maumauHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevMaumauHome === undefined) {
        delete process.env.MAUMAU_HOME;
      } else {
        process.env.MAUMAU_HOME = prevMaumauHome;
      }
