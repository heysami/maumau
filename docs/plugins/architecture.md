...
read_when:
  - Building or debugging native Maumau plugins
...
This page covers the internal architecture of the Maumau plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Maumau. Every
native Maumau plugin registers against one or more capability types:
...
Maumau classifies every loaded plugin into a shape based on its actual
...
Use `maumau plugins inspect <id>` to see a plugin's shape and capability
...
When you run `maumau doctor` or `maumau plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `maumau status --all` and `maumau plugins doctor`.

## Architecture overview

Maumau's plugin system has four layers:

1. **Manifest + discovery**
   Maumau finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `maumau.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Maumau plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Maumau reads the registry to expose tools, channels, provider
...
That split lets Maumau validate config, explain missing/disabled plugins, and
...
normal chat actions. Maumau keeps one shared `message` tool in core, and
...
Maumau treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Maumau-facing
  surfaces
...
So if Maumau adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Maumau has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { MaumauPluginDefinition } from "maumau/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "maumau/plugin-sdk";

const plugin: MaumauPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Maumau already treats image/audio/video understanding as one shared
...
If Maumau adds a new domain later, such as video generation, use the same
...
`MaumauPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Maumau can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Maumau knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `MaumauPluginApi` or
  `api.runtime`
...
Native Maumau plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because Maumau currently treats them
...
name: `@maumau/<id>` by default, or an approved typed suffix such as
...
Maumau exports capabilities, not implementation convenience.
...
At startup, Maumau does roughly this:
...
The manifest is the control-plane source of truth. Maumau uses it to:
...
Maumau keeps short in-process caches for:
...
- Set `MAUMAU_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `MAUMAU_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `MAUMAU_PLUGIN_DISCOVERY_CACHE_MS` and
  `MAUMAU_PLUGIN_MANIFEST_CACHE_MS`.
...
Maumau still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Maumau calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Maumau tries the normal registry/catalog path first                                      | _(not a plugin hook)_                                                                |
...
that still runs on Maumau's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Maumau's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Maumau adds those
...
- If Maumau adds a new capability such as video generation later, define the
...
- Maumau only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Maumau plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `maumau/plugin-sdk` import when
authoring plugins:

- `maumau/plugin-sdk/plugin-entry` for plugin registration primitives.
- `maumau/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `maumau/plugin-sdk/channel-setup`,
  `maumau/plugin-sdk/channel-pairing`,
  `maumau/plugin-sdk/channel-contract`,
  `maumau/plugin-sdk/channel-feedback`,
  `maumau/plugin-sdk/channel-inbound`,
  `maumau/plugin-sdk/channel-lifecycle`,
  `maumau/plugin-sdk/channel-reply-pipeline`,
  `maumau/plugin-sdk/command-auth`,
  `maumau/plugin-sdk/secret-input`, and
  `maumau/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `maumau/plugin-sdk/channel-config-helpers`,
  `maumau/plugin-sdk/allow-from`,
  `maumau/plugin-sdk/channel-config-schema`,
  `maumau/plugin-sdk/channel-policy`,
  `maumau/plugin-sdk/config-runtime`,
  `maumau/plugin-sdk/infra-runtime`,
  `maumau/plugin-sdk/agent-runtime`,
  `maumau/plugin-sdk/lazy-runtime`,
  `maumau/plugin-sdk/reply-history`,
  `maumau/plugin-sdk/routing`,
  `maumau/plugin-sdk/status-helpers`,
  `maumau/plugin-sdk/runtime-store`, and
  `maumau/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `maumau/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `maumau/plugin-sdk/*` subpaths. Maumau core/test code may use the repo
...
- Avoid the root `maumau/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `maumau/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `maumau/plugin-sdk/channel-actions`.
...
`maumau/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Maumau writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Maumau's
...
- if both `catalog` and `discovery` are registered, Maumau uses `catalog`
...
- Read-only command paths such as `maumau status`, `maumau status --all`,
  `maumau channels status`, `maumau channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `maumau.extensions`:
...
Security guardrail: every `maumau.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `maumau plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `maumau.setupEntry` can point at a lightweight setup-only module.
When Maumau needs setup surfaces for a disabled channel plugin, or
...
Optional: `maumau.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Maumau load the
...
Channel plugins can advertise setup/discovery metadata via `maumau.channel` and
install hints via `maumau.install`. This keeps the core catalog data-free.
...
Maumau can also merge **external channel catalogs** (for example, an MPM
...
Or point `MAUMAU_PLUGIN_CATALOG_PATHS` (or `MAUMAU_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "maumau": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "maumau/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `MaumauPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Maumau stays opinionated without becoming hardcoded to one
