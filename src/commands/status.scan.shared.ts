...
export function hasExplicitMemorySearchConfig(cfg: MaumauConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: MaumauConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: MaumauConfig, agentId: string) => { store: { path: string } } | null;
