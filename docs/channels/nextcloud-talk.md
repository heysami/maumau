...
```bash
maumau plugins install @maumau/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
maumau plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Maumau" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `maumau pairing list nextcloud-talk`
  - `maumau pairing approve nextcloud-talk <CODE>`
