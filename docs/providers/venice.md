---
summary: "Use Venice AI privacy-focused models in Maumau"
read_when:
  - You want privacy-focused inference in Maumau
...
```bash
maumau onboard --auth-choice venice-api-key
```
...
```bash
maumau agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Maumau shows all available Venice models. Pick based on your needs:
...
```bash
maumau models set venice/kimi-k2-5
maumau models set venice/claude-opus-4-6
```

List all available models:

```bash
maumau models list | grep venice
```

## Configure via `maumau configure`

1. Run `maumau configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Maumau automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
maumau agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
maumau agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
maumau agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
maumau agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
maumau agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `maumau models list` to see currently available models. Some models may be temporarily offline.
