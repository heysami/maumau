...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const maumauTmpDir = path.resolve(resolvePreferredMaumauTmpDir());
  if (!isPathInside(maumauTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: maumauTmpDir });
  return resolved;
}
