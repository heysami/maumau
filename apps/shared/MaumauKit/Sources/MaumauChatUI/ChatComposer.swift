...
@MainActor
struct MaumauChatComposer: View {
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(MaumauChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(MaumauChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \MaumauPendingAttachment.id)
                { (att: MaumauPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            MaumauPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(MaumauChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(MaumauChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(MaumauChatTheme.subtleCard)
        .clipShape(Capsule())
    }
