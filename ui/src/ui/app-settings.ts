...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as MaumauApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as MaumauApp);
  }
  ...
  if (host.tab === "skills") {
    await loadSkills(host as unknown as MaumauApp);
  }
  if (host.tab === "agents") {
    ...
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as MaumauApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as MaumauApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as MaumauApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as MaumauApp, false);
      }
  ...
  if (host.tab === "nodes") {
    ...
    await loadConfig(host as unknown as MaumauApp);
    await loadExecApprovals(host as unknown as MaumauApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "logs") {
    ...
    await loadLogs(host as unknown as MaumauApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__MAUMAU_CONTROL_UI_BASE_PATH__;
...
async function loadOverviewLogs(host: MaumauApp) {
...
function buildAttentionItems(host: MaumauApp) {
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as MaumauApp, true),
    loadConfigSchema(host as unknown as MaumauApp),
    loadConfig(host as unknown as MaumauApp),
  ]);
}
