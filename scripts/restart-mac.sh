#!/usr/bin/env bash
# Reset Maumau like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.
...
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Maumau"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Maumau"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.maumau.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/maumau-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${MAUMAU_RESTART_LOG:-/tmp/maumau-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${MAUMAU_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.maumau/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  MAUMAU_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node maumau.mjs daemon install --force --runtime node"
      log "  node maumau.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.maumau/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.maumau.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Maumau instances"
kill_all_maumau
stop_launch_agent
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Maumau.app" ]]; then
    APP_BUNDLE="/Applications/Maumau.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Maumau.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Maumau.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Maumau.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set MAUMAU_APP_BUNDLE to your installed Maumau.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node maumau.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node maumau.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".maumau", "maumau.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: Maumau is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.maumau.gateway.plist' | head -n 40 || true"
fi
