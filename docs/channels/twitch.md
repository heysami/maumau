...
read_when:
  - Setting up Twitch chat integration for Maumau
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Maumau connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
maumau plugins install @maumau/twitch
```

Local checkout (when running from a git repo):

```bash
maumau plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `MAUMAU_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "maumau", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use MAUMAU_TWITCH_ACCESS_TOKEN env var)
...
```bash
MAUMAU_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
