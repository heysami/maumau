...
export function createGatewayReloadHandlers(params: {
  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (MAUMAU_SKIP_GMAIL_WATCHER=1)"),
      });
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (MAUMAU_SKIP_CHANNELS=1 or MAUMAU_SKIP_PROVIDERS=1)",
        );
      } else {
  ...
  const requestGatewayRestart = (
    ...
    const getActiveCounts = () => {
      ...
      const runningWizards = Math.max(0, Math.floor(params.getRunningWizardCount?.() ?? 0));
      return {
        queueSize,
        pendingReplies,
        embeddedRuns,
        runningWizards,
        totalActive: queueSize + pendingReplies + embeddedRuns + runningWizards,
      };
    };
    const formatActiveDetails = (counts: ReturnType<typeof getActiveCounts>) => {
      ...
      if (counts.runningWizards > 0) {
        details.push(`${counts.runningWizards} setup wizard session(s)`);
      }
      return details;
    };
