...
import { ensureMaumauExecMarkerOnProcess } from "./infra/maumau-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "maumau.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "maumau.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `Maumau ${VERSION} (${commit})` : `Maumau ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[maumau] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureCliRespawnReady()) {
    ...
    if (!parsedContainer.ok) {
      console.error(`[maumau] ${parsedContainer.error}`);
      process.exit(2);
    }
    ...
    if (!parsed.ok) {
      ...
      console.error(`[maumau] ${parsed.error}`);
      process.exit(2);
    }
    ...
    if (
      containerTargetName &&
      (parsed.profile ||
        process.env.MAUMAU_PROFILE?.trim() ||
        process.env.MAUMAU_GATEWAY_PORT?.trim() ||
        process.env.MAUMAU_GATEWAY_URL?.trim() ||
        process.env.MAUMAU_GATEWAY_TOKEN?.trim() ||
        process.env.MAUMAU_GATEWAY_PASSWORD?.trim())
    ) {
      console.error(
        "[maumau] --container cannot be combined with --profile/--dev or gateway override env vars",
      );
      process.exit(2);
    }
