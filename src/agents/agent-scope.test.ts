...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: MaumauConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: MaumauConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: MaumauConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: MaumauConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: MaumauConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: MaumauConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/maumau");
  });

  it("uses MAUMAU_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "maumau-home");
  ...
  it("uses MAUMAU_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "maumau-home");
    vi.stubEnv("MAUMAU_HOME", home);
    // Clear state dir so it falls back to MAUMAU_HOME
    vi.stubEnv("MAUMAU_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as MaumauConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".maumau", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/maumau-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/maumau-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-agent-scope-"));
