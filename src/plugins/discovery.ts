...
function resolveDiscoveryCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MAUMAU_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseDiscoveryCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.MAUMAU_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @maumau/voice-call -> voice-call).
    ...
    addCandidate({
      ...
      manifest: {
        ...
        ...(!getPackageManifestMetadata(packageManifest ?? undefined) && entry.packageManifest
          ? { [MANIFEST_KEY]: entry.packageManifest }
          : {}),
      },
      packageDir: rootDir,
      bundledManifest: entry.manifest,
      bundledManifestPath: path.join(rootDir, "maumau.plugin.json"),
    });
  }
...
export function discoverMaumauPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ownershipUid?: number | null;
  cache?: boolean;
