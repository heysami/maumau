...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): MaumauConfig {
    const defaults: NonNullable<NonNullable<MaumauConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: MaumauConfig) {
