...
**Import:** `maumau/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                     | No       | —                   |
| `configSchema` | `MaumauPluginConfigSchema \| () => MaumauPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: MaumauPluginApi) => void`                             | Yes      | —                   |

- `id` must match your `maumau.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.

## `defineChannelPluginEntry`

**Import:** `maumau/plugin-sdk/core`
...
| `registerFull` | `(api: MaumauPluginApi) => void`                             | No       | —                   |
...
Maumau loads this instead of the full entry when a channel is disabled,
...
Maumau classifies loaded plugins by their registration behavior:
...
Use `maumau plugins inspect <id>` to see a plugin's shape.
