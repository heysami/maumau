...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async (importOriginal) => {
    ...
    return {
      ...mod,
      ensureMaumauModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): MaumauConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): MaumauConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): MaumauConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `maumau-auth-rotation-${Date.now()}.log`),
    });
