---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Maumau agents"
read_when:
...
OpenShell is a managed sandbox backend for Maumau. Instead of running Docker
containers locally, Maumau delegates sandbox lifecycle to the `openshell` CLI,
...
- Maumau Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Maumau creates an OpenShell
...
- You edit files locally outside Maumau and want those changes visible in the
...
- When the sandbox is first created, Maumau seeds the remote workspace from
...
- Maumau does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Maumau after the initial seed,
the remote sandbox does **not** see those changes. Use
`maumau sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"maumau"`    | Sandbox source for first-time create                  |
...
1. Maumau calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Maumau calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `maumau sandbox` commands
