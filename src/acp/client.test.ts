...
const createTempDir = () => tempDirs.make("maumau-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets MAUMAU_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "maumau",
    });

    expect(env.MAUMAU_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("maumau");
  });

  it("overrides pre-existing MAUMAU_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      MAUMAU_SHELL: "wrong",
    });
    expect(env.MAUMAU_SHELL).toBe("acp-client");
  ...
  it("preserves MAUMAU_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        MAUMAU_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["MAUMAU_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Maumau bridge", () => {
    ...
    expect(env.MAUMAU_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.MAUMAU_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "maumau",
        serverArgs: ["acp"],
        defaultServerCommand: "maumau",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("MAUMAU_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "maumau", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "maumau", "dist", "entry.js");
    const shimPath = path.join(dir, "maumau.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\maumau\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/maumau-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/maumau-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/maumau-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
