  ...
  it("derives recipient hints in channel-first order with masked slugs", () => {
    expect(
      deriveRecipientHint({
        ...
        requesterTailscaleLogin: "taylor@example.com",
      }),
    ).toEqual(
      expect.objectContaining({
        source: "sender_username",
        normalizedSlug: "taylor",
        maskedSlug: "tay-or",
        displayLabel: "@Taylor",
        verified: true,
      }),
      ...
      async () => {
        await withTempPreviewEnv(
          ...
          async ({ workspaceDir }) => {
            ...
            expect(result.recipientHint).toBe("tay-or");
      ...
      async () => {
        await withTempPreviewEnv(
          ...
          async ({ workspaceDir }) => {
            ...
            expect(result.url).toContain("/preview/for-tay-or/");
          },
        );
      },
    );
    ...
    await withFakeTailscaleBinary({ funnelStatusJson: '{"12345":{"HTTPS":true}}' }, async () => {
      await withTempPreviewEnv(
        ...
        async ({ workspaceDir }) => {
          ...
          expect(result.url).toContain("/share/for-tay-or/");
      ...
      async () => {
        await withTempPreviewEnv(
          ...
          async ({ workspaceDir }) => {
            ...
            expect(authorized.getBody()).toContain("Created for @taylor");
