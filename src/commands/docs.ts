...
const SEARCH_TOOL = "https://docs.maumau.ai/mcp.SearchMaumau";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.maumau.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('maumau docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.maumau.ai/");
      runtime.log(`Search: ${formatCliCommand('maumau docs "your query"')}`);
    }
    return;
  }
