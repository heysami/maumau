...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/maumau/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/maumau");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/maumau/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__MAUMAU_CONTROL_UI_BASE_PATH__ = "/maumau";
    const app = mountApp("/maumau/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/maumau");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/maumau/sessions");
  });
  ...
  it("hydrates token from query params and strips them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("maumau.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "maumau.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/maumau" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("maumau.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/maumau",
    });
    expect(JSON.parse(localStorage.getItem("maumau.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/maumau";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/maumau");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp("/ui/overview?gatewayUrl=wss://other-gateway.example/maumau#token=abc123");
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/maumau");
    expect(app.settings.token).toBe("");
