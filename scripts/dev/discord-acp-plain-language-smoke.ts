...
type DriverMode = "token" | "webhook" | "maumau";
...
function resolveStateDir(): string {
  const override = process.env.MAUMAU_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.MAUMAU_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".maumau");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver maumau] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Maumau spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|maumau> Driver transport mode (default: token)\n" +
    ...
    "  --maumau-bin <path>        Maumau CLI binary for driver=maumau (default: maumau)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  MAUMAU_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  MAUMAU_DISCORD_SMOKE_DRIVER\n" +
    "  MAUMAU_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  MAUMAU_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  MAUMAU_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  MAUMAU_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  MAUMAU_DISCORD_SMOKE_AGENT\n" +
    "  MAUMAU_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  MAUMAU_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  MAUMAU_DISCORD_SMOKE_POLL_MS\n" +
    "  MAUMAU_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  MAUMAU_DISCORD_SMOKE_MAUMAU_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.MAUMAU_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.MAUMAU_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "maumau"
        ? "maumau"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken = resolveArg("--token") || process.env.MAUMAU_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.MAUMAU_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.MAUMAU_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.MAUMAU_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent = resolveArg("--agent") || process.env.MAUMAU_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.MAUMAU_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.MAUMAU_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.MAUMAU_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.MAUMAU_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.MAUMAU_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const maumauBin =
    resolveArg("--maumau-bin") || process.env.MAUMAU_DISCORD_SMOKE_MAUMAU_BIN || "maumau";
...
async function maumauCliJson<T>(params: { maumauBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.maumauBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`maumau ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithMaumau(params: {
  maumauBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await maumauCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    maumauBin: params.maumauBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "maumau") {
    return await readMessagesWithMaumau({
      maumauBin: params.args.maumauBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `maumau-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await maumauCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("maumau message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "maumau"
            ? await readMessagesWithMaumau({
                maumauBin: args.maumauBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Maumau.`,
        diagnostics: {
          bindingCandidates: [
            {
