...
read_when:
  ...
  - Wiring external systems into Maumau
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Maumau config directory (typically `~/.maumau/hooks/transforms`).
...
- `maumau webhooks gmail setup` writes `hooks.gmail` config for `maumau webhooks gmail run`.
