...
This guide summarizes a sane workflow for working on the pi integration in Maumau.
...
```bash
MAUMAU_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm maumau agent --message "Hello" --thinking low`
...
State lives under the Maumau state directory. Default is `~/.maumau`. If `MAUMAU_STATE_DIR` is set, use that directory instead.

To reset everything:

- `maumau.json` for config
