...
summary: "Complete reference for every Maumau config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.maumau/maumau.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Maumau uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.maumau/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `maumau doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `maumau doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["maumau-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-maumau",
          requireMention: false,
...
- Maumau additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `maumau plugins install @maumau/mattermost`.
...
- `commands.callbackUrl` must resolve to the Maumau gateway endpoint and be reachable from the Mattermost server.
...
Maumau spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `maumau channels add` (or channel onboarding) while still on a single-account top-level channel config, Maumau moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `maumau doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@maumau", "maumau"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@maumau"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `maumau.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Maumau auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/maumau" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Maumau assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Maumau aborts it. Default: `900`.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.maumau/sandboxes",
        docker: {
          image: "maumau-sandbox:bookworm-slim",
          containerPrefix: "maumau-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "maumau-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/maumau-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "maumau-sandbox-browser:bookworm-slim",
          network: "maumau-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Maumau materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.maumau/sandboxes`
...
In `remote` mode, host-local edits made outside Maumau are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `maumau.json`.
noVNC observer access uses VNC auth by default and Maumau emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `maumau-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from MAUMAU_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `MAUMAU_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `MAUMAU_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `MAUMAU_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.maumau/workspace",
        agentDir: "~/.maumau/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@maumau"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.maumau/workspace-home" },
      { id: "work", workspace: "~/.maumau/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Maumau resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.maumau/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.maumau/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.maumau/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.maumau/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Maumau starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.maumau/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Maumau treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:maumau`     | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.maumau/attachments/<uuid>/` with a `.manifest.json`.
...
Maumau uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.maumau/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `MAUMAU_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Maumau forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `maumau onboard --auth-choice opencode-zen` or `maumau onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `maumau onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `maumau onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `maumau onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `maumau onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `maumau onboard --auth-choice minimax-api`.
...
- Loaded from `~/.maumau/extensions`, `<workspace>/.maumau/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Maumau plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Maumau plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Maumau applies those as sanitized agent settings, not as raw Maumau config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `maumau plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or MAUMAU_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `MAUMAU_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `MAUMAU_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `MAUMAU_APNS_RELAY_BASE_URL` / `MAUMAU_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `MAUMAU_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
MAUMAU_CONFIG_PATH=~/.maumau/a.json \
MAUMAU_STATE_DIR=~/.maumau-a \
maumau gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.maumau-dev` + port `19001`), `--profile <name>` (uses `~/.maumau-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.maumau/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-maumau-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `MAUMAU_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.maumau/workspace/canvas",
    liveReload: true,
    // enabled: false, // or MAUMAU_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__maumau__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__maumau__/a2ui/`
...
- Also serves A2UI at `/__maumau__/a2ui/`.
...
- Hostname defaults to `maumau`. Override with `MAUMAU_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.maumau/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `maumau dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.maumau/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${MAUMAU_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `maumau.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.maumau/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/maumau-vault-resolver",
...
- Legacy OAuth imports from `~/.maumau/credentials/oauth.json`.
...
- Default log file: `/tmp/maumau/maumau-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Maumau.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `MAUMAU_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `maumau doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `maumau.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
