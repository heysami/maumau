...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.maumau/maumau.json. Prefer MAUMAU_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.maumau/maumau.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.maumau/maumau.json. Restart the gateway to apply."
        }
