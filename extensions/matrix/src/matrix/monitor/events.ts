...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Maumau runtime is using the same account. " +
    `Check 'maumau matrix verify status --verbose --account ${accountId}' and 'maumau matrix devices list --account ${accountId}'.`
  );
}
