...
const TEST_MANAGED_HOME = "/tmp/maumau-test-home";
const GATEWAY_SERVICE = "maumau-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/maumau gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/maumau", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/maumau-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "maumau-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "maumau-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/maumau-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled maumau-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled maumau-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when MAUMAU_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/maumau-gateway.service",
    },
    {
      name: "uses profile-specific service name when MAUMAU_PROFILE is set to a custom value",
      env: { HOME: "/home/test", MAUMAU_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/maumau-gateway-jbphoenix.service",
    },
    {
      name: "prefers MAUMAU_SYSTEMD_UNIT over MAUMAU_PROFILE",
      env: {
        HOME: "/home/test",
        MAUMAU_PROFILE: "jbphoenix",
        MAUMAU_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles MAUMAU_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        MAUMAU_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from MAUMAU_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        MAUMAU_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/maumau gateway start --name "My Bot"')).toEqual([
      "/usr/bin/maumau",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('maumau --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["maumau", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('maumau --path "C:\\\\Program Files\\\\Maumau"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["maumau", "--path", "C:\\\\Program Files\\\\Maumau"]);

    expect(
      splitArgsPreservingQuotes('maumau --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["maumau", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/maumau gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads MAUMAU_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/maumau gateway run", "EnvironmentFile=%h/.maumau/.env"],
      { [`${TEST_SERVICE_HOME}/.maumau/.env`]: "MAUMAU_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.MAUMAU_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/maumau gateway run",
        "EnvironmentFile=%h/.maumau/.env",
        'Environment="MAUMAU_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.maumau/.env`]: "MAUMAU_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.MAUMAU_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.MAUMAU_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.maumau/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/maumau-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/maumau gateway run",
          'EnvironmentFile=%h/.maumau/first.env "%h/.maumau/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.maumau/first.env") {
        return "MAUMAU_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.maumau/second env.env") {
        return 'MAUMAU_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ...
      MAUMAU_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "MAUMAU_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "MAUMAU_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "MAUMAU_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      MAUMAU_GATEWAY_TOKEN: "override-token",
      MAUMAU_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/maumau-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/maumau gateway run",
          "EnvironmentFile=%h/.maumau/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.maumau/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'MAUMAU_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "MAUMAU_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      MAUMAU_GATEWAY_TOKEN: "quoted token",
      MAUMAU_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      MAUMAU_GATEWAY_TOKEN: "file",
      MAUMAU_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "maumau-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ MAUMAU_PROFILE: "work" });
  });
