...
describe("loadEnabledBundleMcpConfig", () => {
  it("loads enabled Claude bundle MCP config and absolutizes relative args", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "MAUMAU_HOME", "MAUMAU_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("maumau-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("maumau-bundle-mcp-workspace-");
  ...
  it("merges inline bundle MCP servers and skips disabled bundles", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "MAUMAU_HOME", "MAUMAU_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("maumau-bundle-inline-home-");
      const workspaceDir = await tempHarness.createTempDir("maumau-bundle-inline-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.MAUMAU_HOME;
      delete process.env.MAUMAU_STATE_DIR;

      const enabledRoot = path.join(homeDir, ".maumau", "extensions", "inline-enabled");
      const disabledRoot = path.join(homeDir, ".maumau", "extensions", "inline-disabled");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "MAUMAU_HOME", "MAUMAU_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("maumau-bundle-inline-placeholder-home-");
      const workspaceDir = await tempHarness.createTempDir(
        "maumau-bundle-inline-placeholder-workspace-",
      );
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.MAUMAU_HOME;
      ...
      const pluginRoot = path.join(homeDir, ".maumau", "extensions", "inline-claude");
