...
vi.mock("./chrome.js", () => ({
  resolveMaumauUserDataDir: vi.fn(() => "/tmp/maumau-test/maumau/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "maumau-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveMaumauUserDataDir).mockReturnValue(userDataDir);
