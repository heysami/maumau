    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "maumau.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "maumau.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "maumau.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("maumau-");
  const acpxRoot = await tempDirs.make("maumau-acpx-plugin-");
  const helperRoot = await tempDirs.make("maumau-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("maumau-");
  const pluginRoot = await tempDirs.make("maumau-plugin-");
  const outsideDir = await tempDirs.make("maumau-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("maumau-");
    const pluginRoot = await tempDirs.make("maumau-claude-bundle-");
