...
import { info, success } from "maumau/plugin-sdk/runtime-env";
...
import { resolveOAuthDir } from "maumau/plugin-sdk/state-paths";
import type { WebChannel } from "maumau/plugin-sdk/text-runtime";
import { jidToE164, resolveUserPath } from "maumau/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("maumau channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
