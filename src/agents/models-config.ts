...
import { planMaumauModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  config: MaumauConfig;
  sourceConfigForSecrets: MaumauConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: MaumauConfig): {
  config: MaumauConfig;
  sourceConfigForSecrets: MaumauConfig;
} {
...
export async function ensureMaumauModelsJson(
  config?: MaumauConfig,
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planMaumauModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
