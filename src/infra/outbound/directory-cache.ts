...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: MaumauConfig | null = null;
  ...
  get(key: string, cfg: MaumauConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: MaumauConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: MaumauConfig): void {
