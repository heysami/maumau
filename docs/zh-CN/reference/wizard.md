...
<Steps>
  ...
  <Step title="模型/认证">
    ...
    - OAuth 凭证保存在 `~/.maumau/credentials/oauth.json`；auth-profile 保存在 `~/.maumau/agents/<agentId>/agent/auth-profiles.json`（API key + OAuth）。
    - 更多细节： [/concepts/oauth](/concepts/oauth)
    <Note>
    无头/服务器提示：在有浏览器的机器上完成 OAuth，然后将
    `~/.maumau/credentials/oauth.json`（或 `$MAUMAU_STATE_DIR/credentials/oauth.json`）复制到
    Gateway 网关主机上。
    </Note>
  </Step>
  <Step title="工作区">
    - 默认为 `~/.maumau/workspace`（可配置）。
    - 为工作区植入智能体引导仪式所需的文件。
  ...
  <Step title="渠道">
    ...
    - 私信安全：默认为 pairing。首次私信会发送一个代码；通过 `maumau pairing approve <channel> <code>` 批准，或使用允许列表。
  </Step>
  <Step title="Web 搜索">
    - 选择一个提供商：Perplexity、Brave、Gemini、Grok 或 Kimi（也可跳过）。
    - 粘贴你的 API key（QuickStart 会自动从环境变量或现有配置中检测 key）。
    - 使用 `--skip-search` 跳过。
    - 之后再配置：`maumau configure --section web`。
  </Step>
  <Step title="守护进程安装">
    - macOS：LaunchAgent
  ...
  <Step title="健康检查">
    - 启动 Gateway 网关（如果需要）并运行 `maumau health`。
    - 提示：`maumau status --deep` 会在状态输出中添加 Gateway 网关健康探测（需要能访问到 Gateway 网关）。
  </Step>
...
- 下载适合的发布资源。
- 将其存储到 `~/.maumau/tools/signal-cli/<version>/` 下。
...
`maumau agents add` 会写入 `agents.list[]` 和可选的 `bindings`。

WhatsApp 凭证位于 `~/.maumau/credentials/whatsapp/<accountId>/` 下。
会话存储在 `~/.maumau/agents/<agentId>/sessions/` 下。
