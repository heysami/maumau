...
- `/usage cost` in chats: local cost summary aggregated from Maumau session logs.
- CLI: `maumau status --usage` prints a full per-provider breakdown.
- CLI: `maumau channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
