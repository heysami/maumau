...
#
# Suggested cron: */30 * * * * /home/admin/maumau/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Maumau notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., maumau-alerts)
...
STATE_FILE="$HOME/.maumau/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Maumau if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use maumau
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Maumau to $NOTIFY_PHONE..."
            maumau send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Maumau is down. Run: ssh l36 '~/maumau/scripts/mobile-reauth.sh'" "urgent"
    exit 1
