...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "MAUMAU_ENV_TEST_A";
    const keyB = "MAUMAU_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "MAUMAU_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "MAUMAU_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "MAUMAU_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "MAUMAU_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "MAUMAU_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "MAUMAU_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "MAUMAU_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "maumau-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousMaumauHome = process.env.MAUMAU_HOME;
    const previousStateDir = process.env.MAUMAU_STATE_DIR;
    const previousBundledDir = process.env.MAUMAU_BUNDLED_PLUGINS_DIR;
    process.env.MAUMAU_HOME = "/srv/maumau-home";
    process.env.MAUMAU_STATE_DIR = "/srv/maumau-state";
    process.env.MAUMAU_BUNDLED_PLUGINS_DIR = "/srv/maumau-bundled";
    ...
    try {
      ...
      expect(env.MAUMAU_HOME).toBeUndefined();
      expect(env.MAUMAU_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.MAUMAU_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("MAUMAU_HOME", previousMaumauHome);
      restoreEnvKey("MAUMAU_STATE_DIR", previousStateDir);
      restoreEnvKey("MAUMAU_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { MAUMAU_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processMaumauHome: process.env.MAUMAU_HOME,
          processBundledDir: process.env.MAUMAU_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.MAUMAU_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processMaumauHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.MAUMAU_HOME).toBe("/srv/maumau-home");
    } finally {
      restoreEnvKey("MAUMAU_HOME", previousMaumauHome);
    }
  });
