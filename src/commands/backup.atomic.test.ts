...
describe("backupCreateCommand atomic archive write", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("maumau-backup-atomic-test-");
  });
  ...
  it("does not leave a partial final archive behind when tar creation fails", async () => {
    ...
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-backup-failure-"));
    try {
  ...
  it("does not overwrite an archive created after readiness checks complete", async () => {
    const stateDir = path.join(tempHome.home, ".maumau");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-backup-race-"));
  ...
  it("falls back to exclusive copy when hard-link publication is unsupported", async () => {
    const stateDir = path.join(tempHome.home, ".maumau");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-backup-copy-fallback-"));
    const linkSpy = vi.spyOn(fs, "link");
    try {
      await fs.writeFile(path.join(stateDir, "maumau.json"), JSON.stringify({}), "utf8");
      await fs.writeFile(path.join(stateDir, "state.txt"), "state\n", "utf8");
