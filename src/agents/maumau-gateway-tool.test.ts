...
let createMaumauTools: typeof import("./maumau-tools.js").createMaumauTools;

async function loadFreshMaumauToolsModuleForTest() {
  vi.resetModules();
  vi.doMock("./tools/gateway.js", () => createGatewayToolModuleMocks());
  ({ createMaumauTools } = await import("./maumau-tools.js"));
}
...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshMaumauToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-test-"));

    try {
      await withEnvAsync({ MAUMAU_STATE_DIR: stateDir, MAUMAU_PROFILE: "isolated" }, async () => {
        const tool = requireGatewayTool();
        ...
        expect(parsed.payload?.doctorHint).toBe(
          "Run: maumau --profile isolated doctor --non-interactive",
        );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/maumau" } }\n}\n';
