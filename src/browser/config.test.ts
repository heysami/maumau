...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.driver).toBe("maumau");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const maumau = resolveProfile(resolved, "maumau");
    expect(maumau?.driver).toBe("maumau");
    expect(maumau?.cdpPort).toBe(18800);
    expect(maumau?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from MAUMAU_GATEWAY_PORT when unset", () => {
    withEnv({ MAUMAU_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const maumau = resolveProfile(resolved, "maumau");
      expect(maumau?.cdpPort).toBe(19012);
      expect(maumau?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ MAUMAU_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const maumau = resolveProfile(resolved, "maumau");
      expect(maumau?.cdpPort).toBe(19022);
      expect(maumau?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(maumau?.cdpPort).toBe(19000);
    expect(maumau?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "maumau");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "maumau")!;
  ...
  describe("default profile preference", () => {
    it("defaults to maumau profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("maumau");
    });

    it("keeps maumau default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("maumau");
    ...
    it("keeps maumau default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("maumau");
    ...
    it("keeps maumau default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
