...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be inspected with `maumau hooks`, while hook-pack installation and updates now go through `maumau plugins`.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Maumau. See [Webhook Hooks](/automation/webhook) or use `maumau webhooks` for Gmail helper commands.

Hooks can also be bundled inside plugins; see [Plugin hooks](/plugins/architecture#provider-runtime-hooks). `maumau hooks list` shows both standalone hooks and plugin-managed hooks.
...
If you can write a small TypeScript function, you can write a hook. Managed and bundled hooks are trusted local code. Workspace hooks are discovered automatically, but Maumau keeps them disabled until you explicitly enable them via the CLI or config.
...
- Extend Maumau's behavior without modifying core code

## Getting Started

### Bundled Hooks

Maumau ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.maumau/workspace/memory/`) when you issue `/new` or `/reset`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.maumau/logs/commands.log`
...
```bash
maumau hooks enable session-memory
```
...
```bash
maumau hooks info session-memory
```

### Onboarding

During onboarding (`maumau onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.

### Trust Boundary

Hooks run inside the Gateway process. Treat bundled hooks, managed hooks, and `hooks.internal.load.extraDirs` as trusted local code. Workspace hooks under `<workspace>/hooks/` are repo-local code, so Maumau requires an explicit enable step before loading them.

## Hook Discovery

Hooks are automatically discovered from these directories, in order of increasing override precedence:

1. **Bundled hooks**: shipped with Maumau; located at `<maumau>/dist/hooks/bundled/` for npm installs (or a sibling `hooks/bundled/` for compiled binaries)
2. **Plugin hooks**: hooks bundled inside installed plugins (see [Plugin hooks](/plugins/architecture#provider-runtime-hooks))
3. **Managed hooks**: `~/.maumau/hooks/` (user-installed, shared across workspaces; can override bundled and plugin hooks). **Extra hook directories** configured via `hooks.internal.load.extraDirs` are also treated as managed hooks and share the same override precedence.
...
Hook packs are standard npm packages that export one or more hooks via `maumau.hooks` in
`package.json`. Install them with:

```bash
maumau plugins install <path-or-spec>
```
...
those to a prerelease, Maumau stops and asks you to opt in explicitly with a
...
Hook packs can ship dependencies; they will be installed under `~/.maumau/hooks/<id>`.
Each `maumau.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `maumau plugins install` installs hook-pack dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.maumau.ai/automation/hooks#my-hook
metadata:
  { "maumau": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.maumau` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Maumau persists them.
...
- **Managed hooks** (`~/.maumau/hooks/`): Shared across workspaces; can override bundled and plugin hooks

### 2. Create Directory Structure

```bash
mkdir -p ~/.maumau/hooks/my-hook
cd ~/.maumau/hooks/my-hook
```
...
metadata: { "maumau": { "emoji": "🎯", "events": ["command:new"] } }
---
...
maumau hooks list --eligible

# Verbose output (show missing requirements)
maumau hooks list --verbose

# JSON output
maumau hooks list --json
```
...
# JSON output
maumau hooks info session-memory --json
```
...
# Disable a hook
maumau hooks disable command-logger
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.maumau/workspace`)
...
```bash
maumau hooks enable bootstrap-extra-files
```
...
**Output**: `~/.maumau/logs/commands.log`
...
tail -n 20 ~/.maumau/logs/commands.log

# Pretty-print with jq
cat ~/.maumau/logs/commands.log | jq .

# Filter by action
grep '"action":"new"' ~/.maumau/logs/commands.log | jq .
```

**Enable**:

```bash
maumau hooks enable command-logger
```
...
```yaml
metadata: { "maumau": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "maumau": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.maumau/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.maumau/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.maumau/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/maumau/maumau/tree/main/src/hooks/bundled)
