...
```bash
maumau tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `maumau tui --deliver`
...
- Local shell commands receive `MAUMAU_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `MAUMAU_THEME=light` before launching `maumau tui`.
- To force the original dark palette instead, set `MAUMAU_THEME=dark`.
...
- Check the Gateway logs: `maumau logs --follow`.
- Confirm the agent can run: `maumau status` and `maumau models status`.
...
- No agents in picker: check `maumau agents list` and your routing config.
