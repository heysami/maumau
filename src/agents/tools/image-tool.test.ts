...
type CreateMaumauCodingToolsArgs = Parameters<PiToolsModule["createMaumauCodingTools"]>[0];
type MockMaumauToolsOptions = {
  config?: MaumauConfig;
  agentDir?: string;
  workspaceDir?: string;
  sandboxRoot?: string;
...
vi.mock("../maumau-tools.js", async () => {
  const { createImageTool } = await import("./image-tool.js");
  return {
    createMaumauTools: vi.fn((options?: MockMaumauToolsOptions) => {
...
async function createMaumauCodingToolsWithFreshModules(options?: CreateMaumauCodingToolsArgs) {
  vi.resetModules();
  const { createMaumauCodingTools } = await import("../pi-tools.js");
  return createMaumauCodingTools(options);
}

async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-image-"));
  try {
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(parentDir, "maumau-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): MaumauConfig {
  return {
    agents: {
      defaults: {
...
async function withTempSandboxState(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-image-sandbox-"));
...
describe("image tool implicit imageModel config", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-outside-"));
  ...
  it("allows workspace images via createMaumauCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createMaumauCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.maumau/media/inbound/photo.png",
      });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg = createMinimaxImageConfig();
