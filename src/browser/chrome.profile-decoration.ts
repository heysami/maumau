...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".maumau-profile-decorated");
}
 ...
 */
export function decorateMaumauProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MAUMAU_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MAUMAU_BROWSER_COLOR).toUpperCase();
