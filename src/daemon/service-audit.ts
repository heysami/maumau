...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds MAUMAU_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `maumau gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service MAUMAU_GATEWAY_TOKEN does not match gateway.auth.token in maumau.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.MAUMAU_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.MAUMAU_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `maumau gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
