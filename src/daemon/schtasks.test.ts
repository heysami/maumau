...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when MAUMAU_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".maumau", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when MAUMAU_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", MAUMAU_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".maumau-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers MAUMAU_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        MAUMAU_PROFILE: "rescue",
        MAUMAU_STATE_DIR: "C:\\State\\maumau",
      },
      expected: path.join("C:\\State\\maumau", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", MAUMAU_PROFILE: "default" },
      expected: path.join("/home/test", ".maumau", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Maumau Gateway",
          "cd /d C:\\Projects\\maumau",
          "set NODE_ENV=production",
          "set MAUMAU_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\maumau",
          environment: {
            NODE_ENV: "production",
            MAUMAU_PORT: "18789",
          },
  ...
  it("reads script from MAUMAU_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ MAUMAU_STATE_DIR: path.join(tmpDir, "custom-state") }),
