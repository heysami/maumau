...
const ensureTempArtifactDir = () => {
  if (tempArtifactDir === null) {
    tempArtifactDir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-test-parallel-"));
  }
...
const cleanupTempArtifacts = () => {
  ...
  if (process.env.MAUMAU_TEST_KEEP_TEMP_ARTIFACTS === "1") {
...
const rawTestProfile = process.env.MAUMAU_TEST_PROFILE?.trim().toLowerCase();
const testProfile =
  rawTestProfile === "low" ||
...
const forceIsolation =
  process.env.MAUMAU_TEST_ISOLATE === "1" || process.env.MAUMAU_TEST_ISOLATE === "true";
const disableIsolation =
  !forceIsolation &&
  process.env.MAUMAU_TEST_NO_ISOLATE !== "0" &&
  process.env.MAUMAU_TEST_NO_ISOLATE !== "false";
const includeGatewaySuite = process.env.MAUMAU_TEST_INCLUDE_GATEWAY === "1";
const includeChannelsSuite = process.env.MAUMAU_TEST_INCLUDE_CHANNELS === "1";
const includeExtensionsSuite = process.env.MAUMAU_TEST_INCLUDE_EXTENSIONS === "1";
const noIsolateArgs = disableIsolation ? ["--isolate=false"] : [];
const skipDefaultRuns = process.env.MAUMAU_TEST_SKIP_DEFAULT === "1";
...
let runs = [];
const shardOverride = Number.parseInt(process.env.MAUMAU_TEST_SHARDS ?? "", 10);
...
const shardIndexOverride = (() => {
  const parsed = Number.parseInt(process.env.MAUMAU_TEST_SHARD_INDEX ?? "", 10);
...
if (shardIndexOverride !== null && shardCount <= 1) {
  console.error(
    `[test-parallel] MAUMAU_TEST_SHARD_INDEX=${String(
      shardIndexOverride,
    )} requires MAUMAU_TEST_SHARDS>1.`,
  );
  process.exit(2);
}
...
if (shardIndexOverride !== null && shardIndexOverride > shardCount) {
  console.error(
    `[test-parallel] MAUMAU_TEST_SHARD_INDEX=${String(
      shardIndexOverride,
    )} exceeds MAUMAU_TEST_SHARDS=${String(shardCount)}.`,
  );
  process.exit(2);
}
const windowsCiArgs = isWindowsCi ? ["--dangerouslyIgnoreUnhandledErrors"] : [];
const silentArgs = process.env.MAUMAU_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
...
const defaultUnitPool = parsePoolOverride(process.env.MAUMAU_TEST_UNIT_DEFAULT_POOL, "threads");
...
const heavyUnitFileLimit = parseEnvNumber(
  "MAUMAU_TEST_HEAVY_UNIT_FILE_LIMIT",
  defaultHeavyUnitFileLimit,
);
const heavyUnitLaneCount = parseEnvNumber(
  "MAUMAU_TEST_HEAVY_UNIT_LANES",
  defaultHeavyUnitLaneCount,
);
const heavyUnitMinDurationMs = parseEnvNumber("MAUMAU_TEST_HEAVY_UNIT_MIN_MS", 1200);
...
const extensionsBatchTargetMs = parseEnvNumber(
  "MAUMAU_TEST_EXTENSIONS_BATCH_TARGET_MS",
...
const extensionsSharedEntries = extensionsSharedBatches
  ...
  .map((batch, batchIndex) => ({
    ...
    env: {
      MAUMAU_VITEST_INCLUDE_FILE: writeTempJsonArtifact(
...
const unitFastLaneCount = Math.max(
  1,
  parseEnvNumber("MAUMAU_TEST_UNIT_FAST_LANES", defaultUnitFastLaneCount),
);
...
const unitFastBatchTargetMs = parseEnvNumber(
  "MAUMAU_TEST_UNIT_FAST_BATCH_TARGET_MS",
...
const channelsBatchTargetMs = parseEnvNumber(
  "MAUMAU_TEST_CHANNELS_BATCH_TARGET_MS",
...
const topLevelParallelLimit = Math.max(
  1,
  parseEnvNumber("MAUMAU_TEST_TOP_LEVEL_CONCURRENCY", defaultTopLevelParallelLimit),
);
const overrideWorkers = Number.parseInt(process.env.MAUMAU_TEST_WORKERS ?? "", 10);
const resolvedOverride =
  Number.isFinite(overrideWorkers) && overrideWorkers > 0 ? overrideWorkers : null;
const parallelGatewayEnabled =
  !isMacMiniProfile &&
  (process.env.MAUMAU_TEST_PARALLEL_GATEWAY === "1" || (!isCI && highMemLocalHost));
// Keep gateway serial by default except when explicitly requested or on high-memory local hosts.
const keepGatewaySerial =
  isWindowsCi ||
  process.env.MAUMAU_TEST_SERIAL_GATEWAY === "1" ||
  testProfile === "serial" ||
...
const loadAwareDisabledRaw = process.env.MAUMAU_TEST_LOAD_AWARE?.trim().toLowerCase();
...
const maxOldSpaceSizeMb = (() => {
  ...
  const raw = process.env.MAUMAU_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
...
const rawMemoryTrace = process.env.MAUMAU_TEST_MEMORY_TRACE?.trim().toLowerCase();
const memoryTraceEnabled =
  process.platform !== "win32" &&
  (rawMemoryTrace === "1" ||
    rawMemoryTrace === "true" ||
    (rawMemoryTrace !== "0" && rawMemoryTrace !== "false" && isCI));
const memoryTracePollMs = Math.max(250, parseEnvNumber("MAUMAU_TEST_MEMORY_TRACE_POLL_MS", 1000));
const memoryTraceTopCount = Math.max(1, parseEnvNumber("MAUMAU_TEST_MEMORY_TRACE_TOP_COUNT", 6));
const requestedHeapSnapshotIntervalMs = Math.max(
  0,
  parseEnvNumber("MAUMAU_TEST_HEAPSNAPSHOT_INTERVAL_MS", 0),
);
...
const heapSnapshotSignal = process.env.MAUMAU_TEST_HEAPSNAPSHOT_SIGNAL?.trim() || "SIGUSR2";
const heapSnapshotBaseDir = heapSnapshotEnabled
  ? path.resolve(
      process.env.MAUMAU_TEST_HEAPSNAPSHOT_DIR?.trim() ||
        path.join(os.tmpdir(), `maumau-heapsnapshots-${Date.now()}`),
    )
  : null;
...
if (process.env.MAUMAU_TEST_LIST_LANES === "1") {
