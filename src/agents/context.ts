...
let configuredConfig: MaumauConfig | undefined;
...
function isLikelyMaumauCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = path
    .basename(argv[1] ?? "")
    .trim()
    .toLowerCase();
  return (
    entryBasename === "maumau" ||
    entryBasename === "maumau.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Maumau CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureMaumauModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyMaumauCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): MaumauConfig | undefined {
  if (configuredConfig) {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureMaumauModelsJson(cfg);
    } catch {
