    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "maumau", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-maumau-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "maumau", messages: [{ role: "user", content: "hi" }] },
          {
            "x-maumau-agent-id": "beta",
            "x-maumau-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        expect(json.error?.message).toBe("Invalid `model`. Use `maumau` or `maumau/<agentId>`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }

      {
        ...
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Invalid `x-maumau-model`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from Maumau.");
      }
