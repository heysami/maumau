...
import type { MaumauConfig, ConfigFileSnapshot } from "../config/types.maumau.js";
...
vi.mock("../infra/maumau-root.js", () => ({
  resolveMaumauPackageRoot: vi.fn(),
}));
...
const { resolveMaumauPackageRoot } = await import("../infra/maumau-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/maumau-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as MaumauConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/maumau-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveMaumauPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("maumau-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("maumau-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveMaumauPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("maumau");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/maumau-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "maumau-gateway" }],
      hints: [],
    });
    ...
    const cases = [
      {
        ...
        assert: () => {
          ...
          expect(logs.join("\n")).toContain("Maumau update status");
        },
      },
      {
        name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "maumau", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as MaumauConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "maumau@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
    ...
    for (const scenario of [
      {
        ...
        run: async () => {
          mockPackageInstallStatus(createCaseDir("maumau-update"));
          await updateCommand({ tag: "next" });
        },
        expectedSpec: "maumau@next",
      },
      {
        name: "main shorthand",
        run: async () => {
        ...
        expectedSpec: "github:maumau/maumau#main",
      },
      {
        name: "explicit git package spec",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("maumau-update"));
          await updateCommand({ yes: true, tag: "github:maumau/maumau#main" });
        },
        expectedSpec: "github:maumau/maumau#main",
      },
      {
        name: "MAUMAU_UPDATE_PACKAGE_SPEC override",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("maumau-update"));
          await withEnvAsync(
        ...
        expectedSpec: "http://10.211.55.2:8138/maumau-next.tgz",
      },
    ]) {
      vi.clearAllMocks();
      readPackageName.mockResolvedValue("maumau");
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "maumau", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("maumau-update");
    const localAppData = createCaseDir("maumau-localappdata");
  ...
  it.each([
    ...
    },
    {
      ...
      invoke: async () => {
        await withEnvAsync(
          {
            MAUMAU_STATE_DIR: "./state",
            MAUMAU_CONFIG_PATH: "./config/maumau.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            MAUMAU_STATE_DIR: path.resolve("./state"),
            MAUMAU_CONFIG_PATH: path.resolve("./config/maumau.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            MAUMAU_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ MAUMAU_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.MAUMAU_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("maumau-update-wizard");
    await withEnvAsync({ MAUMAU_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/maumau as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ MAUMAU_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "maumau"));
    });
