...
function stripWorkspaceMaumauDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.maumau !== "workspace:*") {
    return false;
  }
  delete devDeps.maumau;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceMaumauDevDependency(pkg);
