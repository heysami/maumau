...
final class RemindersService: RemindersServicing {
    func list(params: MaumauRemindersListParams) async throws -> MaumauRemindersListPayload {
        ...
        let payload: [MaumauReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return MaumauRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: MaumauRemindersAddParams) async throws -> MaumauRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = MaumauReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return MaumauRemindersAddPayload(reminder: payload)
    }
