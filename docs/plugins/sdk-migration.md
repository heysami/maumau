...
read_when:
  - You see the MAUMAU_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the MAUMAU_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Maumau plugin
---

# Plugin SDK Migration

Maumau has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`maumau/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`maumau/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`maumau/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "maumau/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    import { runEmbeddedPiAgent } from "maumau/extension-api";
...
```bash
MAUMAU_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 maumau gateway run
MAUMAU_SUPPRESS_EXTENSION_API_WARNING=1 maumau gateway run
```
