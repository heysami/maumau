...
export function connectGateway(host: GatewayHost) {
  ...
  client = new GatewayBrowserClient({
    ...
    onHello: (hello) => {
      ...
      void subscribeSessions(host as unknown as MaumauApp);
      void loadAssistantIdentity(host as unknown as MaumauApp);
      void loadAgents(host as unknown as MaumauApp);
      void loadHealthState(host as unknown as MaumauApp);
      void loadNodes(host as unknown as MaumauApp, { quiet: true });
      void loadDevices(host as unknown as MaumauApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (hadToolEvents && state === "final") {
    void loadChatHistory(host as unknown as MaumauApp);
    return true;
  }
  return false;
}
