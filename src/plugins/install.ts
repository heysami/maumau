...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing maumau.extensions; update the plugin package to include maumau.extensions (for example ["./dist/index.js"]). See https://docs.maumau.ai/help/troubleshooting#plugin-install-fails-with-missing-maumau-extensions';
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_MAUMAU_EXTENSIONS: "missing_maumau_extensions",
  EMPTY_MAUMAU_EXTENSIONS: "empty_maumau_extensions",
...
function ensureMaumauExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_MAUMAU_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json maumau.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_MAUMAU_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    logger.warn?.(
      `Bundle "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "maumau security audit --deep" after install.`,
    );
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureMaumauExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureMaumauExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from maumau.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-maumau"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json maumau.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires Maumau >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set MAUMAU_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires Maumau >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Maumau and retry.`,
  ...
  try {
    ...
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "maumau security audit --deep" after install.`,
    );
  }
