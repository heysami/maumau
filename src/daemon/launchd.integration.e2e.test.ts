...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `maumau-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      MAUMAU_LAUNCHD_LABEL: `ai.maumau.launchd-int-${testId}`,
      MAUMAU_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
