...
export const loadConfig = vi.fn<() => MaumauConfig>(() => ({}) as MaumauConfig);
export const readConfigFileSnapshot = vi.fn();
export const writeConfigFile = vi.fn<(config: MaumauConfig) => Promise<void>>(
  async () => undefined,
);
export const resolveStateDir = vi.fn(() => "/tmp/maumau-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: (config: MaumauConfig) => writeConfigFile(config),
}));
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as MaumauConfig);
  readConfigFileSnapshot.mockResolvedValue({
    path: "/tmp/maumau-config.json5",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: {},
  ...
  resolveStateDir.mockReturnValue("/tmp/maumau-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation((cfg: MaumauConfig) => ({ config: cfg }));
  recordPluginInstall.mockImplementation((cfg: MaumauConfig) => cfg);
  buildPluginStatusReport.mockReturnValue({
    plugins: [],
    diagnostics: [],
  });
  applyExclusiveSlotSelection.mockImplementation(({ config }: { config: MaumauConfig }) => ({
    config,
    warnings: [],
  }));
  ...
  recordHookInstall.mockImplementation((cfg: MaumauConfig) => cfg);
}
