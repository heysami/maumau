  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          maumau: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  ...
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.MAUMAU_GATEWAY_PORT;
    process.env.MAUMAU_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.MAUMAU_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.MAUMAU_GATEWAY_PASSWORD;
