...
private let webChatSwiftLogger = Logger(subsystem: "ai.maumau", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "maumau.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: MaumauChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> MaumauChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(MaumauChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            MaumauChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return MaumauChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> MaumauChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                MaumauGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: MaumauProtocol.ModelChoice) -> MaumauChatModelChoice {
        MaumauChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<MaumauChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any MaumauChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = MaumauChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<MaumauChatView>) -> NSViewController
    {
