...
const TEMP_MEDIA_PREFIX = "maumau-media-";
...
function createGroqAudioConfig(): MaumauConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): MaumauConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): MaumauConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: MaumauConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredMaumauTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        MAUMAU_AGENT_DIR: isolatedAgentDir,
