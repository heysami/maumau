...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    maumau: process.env.MAUMAU_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.maumau === undefined) {
      delete process.env.MAUMAU_GATEWAY_TOKEN;
    } else {
      process.env.MAUMAU_GATEWAY_TOKEN = envSnapshot.maumau;
    }
  });
  ...
  it("uses MAUMAU_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.MAUMAU_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
