...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.MAUMAU_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.MAUMAU_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from maumau.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "Maumau update wizard" : "Maumau configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: MaumauConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("maumau doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.MAUMAU_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.MAUMAU_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });
