...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    nodeFs.mkdirSync(path.join(base, ".maumau", "agents", "main", "sessions"), {
      recursive: true,
    });
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".maumau", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      MAUMAU_STATE_DIR: path.join(home, ".maumau"),
      ...env,
    };
