...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-bundled-skills-test-"));
    process.env.MAUMAU_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
