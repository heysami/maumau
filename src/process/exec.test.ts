...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        MAUMAU_BASE_ENV: "base",
        MAUMAU_TO_REMOVE: undefined,
      },
      env: {
        MAUMAU_TEST_ENV: "ok",
      },
    ...
    expect(resolved.MAUMAU_BASE_ENV).toBe("base");
    expect(resolved.MAUMAU_TEST_ENV).toBe("ok");
    expect(resolved.MAUMAU_TO_REMOVE).toBeUndefined();
    expect(resolved.MAUMAU_CLI).toBe(MAUMAU_CLI_ENV_VALUE);
  });
