...
struct MaumauMacCLI {
    static func main() async {
        ...
        default:
            fputs("maumau-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      maumau-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      maumau-mac discover [--timeout <ms>] [--json] [--include-local]
      maumau-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      maumau-mac connect
      maumau-mac connect --url ws://127.0.0.1:18789 --json
      maumau-mac discover --timeout 3000 --json
      maumau-mac wizard --mode local
    """)
}
