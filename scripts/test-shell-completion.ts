 ...
 * `maumau update`. Use it to verify the completion installation flow
...
\`maumau update\`, \`maumau doctor\`, and \`maumau onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Maumau Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`maumau completion --install\` later to enable.`));
    return;
  }
