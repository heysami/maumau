...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("maumau-bundle-commands-home-");
      const workspaceDir = await tempHarness.createTempDir("maumau-bundle-commands-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.MAUMAU_HOME;
      delete process.env.MAUMAU_STATE_DIR;

      const pluginRoot = path.join(homeDir, ".maumau", "extensions", "compound-bundle");
