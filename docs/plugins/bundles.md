---
summary: "Install and use Codex, Claude, and Cursor bundles as Maumau plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Maumau maps bundle content into native features
...
Maumau can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Maumau maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Maumau plugins. Native plugins run
...
of requiring authors to rewrite them as native Maumau plugins, Maumau
...
## What Maumau maps from bundles

Not every bundle feature runs in Maumau today. Here is what works and what
...
| ------------- | ---------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Maumau skills                                                      | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                           | Claude, Cursor |
| Hook packs    | Maumau-style `HOOK.md` + `handler.ts` layouts                                                        | Codex          |
...
These are recognized and shown in diagnostics, but Maumau does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Maumau best when they use skill roots and Maumau-style
...
Maumau checks for native plugin format first:

1. `maumau.plugin.json` or valid `package.json` with `maumau.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Maumau uses the native path. This prevents
...
- Maumau does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `maumau plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Maumau does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Maumau hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
