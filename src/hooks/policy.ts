import type { MaumauConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "maumau-bundled": {
    ...
    canOverride: ["maumau-bundled"],
    canBeOverriddenBy: ["maumau-managed", "maumau-plugin"],
  },
  "maumau-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["maumau-bundled", "maumau-plugin"],
    canBeOverriddenBy: ["maumau-managed"],
  },
  "maumau-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["maumau-bundled", "maumau-managed", "maumau-plugin"],
    canBeOverriddenBy: ["maumau-managed"],
  },
  "maumau-workspace": {
    precedence: 40,
    ...
    canOverride: ["maumau-workspace"],
    canBeOverriddenBy: ["maumau-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "maumau-plugin") {
    return { enabled: true };
  }
