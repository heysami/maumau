...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseMaumauManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyMaumauManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveMaumauMetadata(
  frontmatter: ParsedSkillFrontmatter,
): MaumauSkillMetadata | undefined {
  const metadataObj = resolveMaumauManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveMaumauManifestRequires(metadataObj);
  const install = resolveMaumauManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveMaumauManifestOs(metadataObj);
  return {
