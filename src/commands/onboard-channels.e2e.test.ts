...
function createTelegramCfg(botToken: string, enabled?: boolean): MaumauConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function patchTelegramAdapter(overrides: Parameters<typeof patchChannelSetupWizardAdapter>[1]) {
  return patchChannelSetupWizardAdapter("telegram", {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: MaumauConfig }) => ({
        channel: "telegram",
...
async function expectQuickstartPickerSkipsWithoutRuntime() {
  ...
  await expect(
    runSetupChannels({} as MaumauConfig, prompter, {
      quickstartDefaults: true,
    }),
  ).resolves.toEqual({} as MaumauConfig);
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as MaumauConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("./channel-setup/plugin-install.js", async (importOriginal) => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: MaumauConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    const text = vi.fn(async ({ message }: { message: string }) => {
      ...
      if (message.includes("Your WhatsApp number")) {
        return "+15555550123";
      }
      throw new Error(`unexpected text prompt: ${message}`);
    });
    ...
    await runSetupChannels({} as MaumauConfig, prompter, {
      quickstartDefaults: true,
  ...
  it("keeps configured external plugin channels visible when the active registry starts empty", async () => {
    ...
    catalogMocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        id: "msteams",
        pluginId: "@maumau/msteams-plugin",
        meta: {
          id: "msteams",
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@maumau/msteams-plugin": { enabled: true },
          },
        },
      } as MaumauConfig,
      prompter,
        ...
        if (channel === "msteams") {
          registry.channels.push({
            ...
            plugin: {
              ...
              capabilities: { chatTypes: ["direct"] },
              config: {
                listAccountIds: (cfg: MaumauConfig) =>
                  Object.keys(
                    (cfg.channels?.msteams as { accounts?: Record<string, unknown> } | undefined)
                      ?.accounts ?? {},
                  ),
                resolveAccount: (cfg: MaumauConfig, accountId: string) =>
                  (
                    cfg.channels?.msteams as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
              ...
              setupWizard: {
                ...
                status: {
                  ...
                  resolveConfigured: ({ cfg }: { cfg: MaumauConfig }) =>
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: MaumauConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: MaumauConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
