...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleMaumauDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Maumau devices detected for ${params.auth.userId}: ${deviceHealth.staleMaumauDevices.map((device) => device.deviceId).join(", ")}. Run 'maumau matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
