...
- With Anthropic API-key auth profiles, Maumau seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Maumau injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Maumau exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.maumau/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$MAUMAU_STATE_DIR/logs/cache-trace.jsonl`
...
- `MAUMAU_CACHE_TRACE=1` enables cache tracing.
- `MAUMAU_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `MAUMAU_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `MAUMAU_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `MAUMAU_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
