...
final class ContactsService: ContactsServicing {
    ...
    func search(params: MaumauContactsSearchParams) async throws -> MaumauContactsSearchPayload {
        ...
        return MaumauContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: MaumauContactsAddParams) async throws -> MaumauContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return MaumauContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return MaumauContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> MaumauContactPayload {
        MaumauContactPayload(
