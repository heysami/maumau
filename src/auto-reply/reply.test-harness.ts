...
type HomeEnvSnapshot = {
  ...
  MAUMAU_STATE_DIR: string | undefined;
  MAUMAU_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    MAUMAU_AGENT_DIR: process.env.MAUMAU_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".maumau", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.MAUMAU_STATE_DIR = path.join(home, ".maumau");
    process.env.MAUMAU_AGENT_DIR = path.join(home, ".maumau", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".maumau", "agent");
