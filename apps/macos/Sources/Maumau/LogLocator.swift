...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MAUMAU_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/maumau")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("maumau-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("maumau-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/maumau/ (rolling or stdout), or nil if none exist.
