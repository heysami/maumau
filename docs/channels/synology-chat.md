---
summary: "Synology Chat webhook setup and Maumau config"
read_when:
  - Setting up Synology Chat with Maumau
...
```bash
maumau plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `maumau onboard` now shows Synology Chat in the same channel setup list as `maumau channels add`.
   - Non-interactive setup: `maumau channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Maumau gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Maumau.
   - Guided: `maumau onboard`
   - Direct: `maumau channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `maumau pairing list synology-chat`
  - `maumau pairing approve synology-chat <CODE>`
...
```bash
maumau message send --channel synology-chat --target 123456 --text "Hello from Maumau"
maumau message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Maumau now rejects duplicate exact paths
