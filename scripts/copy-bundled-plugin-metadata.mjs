...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "maumau.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "maumau.plugin.json");
    ...
    if (packageJson.maumau && "extensions" in packageJson.maumau) {
      packageJson.maumau = {
        ...packageJson.maumau,
        extensions: rewritePackageExtensions(packageJson.maumau.extensions),
        ...(typeof packageJson.maumau.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.maumau.setupEntry) }
          : {}),
      };
    }
