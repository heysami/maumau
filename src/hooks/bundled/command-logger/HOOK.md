...
homepage: https://docs.maumau.ai/automation/hooks#command-logger
metadata:
  {
    "maumau":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.maumau/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.maumau/logs/commands.log ~/.maumau/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/maumau`:
   ```
   /home/username/.maumau/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
