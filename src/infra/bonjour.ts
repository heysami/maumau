...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.MAUMAU_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Maumau";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Maumau\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw = process.env.MAUMAU_MDNS_HOSTNAME?.trim() || "maumau";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
