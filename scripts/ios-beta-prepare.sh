...
- regenerates apps/ios/Maumau.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${MAUMAU_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid MAUMAU_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid MAUMAU_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid MAUMAU_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid MAUMAU_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing MAUMAU_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(MAUMAU_URL_SLASH)$(MAUMAU_URL_SLASH)#g'
)"
...
MAUMAU_CODE_SIGN_STYLE = Automatic
MAUMAU_DEVELOPMENT_TEAM = ${TEAM_ID}
MAUMAU_IOS_SELECTED_TEAM = ${TEAM_ID}
MAUMAU_APP_BUNDLE_ID = ai.maumau.client
MAUMAU_SHARE_BUNDLE_ID = ai.maumau.client.share
MAUMAU_ACTIVITY_WIDGET_BUNDLE_ID = ai.maumau.client.activitywidget
MAUMAU_WATCH_APP_BUNDLE_ID = ai.maumau.client.watchkitapp
MAUMAU_WATCH_EXTENSION_BUNDLE_ID = ai.maumau.client.watchkitapp.extension
MAUMAU_APP_PROFILE =
MAUMAU_SHARE_PROFILE =
MAUMAU_PUSH_TRANSPORT = relay
MAUMAU_PUSH_DISTRIBUTION = official
MAUMAU_URL_SLASH = /
MAUMAU_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
MAUMAU_PUSH_APNS_ENVIRONMENT = production
EOF
