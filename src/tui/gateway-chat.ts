...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set MAUMAU_GATEWAY_TOKEN/MAUMAU_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.MAUMAU_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.MAUMAU_GATEWAY_PASSWORD);
