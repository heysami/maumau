...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveMaumauAgentDir: vi.fn().mockReturnValue("/tmp/maumau-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/maumau-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/maumau-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/maumau-dev/maumau.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveMaumauAgentDir: mocks.resolveMaumauAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/maumau-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveMaumauAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/maumau-dev/maumau.json");
    expect(payload.auth.storePath).toBe("/tmp/maumau-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/maumau-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/maumau-agent-custom");
