...
Hooks 提供了一个可扩展的事件驱动系统，用于在响应智能体命令和事件时自动执行操作。Hooks 会从目录中自动发现，并且可以通过 CLI 命令进行管理，方式与 Maumau 中的 Skills 类似。
...
- **Webhooks**：外部 HTTP webhook，可让其他系统在 Maumau 中触发工作。请参阅 [Webhook Hooks](/automation/webhook)，或使用 `maumau webhooks` 获取 Gmail 辅助命令。
...
- 记录所有命令以供审计
- 在智能体生命周期事件上触发自定义自动化
- 在不修改核心代码的情况下扩展 Maumau 的行为

## 入门指南

### 内置 Hooks

Maumau 自带四个会被自动发现的内置 hook：

- **💾 session-memory**：当你发出 `/new` 时，将会话上下文保存到你的智能体工作区（默认是 `~/.maumau/workspace/memory/`）
- **📎 bootstrap-extra-files**：在 `agent:bootstrap` 期间，从已配置的 glob/路径模式中注入额外的工作区引导文件
- **📝 command-logger**：将所有命令事件记录到 `~/.maumau/logs/commands.log`
...
在新手引导期间（`maumau onboard`），系统会提示你启用推荐的 hooks。向导会自动发现符合条件的 hooks 并供你选择。
...
2. **托管 hooks**：`~/.maumau/hooks/`（用户安装，在各工作区之间共享）
3. **内置 hooks**：`<maumau>/dist/hooks/bundled/`（随 Maumau 一起提供）
...
Hook 包是标准的 npm 包，它们通过 `package.json` 中的 `maumau.hooks` 导出一个或多个 hook。使用以下命令安装它们：

```bash
maumau hooks install <path-or-spec>
```

npm spec 仅支持注册表形式（包名 + 可选的精确版本或 dist-tag）。
Git/URL/file spec 和 semver 范围会被拒绝。

裸 spec 和 `@latest` 会保持在稳定轨道上。如果 npm 将其中任意一种解析为预发布版本，Maumau 会停止并要求你通过预发布标签（例如 `@beta`/`@rc`）或精确的预发布版本显式选择加入。
...
Hook 包可以携带依赖；它们会安装到 `~/.maumau/hooks/<id>` 下。
每个 `maumau.hooks` 条目在解析符号链接后都必须保持在包目录内部；超出目录范围的条目会被拒绝。

安全说明：`maumau hooks install` 会使用 `npm install --ignore-scripts` 安装依赖
...
这些 hooks 不是事件流监听器；它们允许插件在 Maumau 持久化工具结果之前同步调整工具结果。
...
**输出**：`<workspace>/memory/YYYY-MM-DD-slug.md`（默认是 `~/.maumau/workspace`）
...
```yaml
metadata: { "maumau": { "events": ["command:new"] } } # 精确
```

而不是：

```yaml
metadata: { "maumau": { "events": ["command"] } } # 通用 - 开销更大
```
