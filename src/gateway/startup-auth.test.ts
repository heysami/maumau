...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: MaumauConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: MaumauConfig) {
  ...
  async function expectNoTokenGeneration(cfg: MaumauConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  function createMissingGatewayTokenSecretRefConfig(): MaumauConfig {
    return {
      gateway: {
        auth: {
          mode: "token",
    ...
    await expectResolvedToken({
      cfg: {
        gateway: {
          auth: {
            mode: "token",
            token: "${MAUMAU_GATEWAY_TOKEN}",
          },
        },
      },
      env: {
        MAUMAU_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${MAUMAU_GATEWAY_TOKEN}",
    });
  });

  it("uses MAUMAU_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    await expectResolvedToken({
      cfg: createMissingGatewayTokenSecretRefConfig(),
      env: {
        MAUMAU_GATEWAY_TOKEN: "token-from-env",
      } as NodeJS.ProcessEnv,
  ...
  it("uses MAUMAU_GATEWAY_PASSWORD without resolving configured password SecretRef", async () => {
    const result = await ensureGatewayStartupAuth({
      cfg: {
        gateway: {
          auth: {
      ...
      env: {
        MAUMAU_GATEWAY_PASSWORD: "password-from-env", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      persist: true,
    });
