...
metadata: { "maumau": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Maumau `sessions_spawn`
- **File I/O** == Maumau `read`/`write`
- **Remote fetch** == Maumau `web_fetch` (or `exec` with curl when POST is required)
