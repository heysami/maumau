...
function shouldStageRuntimeDeps(packageJson) {
  return packageJson.maumau?.bundle?.stageRuntimeDependencies === true;
}
...
function sanitizeBundledManifestForRuntimeInstall(pluginDir) {
  ...
  if (packageJson.peerDependencies?.maumau) {
    const nextPeerDependencies = { ...packageJson.peerDependencies };
    delete nextPeerDependencies.maumau;
  ...
  if (packageJson.peerDependenciesMeta?.maumau) {
    const nextPeerDependenciesMeta = { ...packageJson.peerDependenciesMeta };
    delete nextPeerDependenciesMeta.maumau;
  ...
  if (packageJson.devDependencies?.maumau) {
    const nextDevDependencies = { ...packageJson.devDependencies };
    delete nextDevDependencies.maumau;
  ...
  if (platform === "win32") {
    ...
    throw new Error(
      `failed to resolve a toolchain-local npm next to ${execPath}. ` +
        ...
        "Maumau refuses to shell out to bare npm on Windows; install a Node.js toolchain that bundles npm or run with a matching Node installation.",
    );
  }
