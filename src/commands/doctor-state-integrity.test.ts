...
type EnvSnapshot = {
  HOME?: string;
  MAUMAU_HOME?: string;
  MAUMAU_STATE_DIR?: string;
  MAUMAU_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    MAUMAU_HOME: process.env.MAUMAU_HOME,
    MAUMAU_STATE_DIR: process.env.MAUMAU_STATE_DIR,
    MAUMAU_OAUTH_DIR: process.env.MAUMAU_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: MaumauConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: MaumauConfig) {
...
async function runStateIntegrityText(cfg: MaumauConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.MAUMAU_HOME = tempHome;
    process.env.MAUMAU_STATE_DIR = path.join(tempHome, ".maumau");
    delete process.env.MAUMAU_OAUTH_DIR;
    fs.mkdirSync(process.env.MAUMAU_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when MAUMAU_OAUTH_DIR is explicitly configured", async () => {
    process.env.MAUMAU_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: MaumauConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints maumau-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: MaumauConfig = {};
    ...
    expect(text).toMatch(/maumau sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/maumau sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /maumau sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
