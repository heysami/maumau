...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["maumau security audit", "Run a local security audit."],
          ["maumau security audit --deep", "Include best-effort live Gateway probe checks."],
          ["maumau security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "maumau security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["maumau security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["maumau security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.maumau.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Maumau security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("maumau security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("maumau security audit --fix")}`));
        if (!fixResult) {
