...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "maumau";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "maumau" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (maumau|existing-session). Default: maumau")
    .action(
      async (
        opts: {
          name: string;
