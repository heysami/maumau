...
type GatewayInstallAuthMode = NonNullable<NonNullable<MaumauConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: MaumauConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    durableServiceEnv.MAUMAU_GATEWAY_PASSWORD?.trim() ||
