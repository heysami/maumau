---
summary: "ClawHub guide: public registry, native Maumau install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Maumau skills and plugins**.

- Use native `maumau` commands to search/install/update skills and install
...
```bash
maumau plugins install clawhub:<package>
...
Native `maumau` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for Maumau skills.
...
4. Users browse, download, and install skills in Maumau.
...
If you want to add new capabilities to your Maumau agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `maumau skills install <skill-slug>`
3. Start a new Maumau session so it picks up the new skill.
...
## How it fits into Maumau

Native `maumau skills install` installs into the active workspace `skills/`
directory. `maumau plugins install clawhub:...` records a normal managed
...
current working directory. If an Maumau workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Maumau loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.maumau/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Maumau how to perform a
...
Interested in becoming a moderator? Ask in the Maumau Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Maumau workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/maumau/skills` and `~/.maumau/skills`). This is designed to find older skill installs without extra flags.
