...
const EXPECTED_REPOSITORY_URL = "https://github.com/maumau/maumau";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "maumau") {
    errors.push(`package.json name must be "maumau"; found "${pkg.name ?? ""}".`);
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`maumau-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `maumau-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
