...
function findTool(name: string, config: MaumauConfig) {
  const allTools = createMaumauTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: MaumauConfig; env?: NodeJS.ProcessEnv }) {
...
describe("maumau tools runtime web metadata wiring", () => {
