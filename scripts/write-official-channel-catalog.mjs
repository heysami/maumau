...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.maumau) ? packageJson.maumau : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.maumau?.channel?.id) || left.name;
    const rightId = trimString(right.maumau?.channel?.id) || right.name;
