---
summary: "CLI reference for `maumau configure` (interactive configuration prompts)"
read_when:
...
Tip: `maumau config` without a subcommand opens the same wizard. Use
`maumau config get|set|unset` for non-interactive edits.
...
```bash
maumau configure
maumau configure --section model --section channels
```
