...
Maumau features that can generate provider usage or paid API calls.
...
- `maumau status --usage` and `maumau channels list` show provider **usage windows**
...
Maumau can pick up credentials from:
...
`maumau models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
