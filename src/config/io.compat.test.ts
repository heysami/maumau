...
import { parseMaumauVersion } from "./version.js";

async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  it("uses ~/.maumau/maumau.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".maumau", 19001);
  ...
  it("defaults to ~/.maumau/maumau.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".maumau", "maumau.json"));
    });
  });

  it("uses MAUMAU_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { MAUMAU_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".maumau", "maumau.json"));
    });
  });

  it("honors explicit MAUMAU_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".maumau", 20002, "custom.json");
      const io = createIoForHome(home, { MAUMAU_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("does not warn when config was last touched by a same-base correction publish", async () => {
    const parsedVersion = parseMaumauVersion(VERSION);
    if (!parsedVersion) {
    ...
    await withTempHome(async (home) => {
      ...
      expect(logger.warn).not.toHaveBeenCalledWith(
        expect.stringContaining("Config was last written by a newer Maumau"),
      );
      expect(io.configPath).toBe(configPath);
    });
  });
