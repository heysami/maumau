...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-launcher-"));
  fixtureRoots.push(fixtureRoot);
  await fs.copyFile(
    path.resolve(process.cwd(), "maumau.mjs"),
    path.join(fixtureRoot, "maumau.mjs"),
  );
...
describe("maumau launcher", () => {
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      ...
      'import "missing-maumau-launcher-dep";\nexport {};\n',
      "utf8",
    );

    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "maumau.mjs"), "--help"], {
      cwd: fixtureRoot,
      encoding: "utf8",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("missing-maumau-launcher-dep");
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:maumau/maumau#<ref>");
  });
});
