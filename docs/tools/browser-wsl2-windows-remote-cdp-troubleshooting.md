...
read_when:
  - Running Maumau Gateway in WSL2 while Chrome lives on Windows
...
- Maumau Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/maumau/maumau/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Maumau and Chrome are on the same machine
...
- Maumau points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Maumau is not the problem yet.
...
Fix that before touching Maumau config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Maumau at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Maumau to succeed
...
```bash
maumau browser open https://example.com --browser-profile remote
maumau browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `maumau browser tabs` returns the target
...
3. Maumau config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Maumau config or Control UI auth
