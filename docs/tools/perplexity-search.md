...
Maumau supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Maumau also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `maumau configure --section web`. It stores the key in
`~/.maumau/maumau.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.maumau/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
