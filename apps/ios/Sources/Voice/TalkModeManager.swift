...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__MAUMAU_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<MaumauTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.maumau", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = MaumauTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> MaumauTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return MaumauTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> MaumauTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> MaumauTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return MaumauTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> MaumauTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = MaumauTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: MaumauTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: MaumauAgentEventPayload.self
            ) else {
                continue
            }
