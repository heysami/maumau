...
let originalMaumauLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.MAUMAU_TEST_FILE_LOG;
  originalMaumauLogLevel = process.env.MAUMAU_LOG_LEVEL;
  delete process.env.MAUMAU_TEST_FILE_LOG;
  delete process.env.MAUMAU_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.MAUMAU_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalMaumauLogLevel === undefined) {
    delete process.env.MAUMAU_LOG_LEVEL;
  } else {
    process.env.MAUMAU_LOG_LEVEL = originalMaumauLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
