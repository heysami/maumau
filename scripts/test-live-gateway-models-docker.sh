...
elif [[ -n "${MAUMAU_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(maumau_live_collect_auth_dirs_from_csv "${MAUMAU_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
docker run --rm -t \
  ...
  -e MAUMAU_LIVE_GATEWAY_MODELS="${MAUMAU_LIVE_GATEWAY_MODELS:-modern}" \
  -e MAUMAU_LIVE_GATEWAY_PROVIDERS="${MAUMAU_LIVE_GATEWAY_PROVIDERS:-}" \
  -e MAUMAU_LIVE_GATEWAY_MAX_MODELS="${MAUMAU_LIVE_GATEWAY_MAX_MODELS:-24}" \
  -e MAUMAU_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${MAUMAU_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}" \
  -v "$ROOT_DIR":/src:ro \
