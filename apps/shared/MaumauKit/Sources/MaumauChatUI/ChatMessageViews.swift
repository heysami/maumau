...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? MaumauChatTheme.userText : MaumauChatTheme.assistantText
    ...
    private var inlineAttachments: [MaumauChatMessageContent] {
    ...
    private var toolCalls: [MaumauChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [MaumauChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? MaumauChatTheme.userBubble
        }
        if self.style == .onboarding {
            return MaumauChatTheme.onboardingAssistantBubble
        }
        return MaumauChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return MaumauChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: MaumauChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? MaumauChatTheme.userText : MaumauChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: MaumauChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(MaumauChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [MaumauChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: MaumauChatTheme.assistantText)
            }
        }
    }
}
