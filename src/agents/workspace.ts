...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.MAUMAU_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".maumau", `workspace-${profile}`);
  }
  return path.join(home, ".maumau", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".maumau";
