...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".maumau", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: MaumauConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [["MAUMAU_GATEWAY_TOKEN", await getenv("MAUMAU_GATEWAY_TOKEN")]] as const;
  const passwordEntries = [
    ["MAUMAU_GATEWAY_PASSWORD", await getenv("MAUMAU_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "MAUMAU_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.MAUMAU_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- MAUMAU_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
