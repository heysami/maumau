...
# Maumau Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://maumau.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Maumau."
...
GUM_VERSION="${MAUMAU_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Maumau Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://maumau.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$MAUMAU_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.maumau.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_maumau_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.maumau-* "$npm_root"/maumau 2>/dev/null || true
}

extract_maumau_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_maumau_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/maumau" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/maumau"|"/usr/local/bin/maumau")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/maumau/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale maumau symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing maumau binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Maumau package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_maumau_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Maumau npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*maumau" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_maumau_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_maumau_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Maumau npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an maumau binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${MAUMAU_TAGLINE_INDEX:-}" ]]; then
        if [[ "${MAUMAU_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((MAUMAU_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${MAUMAU_NO_ONBOARD:-0}
NO_PROMPT=${MAUMAU_NO_PROMPT:-0}
DRY_RUN=${MAUMAU_DRY_RUN:-0}
INSTALL_METHOD=${MAUMAU_INSTALL_METHOD:-}
MAUMAU_VERSION=${MAUMAU_VERSION:-latest}
USE_BETA=${MAUMAU_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/maumau"
GIT_DIR=${MAUMAU_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${MAUMAU_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${MAUMAU_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${MAUMAU_VERBOSE:-0}"
VERIFY_INSTALL="${MAUMAU_VERIFY_INSTALL:-0}"
MAUMAU_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Maumau installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://maumau.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/maumau)
...
Environment variables:
  MAUMAU_INSTALL_METHOD=git|npm
  MAUMAU_VERSION=latest|next|main|<semver>|<spec>
  MAUMAU_BETA=0|1
  MAUMAU_GIT_DIR=...
  MAUMAU_GIT_UPDATE=0|1
  MAUMAU_NO_PROMPT=1
  MAUMAU_VERIFY_INSTALL=1
  MAUMAU_DRY_RUN=1
  MAUMAU_NO_ONBOARD=1
  MAUMAU_VERBOSE=1
  MAUMAU_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://maumau.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://maumau.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://maumau.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://maumau.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://maumau.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Maumau checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Maumau source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_maumau_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"maumau"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://maumau.ai/install.sh | bash"
}
...
ensure_maumau_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/maumau" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/maumau" ]]; then
        ln -sf "$npm_root/maumau/dist/entry.js" "${npm_bin}/maumau"
        ui_info "Created maumau bin link at ${npm_bin}/maumau"
    fi
    return 0
}

# Check for existing Maumau installation
check_existing_maumau() {
    if [[ -n "$(type -P maumau 2>/dev/null || true)" ]]; then
        ui_info "Existing Maumau installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make maumau show as \"command not found\" in new terminals."
...
warn_maumau_not_found() {
    ui_warn "Installed, but maumau is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t maumau 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named maumau; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_maumau_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P maumau 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/maumau" ]]; then
        echo "${npm_bin}/maumau"
        return 0
    fi
...
install_maumau_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/maumau/maumau.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Maumau from git checkout: ${repo_dir}"
    else
        ui_info "Installing Maumau from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Maumau" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Maumau" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/maumau" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/maumau"
    ui_success "Maumau wrapper installed to \$HOME/.local/bin/maumau"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view maumau dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:maumau/maumau#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_maumau() {
    local package_name="maumau"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            MAUMAU_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="maumau"
        else
            MAUMAU_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${MAUMAU_VERSION}" ]]; then
        MAUMAU_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${MAUMAU_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${MAUMAU_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Maumau v${resolved_version}"
    else
        ui_info "Installing Maumau (${MAUMAU_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${MAUMAU_VERSION}")"

    if ! install_maumau_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_maumau_paths
        install_maumau_npm "${install_spec}"
    fi

    if [[ "${MAUMAU_VERSION}" == "latest" && "${package_name}" == "maumau" ]]; then
        if ! resolve_maumau_bin &> /dev/null; then
            ui_warn "npm install maumau@latest failed; retrying maumau@next"
            cleanup_npm_maumau_paths
            install_maumau_npm "maumau@next"
        fi
    fi

    ensure_maumau_bin_link || true

    ui_success "Maumau installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${MAUMAU_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_maumau_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (maumau not on PATH yet)"
        warn_maumau_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${MAUMAU_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.maumau/workspace-${profile}"
    else
        echo "${HOME}/.maumau/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${MAUMAU_CONFIG_PATH:-$HOME/.maumau/maumau.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run maumau onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but maumau not on PATH; skipping onboarding"
        warn_maumau_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run maumau onboard to retry"
        return
    }
}
...
resolve_maumau_version() {
    local version=""
    local raw_version_output=""
    local claw="${MAUMAU_BIN:-}"
    if [[ -z "$claw" ]] && command -v maumau &> /dev/null; then
        claw="$(command -v maumau)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_maumau_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/maumau/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/maumau/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: maumau not on PATH yet"
        warn_maumau_not_found
        return 1
    fi

    run_quiet_step "Checking Maumau version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: maumau gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_maumau_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Maumau checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set MAUMAU_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_maumau_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/maumau" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/maumau"
    ...
    MAUMAU_BIN="$(resolve_maumau_bin || true)"
    ...
    installed_version=$(resolve_maumau_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Maumau installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Maumau installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/maumau"
        ui_kv "Update command" "maumau update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://maumau.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running maumau doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                MAUMAU_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                MAUMAU_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                MAUMAU_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run maumau doctor and maumau plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run maumau onboard later"
        else
            local config_path="${MAUMAU_CONFIG_PATH:-$HOME/.maumau/maumau.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (maumau not on PATH yet)"
                    warn_maumau_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run maumau onboard to finish setup"
            return 0
        fi
    fi

    if command -v maumau &> /dev/null; then
        local claw="${MAUMAU_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_maumau_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (maumau daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if MAUMAU_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: maumau daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${MAUMAU_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
