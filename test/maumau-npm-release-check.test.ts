...
} from "../scripts/maumau-npm-release-check.ts";
...
describe("parseNpmPackJsonOutput", () => {
  it("parses a plain npm pack JSON array", () => {
    expect(parseNpmPackJsonOutput('[{"filename":"maumau.tgz","files":[]}]')).toEqual([
      { filename: "maumau.tgz", files: [] },
    ]);
  });
  ...
  it("parses the trailing JSON payload after npm lifecycle logs", () => {
    const stdout = [
      ...
      "",
      "> maumau@2026.3.23 prepack",
      "> pnpm build && pnpm ui:build",
      "",
      "[copy-hook-metadata] Copied 4 hook metadata files.",
      '[{"filename":"maumau.tgz","files":[{"path":"dist/control-ui/index.html"}]}]',
    ].join("\n");
  ...
  it("returns null when no JSON payload is present", () => {
    expect(parseNpmPackJsonOutput("> maumau@2026.3.23 prepack")).toBeNull();
  });
});
  ...
  it("validates the expected npm package metadata", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        license: "MIT",
        repository: { url: "git+https://github.com/maumau/maumau.git" },
        bin: { maumau: "maumau.mjs" },
