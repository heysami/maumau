import { DEFAULT_ACCOUNT_ID } from "maumau/plugin-sdk/account-id";
import type { DiscordGuildEntry } from "maumau/plugin-sdk/config-runtime";
import type { MaumauConfig } from "maumau/plugin-sdk/config-runtime";
import { createEnvPatchedAccountSetupAdapter } from "maumau/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "maumau/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "maumau/plugin-sdk/setup-runtime";
import { formatDocsLink } from "maumau/plugin-sdk/setup-tools";
import {
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: MaumauConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: MaumauConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: MaumauConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: MaumauConfig; accountId: string }) =>
    ...
    disable: (cfg: MaumauConfig) => setSetupChannelEnabled(cfg, channel, false),
