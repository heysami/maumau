...
async function runExtractedArchiveFailureCase(configureArchive: () => void) {
  vi.spyOn(installSource, "withTempDir").mockImplementation(
    async (_prefix, fn) => await fn("/tmp/maumau-install-flow"),
  );
  configureArchive();
...
describe("resolveExistingInstallPath", () => {
  ...
  beforeEach(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-install-flow-"));
  });
...
describe("withExtractedArchiveRoot", () => {
  ...
  it("extracts archive and passes root directory to callback", async () => {
    const tmpRoot = path.join(path.sep, "tmp", "maumau-install-flow");
    ...
    expect(withTempDirSpy).toHaveBeenCalledWith("maumau-plugin-", expect.any(Function));
