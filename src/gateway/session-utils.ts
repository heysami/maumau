...
function listConfiguredAgentIds(cfg: MaumauConfig): string[] {
...
export function listAgentsForGateway(cfg: MaumauConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: MaumauConfig): string {
  return normalizeAgentId(resolveDefaultAgentId(cfg));
}

export function resolveSessionStoreKey(params: { cfg: MaumauConfig; sessionKey: string }): string {
...
function resolveSessionStoreAgentId(cfg: MaumauConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: MaumauConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: MaumauConfig): GatewaySessionsDefaults {
