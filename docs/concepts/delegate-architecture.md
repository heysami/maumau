---
summary: "Delegate architecture: running Maumau as a named agent on behalf of an organization"
...
Goal: run Maumau as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Maumau agent that:
...
Maumau's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Maumau's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.maumau/workspace-delegate",
  tools: {
...
- Cron run history: `~/.maumau/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.maumau/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Maumau's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
maumau agents add delegate
```

This creates:

- Workspace: `~/.maumau/workspace-delegate`
- State: `~/.maumau/agents/delegate/agent`
- Sessions: `~/.maumau/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.maumau/workspace" },
      {
        id: "delegate",
        workspace: "~/.maumau/workspace-delegate",
        tools: {
...
~/.maumau/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.maumau/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.maumau/workspace-org",
        agentDir: "~/.maumau/agents/org-assistant/agent",
