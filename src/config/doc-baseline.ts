...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.MAUMAU_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveMaumauPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    MAUMAU_STATE_DIR: path.join(os.tmpdir(), "maumau-config-doc-baseline-state"),
    MAUMAU_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
  ...
  const channelPlugins =
    process.env.MAUMAU_CONFIG_DOC_BASELINE_DEBUG === "1"
