...
final class LiveActivityManager {
    ...
    private let logger = Logger(subsystem: "ai.maumau.ios", category: "LiveActivity")
    private var currentActivity: Activity<MaumauActivityAttributes>?
    ...
    func startActivity(agentName: String, sessionKey: String) {
        ...
        let attributes = MaumauActivityAttributes(agentName: agentName, sessionKey: sessionKey)
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<MaumauActivityAttributes>.activities
    ...
    private func updateCurrent(state: MaumauActivityAttributes.ContentState) {
    ...
    private func connectingState() -> MaumauActivityAttributes.ContentState {
        MaumauActivityAttributes.ContentState(
    ...
    private func idleState() -> MaumauActivityAttributes.ContentState {
        MaumauActivityAttributes.ContentState(
            statusText: "Idle",
            isIdle: true,
            isDisconnected: false,
    ...
    private func disconnectedState() -> MaumauActivityAttributes.ContentState {
        MaumauActivityAttributes.ContentState(
