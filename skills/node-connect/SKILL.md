---
name: node-connect
description: Diagnose Maumau node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Maumau is advertising that route, then fix pairing/auth.
...
- whether `maumau devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `maumau qr --json`. It uses the same setup-code payload Android scans.

```bash
maumau config get gateway.mode
maumau config get gateway.bind
maumau config get gateway.tailscale.mode
maumau config get gateway.remote.url
maumau config get gateway.auth.mode
maumau config get gateway.auth.allowTailscale
maumau config get plugins.entries.device-pair.config.publicUrl
maumau qr --json
maumau devices list
maumau nodes status
```

If this Maumau instance is pointed at a remote gateway, also run:

```bash
maumau qr --remote --json
```
...
If `maumau qr --json` says `Gateway is only bound to loopback`:
...
- `maumau devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run maumau qr again, rescan, then approve the pending device pairing.`
