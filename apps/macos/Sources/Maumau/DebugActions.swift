...
enum DebugActions {
    private static let verboseDefaultsKey = "maumau.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "maumau.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = MaumauPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Maumau", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = MaumauPaths.configURL
        guard
