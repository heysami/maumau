...
```bash
maumau plugins install @maumau/msteams
```

Local checkout (when running from a git repo):

```bash
maumau plugins install ./extensions/msteams
```

If you choose Teams during setup and a git checkout is detected,
Maumau will offer the local install path automatically.
...
3. Configure Maumau with those credentials.
...
- Talk to Maumau via Teams DMs, group chats, or channels.
...
- On startup, Maumau resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.maumau/maumau.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Maumau, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                  |
   | ------------------ | ------------------------------------------------------ |
   | **Bot handle**     | Your bot name, e.g., `maumau-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `maumau plugins install @maumau/msteams`
   - From a local checkout: `maumau plugins install ./extensions/msteams`
...
```json5
{
  ...
  description: { short: "Maumau in Teams", full: "Maumau in Teams" },
...
Maumau handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Maumau only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/MaumauShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Maumau sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `maumau message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.maumau/msteams-polls.json`.
...
```bash
maumau message send --channel msteams \
...
maumau message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
maumau message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
maumau message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
maumau message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
