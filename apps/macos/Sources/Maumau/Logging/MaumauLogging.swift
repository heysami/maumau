...
enum MaumauLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = MaumauOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = MaumauFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        MaumauLogging.bootstrapIfNeeded()
        let label = MaumauLogging.makeLabel(subsystem: subsystem, category: category)
...
struct MaumauOSLogHandler: AppLogLevelBackedHandler {
...
struct MaumauFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = MaumauLogging.parseLabel(self.label)
