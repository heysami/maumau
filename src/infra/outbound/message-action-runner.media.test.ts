...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Maumau tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideMaumau = path.join(os.tmpdir(), "outside-maumau", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
