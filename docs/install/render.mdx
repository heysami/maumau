---
summary: "Deploy Maumau on Render with Infrastructure-as-Code"
read_when:
  - Deploying Maumau to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Maumau on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/maumau/maumau)
...
repository configures everything needed to run Maumau:
...
services:
  - type: web
    ...
    envVars:
      - key: MAUMAU_GATEWAY_PORT
        value: "8080"
      - key: MAUMAU_STATE_DIR
        value: /data/.maumau
      - key: MAUMAU_WORKSPACE_DIR
        value: /data/workspace
      - key: MAUMAU_GATEWAY_TOKEN
...
Connect using the `MAUMAU_GATEWAY_TOKEN` value that was auto-generated during deploy (find it in **Dashboard → your service → Environment**).
...
If you use the original Maumau repository, Render will not auto-deploy your Maumau. To update it, run a manual Blueprint sync from the dashboard.
...
For Maumau, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive you can restore on any Maumau host. See [Backup](/cli/backup) for details.
...
- Missing `MAUMAU_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `MAUMAU_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export your config via `maumau backup create` in the Render shell.
