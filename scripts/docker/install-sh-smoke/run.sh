...
SMOKE_PREVIOUS_VERSION="${MAUMAU_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MAUMAU_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="maumau"
PACKAGE_NAME="${MAUMAU_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MAUMAU_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MAUMAU_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MAUMAU_INSTALL_LATEST_OUT:-}"
fi
