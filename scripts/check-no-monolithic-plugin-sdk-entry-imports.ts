...
import { discoverMaumauPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']maumau\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']maumau\/plugin-sdk\/compat["']/;
...
function main() {
  ...
  if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic maumau/plugin-sdk.");
      for (const file of monolithicOffenders.toSorted()) {
        console.error(`- ${relativeToCwd(file)}`);
      }
    }
    if (legacyCompatOffenders.length > 0) {
      console.error("Bundled plugin source files must not import legacy maumau/plugin-sdk/compat.");
    ...
    if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
      console.error(
        "Use maumau/plugin-sdk/<domain> or maumau/plugin-sdk/<channel> subpaths for bundled plugins; root and compat are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
