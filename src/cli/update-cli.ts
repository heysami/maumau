...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Maumau and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["maumau update", "Update a source checkout (git)"],
        ["maumau update --channel beta", "Switch to beta channel (git + npm)"],
        ["maumau update --channel dev", "Switch to dev channel (git + npm)"],
        ["maumau update --tag beta", "One-off update to a dist-tag or version"],
        ["maumau update --tag main", "One-off package install from GitHub main"],
        ["maumau update --dry-run", "Preview actions without changing anything"],
        ["maumau update --no-restart", "Update without restarting the service"],
        ["maumau update --json", "Output result as JSON"],
        ["maumau update --yes", "Non-interactive (accept downgrade prompts)"],
        ["maumau update wizard", "Interactive update wizard"],
        ["maumau --update", "Shorthand for maumau update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run maumau update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.maumau.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["maumau update status", "Show channel + version status."],
          ["maumau update status --json", "JSON output."],
          ["maumau update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.maumau.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
