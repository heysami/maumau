...
import { parseReleaseVersion } from "../maumau-npm-release-check.ts";
...
export function collectPublishablePluginPackageErrors(
  ...
  const extensions = packageJson.maumau?.extensions ?? [];

  if (!packageName.startsWith("@maumau/")) {
    errors.push(`package name must start with "@maumau/"; found "${packageName || "<missing>"}".`);
  }
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("maumau.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("maumau.extensions must contain only non-empty strings.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const dir of dirs) {
    ...
    if (packageJson.maumau?.release?.publishToNpm !== true) {
      continue;
    }
    ...
    publishable.push({
      ...
      installNpmSpec: packageJson.maumau?.install?.npmSpec?.trim() || undefined,
    });
  }
...
export function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "maumau-plugin-npm-view-"));
