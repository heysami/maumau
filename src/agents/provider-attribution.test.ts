...
describe("provider attribution", () => {
  it("resolves the canonical Maumau product and runtime version", () => {
    const identity = resolveProviderAttributionIdentity({
      MAUMAU_VERSION: "2026.3.99",
    });
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Maumau runtime wrapper.",
      product: "Maumau",
      version: "2026.3.22",
      headers: {
        "HTTP-Referer": "https://maumau.ai",
        "X-OpenRouter-Title": "Maumau",
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { MAUMAU_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "maumau",
        "User-Agent": "maumau/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { MAUMAU_VERSION: "2026.3.22" })).toEqual({
      originator: "maumau",
      "User-Agent": "maumau/2026.3.22",
    });
  });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { MAUMAU_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ MAUMAU_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
