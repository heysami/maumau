...
import { clearPluginDiscoveryCache, discoverMaumauPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("maumau-plugins", tempDirs);
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverMaumauPlugins>[0],
) {
  return discoverMaumauPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      maumau: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverMaumauPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".maumau", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".maumau", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverMaumauPlugins({
  ...
  it("strips provider suffixes from package-derived ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@maumau/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("normalizes bundled speech package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@maumau/elevenlabs-speech",
      extensions: ["./src/index.ts"],
    });
    writePluginPackageManifest({
      packageDir: microsoftDir,
      packageName: "@maumau/microsoft-speech",
      extensions: ["./src/index.ts"],
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@maumau/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("falls back to legacy index discovery when a scanned bundle sidecar is malformed", async () => {
    ...
    expect(legacy?.format).toBe("maumau");
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@maumau/escape-pack",
  ...
  it("skips missing package extension entries without escape diagnostics", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@maumau/missing-entry-pack",
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverMaumauPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        MAUMAU_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverMaumauPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        MAUMAU_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverMaumauPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        MAUMAU_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
