...
function resolveAttachmentLimits(config: MaumauConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".maumau", "attachments");
  const relDir = path.posix.join(".maumau", "attachments", attachmentId);
