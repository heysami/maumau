...
function listAgents(cfg: MaumauConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: MaumauConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<MaumauConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: MaumauConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: MaumauConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: MaumauConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<MaumauConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  MaumauConfig,
  {
    bindingsRef: MaumauConfig["bindings"];
    agentsRef: MaumauConfig["agents"];
    sessionRef: MaumauConfig["session"];
...
function resolveRouteCacheForConfig(cfg: MaumauConfig): Map<string, ResolvedAgentRoute> {
