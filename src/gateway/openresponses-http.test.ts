...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("MAUMAU_CONFIG_PATH is required for gateway config tests");
  }
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "maumau", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `maumau` or `maumau/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "maumau", input: "hi" },
        { "x-maumau-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "maumau/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "maumau/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "maumau", input: "hi" },
        { "x-maumau-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-maumau-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
