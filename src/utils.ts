...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".maumau");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.MAUMAU_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$MAUMAU_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via MAUMAU_STATE_DIR.
