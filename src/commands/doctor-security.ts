...
import type { MaumauConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: MaumauConfig): string[] {
...
export async function noteSecurityWarnings(cfg: MaumauConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("maumau security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.maumau/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("maumau approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.maumau.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("maumau configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("maumau config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("maumau doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("maumau config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('maumau config set session.dmScope "per-channel-peer"') +
