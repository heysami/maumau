...
import type { MaumauPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/maumau-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): MaumauPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("maumau.webhook.received")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("maumau.webhook.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("maumau.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("maumau.message.processed")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("maumau.message.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.histograms.get("maumau.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("maumau.session.stuck")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("maumau.session.stuck_age_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("maumau.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("maumau.webhook.processed");
    expect(spanNames).toContain("maumau.message.processed");
    expect(spanNames).toContain("maumau.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["maumau.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("maumau.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "maumau.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["maumau.reason"]).toBe("string");
    expect(String(attrs?.["maumau.reason"])).not.toContain(
