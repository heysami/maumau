...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: MaumauProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: MaumauProtocol.AnyCodable] = [
            "presence": .init([MaumauProtocol.AnyCodable(entry)]),
        ]
        let payload = MaumauProtocol.AnyCodable(payloadMap)
