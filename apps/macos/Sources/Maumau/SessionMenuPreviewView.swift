...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.maumau", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> MaumauSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: MaumauSessionPreviewEntry,
    ...
    private static func cache(payload: MaumauSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: MaumauChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [MaumauKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [MaumauKit.AnyCodable]) -> [MaumauChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(MaumauChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: MaumauChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: MaumauChatMessage) -> Bool {
    ...
    private static func toolNames(for message: MaumauChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: MaumauChatMessage) -> String? {
