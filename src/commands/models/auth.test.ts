...
describe("modelsAuthLoginCommand", () => {
  ...
  beforeEach(async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/maumau/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/maumau/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/maumau/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(async (mutator: (cfg: MaumauConfig) => MaumauConfig) => {
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/maumau/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `maumau plugins list --json`.',
    );
  });
