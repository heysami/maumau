...
read_when:
  - You want to place an outbound voice call from Maumau
...
Voice calls for Maumau via a plugin. Supports outbound notifications and
...
- Use `maumau voicecall ...` or the `voice_call` tool
...
```bash
maumau plugins install ./extensions/voice-call
...
maumau voicecall start --to "+15555550123"   # alias for call
...
maumau voicecall latency                     # summarize turn latency from logs
