...
async function withOutsideHardlinkInMaumauTmp<T>(
  params: {
    maumauTmpDir: string;
  ...
  const hardlinkPath = path.join(params.maumauTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.maumauTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.maumauTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.maumauTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const maumauTmpDir = resolvePreferredMaumauTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Maumau tmp root",
      media: path.join(maumauTmpDir, "image.png"),
      expected: path.join(maumauTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Maumau tmp root",
      media: pathToFileURL(path.join(maumauTmpDir, "photo.png")).href,
      expected: path.join(maumauTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Maumau tmp root",
      media: path.join(maumauTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(maumauTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(maumauTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside maumau tmp root",
      media: path.join(os.tmpdir(), "outside-maumau", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Maumau tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(maumauTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(maumauTmpDir, { recursive: true });
      const symlinkPath = path.join(maumauTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Maumau tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInMaumauTmp(
      {
        maumauTmpDir,
  ...
  it("rejects symlinked Maumau tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInMaumauTmp(
