...
- 原生 **Maumau 插件**（`maumau.plugin.json` + 运行时模块），或
- 兼容的 **bundle**（`.codex-plugin/plugin.json` 或 `.claude-plugin/plugin.json`）

两者都会显示在 `maumau plugins` 下，但只有原生 Maumau 插件会在进程内执行运行时代码。

大多数情况下，当你想要某项核心 Maumau 尚未内置的功能时，就会使用插件（或者你希望将可选功能保留在主安装之外）。
...
```bash
maumau plugins install @maumau/voice-call
```
...
裸规格和 `@latest` 会停留在稳定通道。如果 npm 将其中任一解析为预发布版本，Maumau 会停止并要求你通过预发布标签（例如 `@beta`/`@rc`）或精确的预发布版本显式选择加入。
...
```bash
maumau plugins install ./my-bundle
maumau plugins install ./my-bundle.tgz
```
...
1. **清单 + 发现**
   Maumau 会从已配置路径、工作区根目录、全局扩展根目录以及随附扩展中查找候选插件。发现过程会先读取原生 `maumau.plugin.json` 清单以及受支持的 bundle 清单。
2. **启用 + 验证**
   核心会决定某个已发现插件是启用、禁用、阻止，还是被选用于某个独占插槽（例如内存）。
3. **运行时加载**
   原生 Maumau 插件通过 jiti 在进程内加载，并将功能注册到一个中央注册表中。兼容的 bundle 会被规范化为注册表记录，而不会导入运行时代码。
4. **表面消费**
   Maumau 的其余部分会读取注册表，以公开工具、渠道、提供商设置、hooks、HTTP 路由、CLI 命令和服务。
...
这种拆分让 Maumau 能够在完整运行时激活之前，就验证配置、解释缺失/被禁用的插件，并构建 UI/schema 提示。

## 兼容的 bundle

Maumau 还识别两种兼容的外部 bundle 布局：
...
目前，Maumau 将这些内容视为 **能力包**，而不是原生运行时插件：

- 当前支持：捆绑的 `skills`
- 当前支持：Claude `commands/` markdown 根目录，映射到常规 Maumau 技能加载器
- 当前支持：Claude bundle `settings.json` 默认值，用于嵌入式 Pi 智能体设置（会清理 shell override 键）
- 当前支持：Cursor `.cursor/commands/*.md` 根目录，映射到常规 Maumau 技能加载器
- 当前支持：使用 Maumau hook-pack 布局的 Codex bundle hook 目录（`HOOK.md` + `handler.ts`/`handler.js`）
...
Bundle hook 支持仅限于常规 Maumau hook 目录格式（在声明的 hook 根目录下使用 `HOOK.md` 加上 `handler.ts`/`handler.js`）。
供应商特定的 shell/JSON hook 运行时，包括 Claude `hooks.json`，当前仅会被检测，不会被直接执行。

## 执行模型

原生 Maumau 插件与 Gateway 网关 **在同一进程内** 运行。它们不会进行沙箱隔离。已加载的原生插件与核心代码处于相同的进程级信任边界。

这意味着：

- 原生插件可以注册工具、网络处理器、hooks 和服务
- 原生插件中的 bug 可能导致 gateway 崩溃或不稳定
- 恶意原生插件等同于在 Maumau 进程内部执行任意代码

兼容的 bundle 默认更安全，因为 Maumau 目前将它们视为元数据/内容包。在当前版本中，这主要意味着捆绑的技能。
...
- Microsoft Teams 自 `2026.1.15` 起仅以插件形式提供；如果你使用 Teams，请安装 `@maumau/msteams`。
...
- [Voice Call](/plugins/voice-call) — `@maumau/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@maumau/zalouser`
- [Matrix](/channels/matrix) — `@maumau/matrix`
- [Nostr](/channels/nostr) — `@maumau/nostr`
- [Zalo](/channels/zalo) — `@maumau/zalo`
- [Microsoft Teams](/channels/msteams) — `@maumau/msteams`
...
原生 Maumau 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。
...
原生 Maumau 插件与 Gateway 网关 **在同一进程内** 运行，因此应将其视为受信任代码。
...
Maumau 仍然负责通用的智能体循环、故障切换、转录处理和工具策略。这些 hooks 是 provider 特定行为的接缝，而无需整个自定义推理传输层。
...
对于 model/provider 插件，Maumau 大致按以下顺序使用 hooks：

1. `catalog`
   在生成 `models.json` 期间，将 provider 配置发布到 `models.providers` 中。
2. 内置/已发现模型查找
   Maumau 会先尝试常规注册表/目录路径。
...
如果 provider 需要完全自定义的线协议或自定义请求执行器，那属于另一类扩展。这些 hooks 适用于仍运行在 Maumau 常规推理循环上的 provider 行为。
...
- OpenRouter 使用 `catalog` 以及 `resolveDynamicModel` 和 `prepareDynamicModel`，因为该 provider 是透传型的，可能会在 Maumau 静态目录更新之前暴露新的 model id。
...
编写插件时，请使用 SDK 子路径，而不是单体式 `maumau/plugin-sdk` 导入：

- `maumau/plugin-sdk/core` 用于通用插件 API、provider 身份验证类型和共享辅助工具。
- `maumau/plugin-sdk/compat` 用于比 `core` 需要更广泛共享运行时辅助工具的捆绑/内部插件代码。
- `maumau/plugin-sdk/telegram` 用于 Telegram 渠道插件。
- `maumau/plugin-sdk/discord` 用于 Discord 渠道插件。
- `maumau/plugin-sdk/slack` 用于 Slack 渠道插件。
- `maumau/plugin-sdk/signal` 用于 Signal 渠道插件。
- `maumau/plugin-sdk/imessage` 用于 iMessage 渠道插件。
- `maumau/plugin-sdk/whatsapp` 用于 WhatsApp 渠道插件。
- `maumau/plugin-sdk/line` 用于 LINE 渠道插件。
- `maumau/plugin-sdk/msteams` 用于捆绑的 Microsoft Teams 插件表面。
- 也提供捆绑扩展专用子路径：
  `maumau/plugin-sdk/acpx`、`maumau/plugin-sdk/bluebubbles`、
  `maumau/plugin-sdk/copilot-proxy`、`maumau/plugin-sdk/device-pair`、
  `maumau/plugin-sdk/diagnostics-otel`、`maumau/plugin-sdk/diffs`、
  `maumau/plugin-sdk/feishu`、`maumau/plugin-sdk/googlechat`、
  `maumau/plugin-sdk/irc`、`maumau/plugin-sdk/llm-task`、
  `maumau/plugin-sdk/lobster`、`maumau/plugin-sdk/matrix`、
  `maumau/plugin-sdk/mattermost`、`maumau/plugin-sdk/memory-core`、
  `maumau/plugin-sdk/memory-lancedb`、
  `maumau/plugin-sdk/minimax-portal-auth`、
  `maumau/plugin-sdk/nextcloud-talk`、`maumau/plugin-sdk/nostr`、
  `maumau/plugin-sdk/open-prose`、`maumau/plugin-sdk/phone-control`、
  `maumau/plugin-sdk/qwen-portal-auth`、`maumau/plugin-sdk/synology-chat`、
  `maumau/plugin-sdk/talk-voice`、`maumau/plugin-sdk/test-utils`、
  `maumau/plugin-sdk/thread-ownership`、`maumau/plugin-sdk/tlon`、
  `maumau/plugin-sdk/twitch`、`maumau/plugin-sdk/voice-call`、
  `maumau/plugin-sdk/zalo` 和 `maumau/plugin-sdk/zalouser`。
...
`catalog.order` 控制插件目录相对于 Maumau 内置隐式 provider 的合并时机：
...
- 如果同时注册了 `catalog` 和 `discovery`，Maumau 会使用 `catalog`

兼容性说明：

- `maumau/plugin-sdk` 仍支持现有外部插件。
...
- 只读命令路径，例如 `maumau status`、`maumau status --all`、
  `maumau channels status`、`maumau channels resolve` 以及 Doctor/配置修复流程，不应仅为了描述配置就必须实体化运行时凭据。
...
- 设置 `MAUMAU_DISABLE_PLUGIN_DISCOVERY_CACHE=1` 或
  `MAUMAU_DISABLE_PLUGIN_MANIFEST_CACHE=1` 可禁用这些缓存。
- 使用 `MAUMAU_PLUGIN_DISCOVERY_CACHE_MS` 和
  `MAUMAU_PLUGIN_MANIFEST_CACHE_MS` 调整缓存窗口。
...
- `<workspace>/.maumau/extensions/*.ts`
- `<workspace>/.maumau/extensions/*/index.ts`

3. 全局扩展

- `~/.maumau/extensions/*.ts`
- `~/.maumau/extensions/*/index.ts`

4. 捆绑扩展（随 Maumau 一起提供；默认开启/关闭混合）

- `<maumau>/extensions/*`

许多捆绑的 provider 插件默认启用，这样模型目录/运行时 hooks 无需额外设置即可使用。其他插件仍需要通过 `plugins.entries.<id>.enabled` 或
`maumau plugins enable <id>` 显式启用。
...
- 如果 `plugins.allow` 为空且可发现非捆绑插件，Maumau 会在启动时记录一条警告，其中包含 plugin id 和来源。
- 候选路径在被接受进入发现流程前会经过安全检查。当出现以下情况时，Maumau 会阻止候选项：
...
每个原生 Maumau 插件都必须在其根目录中包含一个 `maumau.plugin.json` 文件。
...
插件目录可以包含带有 `maumau.extensions` 的 `package.json`：
...
安全护栏：每个 `maumau.extensions` 条目在解析符号链接后都必须保持在插件目录内。逃逸出包目录的条目会被拒绝。

安全说明：`maumau plugins install` 会使用
`npm install --ignore-scripts` 安装插件依赖（不会运行生命周期脚本）。
请保持插件依赖树为“纯 JS/TS”，并避免需要 `postinstall` 构建的包。

可选项：`maumau.setupEntry` 可以指向一个轻量级、仅用于 setup 的模块。
当 Maumau 需要为一个已禁用的渠道插件提供 setup 表面，或某个渠道插件虽然已启用但仍未配置时，它会加载 `setupEntry`，而不是完整插件入口。
当你的主插件入口还连接了工具、hooks 或其他仅运行时代码时，这能让启动和 setup 更轻量。

### 渠道目录元数据

渠道插件可以通过 `maumau.channel` 公布 setup/discovery 元数据，并通过 `maumau.install` 公布安装提示。这使核心目录保持无数据化。

示例：

```json
{
  "name": "@maumau/nextcloud-talk",
  "maumau": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
Maumau 还可以合并 **外部渠道目录**（例如 MPM registry 导出）。
将 JSON 文件放到以下任一位置：

- `~/.maumau/mpm/plugins.json`
- `~/.maumau/mpm/catalog.json`
- `~/.maumau/plugins/catalog.json`

或者将 `MAUMAU_PLUGIN_CATALOG_PATHS`（或 `MAUMAU_MPM_CATALOG_PATHS`）指向一个或多个 JSON 文件（用逗号/分号/`PATH` 分隔）。
每个文件应包含
`{ "entries": [ { "name": "@scope/pkg", "maumau": { "channel": {...}, "install": {...} } } ] }`。
...
如果插件导出 `id`，Maumau 会使用它，但如果它与配置的 id 不匹配，则会发出警告。
...
- 原生插件配置使用嵌入在 `maumau.plugin.json` 中的 JSON Schema（`configSchema`）进行验证。
- 兼容的 bundle 当前不公开原生 Maumau 配置 schema。
...
Maumau 会保留已禁用插件的配置，因此重新启用它们不会造成破坏。
...
Maumau 会在运行时根据已发现的插件增强 `uiHints`：
...
```bash
maumau plugins list
maumau plugins info <id>
maumau plugins install <path>                 # 将本地文件/目录复制到 ~/.maumau/extensions/<id>
maumau plugins install ./extensions/voice-call # 支持相对路径
maumau plugins install ./plugin.tgz           # 从本地 tarball 安装
maumau plugins install ./plugin.zip           # 从本地 zip 安装
maumau plugins install -l ./extensions/voice-call # link（不复制），用于开发
maumau plugins install @maumau/voice-call # 从 npm 安装
maumau plugins install @maumau/voice-call --pin # 存储精确解析后的 name@version
maumau plugins update <id>
maumau plugins update --all
maumau plugins enable <id>
maumau plugins disable <id>
maumau plugins doctor
```

`maumau plugins list` 会将顶层格式显示为 `maumau` 或 `bundle`。
详细列表/info 输出还会显示 bundle 子类型（`codex` 或 `claude`）以及已检测到的 bundle 能力。

`plugins update` 仅适用于在 `plugins.installs` 下跟踪的 npm 安装。
如果更新之间存储的完整性元数据发生变化，Maumau 会发出警告并要求确认（使用全局 `--yes` 可绕过提示）。

插件也可以注册自己的顶层命令（例如：`maumau voicecall`）。
...
```ts
import { delegateCompactionToRuntime } from "maumau/plugin-sdk";
...
- 插件管理的 hooks 会在 `maumau hooks list` 中显示为 `plugin:<id>`。
- 你不能通过 `maumau hooks` 启用/禁用插件管理的 hooks；应改为启用/禁用插件本身。
...
- 禁用后，Maumau 会阻止 `before_prompt_build`，并忽略旧版 `before_agent_start` 返回的提示词变更字段，同时保留旧版 `modelOverride` 和 `providerOverride`。
...
插件可以注册 **模型 providers**，从而让用户能够在 Maumau 内完成 OAuth 或 API 密钥设置、在新手引导/模型选择器中显示 provider 设置，并参与隐式 provider 发现。
...
2. **非交互式设置**
   `auth[].runNonInteractive(ctx)` 处理 `maumau onboard --non-interactive`，且不进行提示。当 provider 需要超出内置简单 API 密钥路径之外的自定义无头设置时，请使用它。
3. **向导集成**
   `wizard.setup` 会向 `maumau onboard` 添加一个条目。
...
- `configPatch`：可选的 `maumau.json` 变更
...
当 provider 具有多种身份验证方法时，向导可以显式指向其中一种方法，也可以让 Maumau 为每种方法自动生成选择项。

在插件注册时，Maumau 会验证 provider 向导元数据：
...
- `maumau models auth login --provider <id> [--method <id>]`
- `maumau onboard`
...
- `run` 会接收一个 `ProviderAuthContext`，其中包含 `prompter`、`runtime`、
  ...
  `allowSecretRefPrompt` 辅助工具/状态。新手引导/配置流程可以用它们来遵守 `--secret-input-mode`，或提供 env/file/exec secret-ref 采集，而 `maumau models auth` 会保持更严格的提示表面。
...
- `wizard.setup` 会向 `maumau onboard` 添加一个 provider 选项。
...
- 主包：`maumau`（本仓库）
- 插件：位于 `@maumau/*` 下的独立 npm 包（例如：`@maumau/voice-call`）

发布契约：

- 插件 `package.json` 必须包含 `maumau.extensions`，并指向一个或多个入口文件。
- 可选：`maumau.setupEntry` 可指向一个轻量级的仅 setup 入口，用于已禁用或尚未配置完成的渠道 setup。
- 入口文件可以是 `.js` 或 `.ts`（jiti 会在运行时加载 TS）。
- `maumau plugins install <npm-spec>` 会使用 `npm pack`，提取到 `~/.maumau/extensions/<id>/`，并在配置中启用它。
...
- CLI：`maumau voicecall start|status`
- 工具：`voice_call`
...
- 单独发布的插件应运行自己的 CI（lint/build/test），并验证 `maumau.extensions` 指向构建后的入口点（`dist/index.js`）。
