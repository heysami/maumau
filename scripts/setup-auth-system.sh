#!/bin/bash
# Setup Maumau Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'maumau-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Maumau message: Send warning via Maumau itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/maumau-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/maumau-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/maumau-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now maumau-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status maumau-auth-monitor.timer"
echo ""
echo "Quick commands:"
