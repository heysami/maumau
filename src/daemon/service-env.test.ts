...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.MAUMAU_GATEWAY_PORT).toBe("18789");
    expect(env.MAUMAU_GATEWAY_TOKEN).toBeUndefined();
    expect(env.MAUMAU_SERVICE_MARKER).toBe("maumau");
    expect(env.MAUMAU_SERVICE_KIND).toBe("gateway");
    expect(typeof env.MAUMAU_SERVICE_VERSION).toBe("string");
    expect(env.MAUMAU_SYSTEMD_UNIT).toBe("maumau-gateway.service");
    expect(env.MAUMAU_WINDOWS_TASK_NAME).toBe("Maumau Gateway");
    if (process.platform === "darwin") {
      expect(env.MAUMAU_LAUNCHD_LABEL).toBe("ai.maumau.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", MAUMAU_PROFILE: "work" },
      port: 18789,
    });
    expect(env.MAUMAU_SYSTEMD_UNIT).toBe("maumau-gateway-work.service");
    expect(env.MAUMAU_WINDOWS_TASK_NAME).toBe("Maumau Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.MAUMAU_LAUNCHD_LABEL).toBe("ai.maumau.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through MAUMAU_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", MAUMAU_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.MAUMAU_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits MAUMAU_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".maumau"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", MAUMAU_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".maumau-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", MAUMAU_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".maumau"));
  });

  it("uses MAUMAU_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", MAUMAU_STATE_DIR: "/var/lib/maumau" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/maumau"));
  });

  it("expands ~ in MAUMAU_STATE_DIR", () => {
    const env = { HOME: "/Users/test", MAUMAU_STATE_DIR: "~/maumau-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/maumau-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { MAUMAU_STATE_DIR: "C:\\State\\maumau" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\maumau");
  });
});
