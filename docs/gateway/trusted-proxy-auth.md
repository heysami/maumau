...
read_when:
  - Running Maumau behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Maumau
...
- You run Maumau behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Maumau checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Maumau extracts the user identity from the configured header
...
- Maumau can stay on loopback HTTP behind the proxy.
...
If Maumau itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://maumau.example.com
    to: http://maumau-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`maumau security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Maumau config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `maumau security audit` and review findings
