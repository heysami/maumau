...
import * as ssrf from "maumau/plugin-sdk/infra-runtime";
import { resetInboundDedupe } from "maumau/plugin-sdk/reply-runtime";
import { resetLogger, setLoggerOverride } from "maumau/plugin-sdk/runtime-env";
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-session-"));
