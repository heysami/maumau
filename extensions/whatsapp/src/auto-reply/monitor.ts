import { resolveInboundDebounceMs } from "maumau/plugin-sdk/channel-inbound";
import { formatCliCommand } from "maumau/plugin-sdk/cli-runtime";
import { waitForever } from "maumau/plugin-sdk/cli-runtime";
import { hasControlCommand } from "maumau/plugin-sdk/command-auth";
import { loadConfig } from "maumau/plugin-sdk/config-runtime";
import { formatDurationPrecise } from "maumau/plugin-sdk/infra-runtime";
import { enqueueSystemEvent } from "maumau/plugin-sdk/infra-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "maumau/plugin-sdk/reply-history";
import { getReplyFromConfig } from "maumau/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "maumau/plugin-sdk/routing";
import { logVerbose } from "maumau/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "maumau/plugin-sdk/runtime-env";
import { getChildLogger } from "maumau/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "maumau/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("maumau channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("maumau channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
