...
struct IOSGatewayChatTransport: MaumauChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.maumau", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> MaumauChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(MaumauChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> MaumauChatHistoryPayload {
        ...
        return try JSONDecoder().decode(MaumauChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [MaumauChatAttachmentPayload]) async throws -> MaumauChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [MaumauChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(MaumauChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(MaumauGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MaumauGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MaumauChatEventPayload.self)
                        {
