    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["maumau node run --host 127.0.0.1 --port 18789", "Run the node host in the foreground."],
          ["maumau node status", "Check node host service status."],
          ["maumau node install", "Install the node host service."],
          ["maumau node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.maumau.ai/cli/node")}\n`,
    );
