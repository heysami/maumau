...
summary: "Architecture of Maumau's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Maumau
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Maumau integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Maumau uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Maumau directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Maumau's local TUI mode)                                               |
...
├── pi-tools.ts                    # createMaumauCodingTools()
...
├── maumau-tools.ts              # Maumau-specific tools
...
Image injection is prompt-local: Maumau loads image refs from the current prompt and
...
2. **Custom Replacements**: Maumau replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Maumau Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
Maumau wraps this with `guardSessionManager()` for tool result safety.
...
Maumau maintains an auth profile store with multiple API keys per provider:
...
Maumau loads custom pi extensions for specialized behavior:
...
Maumau also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Maumau Embedded                                                                            |
| --------------- | ----------------------- | ------------------------------------------------------------------------------------------ |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                             |
| Tools           | Default coding tools    | Custom Maumau tool suite                                                                   |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                |
| Session storage | `~/.pi/agent/sessions/` | `~/.maumau/agents/<agentId>/sessions/` (or `$MAUMAU_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `MAUMAU_LIVE_TEST=1`)
