...
export function resolveHooksConfig(cfg: MaumauConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: MaumauConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-maumau-token"] === "string" ? req.headers["x-maumau-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-maumau-idempotency-key"]) ||
