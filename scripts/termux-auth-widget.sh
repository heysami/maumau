...
SERVER="${MAUMAU_SERVER:-l36}"

# Check auth status
termux-toast "Checking Maumau auth..."

STATUS=$(ssh "$SERVER" '$HOME/maumau/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/maumau/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|MAUMAU_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/maumau/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Maumau won't work until re-auth"
                ;;
        esac
        ;;
