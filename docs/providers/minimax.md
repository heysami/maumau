---
summary: "Use MiniMax models in Maumau"
read_when:
  - You want MiniMax models in Maumau
...
Maumau's MiniMax provider defaults to **MiniMax M2.7** and keeps
...
```bash
maumau plugins enable minimax  # skip if already loaded.
maumau gateway restart  # restart if gateway is already running
maumau onboard --auth-choice minimax-portal
```
...
See [MiniMax plugin README](https://github.com/maumau/maumau/tree/main/extensions/minimax) for details.
...
Configure manually via `maumau.json`:
...
- Use `maumau models list` and `maumau models set minimax/MiniMax-M2.7` to switch.
...
- Running `maumau configure` and selecting a **MiniMax** auth option, or
