...
import type { MaumauPluginApi, MaumauPluginToolContext } from "./api.js";
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<MaumauPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: MaumauPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<MaumauPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as MaumauPluginApi);
