...
function resolveQianfanPreset(cfg: MaumauConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: MaumauConfig) => {
...
export function applyQianfanProviderConfig(cfg: MaumauConfig): MaumauConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: MaumauConfig): MaumauConfig {
