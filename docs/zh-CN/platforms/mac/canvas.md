...
- `~/Library/Application Support/Maumau/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `maumau-canvas://<session>/<path>`

示例：

- `maumau-canvas://main/` → `<canvasRoot>/main/index.html`
- `maumau-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `maumau-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
maumau nodes canvas present --node <id>
maumau nodes canvas navigate --node <id> --url "/"
maumau nodes canvas eval --node <id> --js "document.title"
maumau nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__maumau__/a2ui/
```
...
maumau nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
maumau nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "maumau://agent?message=Review%20this%20design";
```
