...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  Maumau  ", 20)).toBe("Maumau");
    expect(coerceIdentityValue("  Maumau  ", 8)).toBe("Maumau");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  Maumau Assistant  ", 8)).toBe("Maumau");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  Maumau  ", 0)).toBe("");
    expect(coerceIdentityValue("  Maumau  ", -1)).toBe("OpenCla");
  });
});
