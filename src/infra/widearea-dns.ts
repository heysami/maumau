...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MAUMAU_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*maumau-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "maumau";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "maumau");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "maumau-gw");
  ...
  records.push(`_maumau-gw._tcp IN PTR ${instanceLabel}._maumau-gw._tcp`);
  records.push(`${instanceLabel}._maumau-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._maumau-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; maumau-content-hash: ${contentHash}\n${contentBody}`;
}
