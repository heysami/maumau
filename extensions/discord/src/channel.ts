...
import { createScopedDmSecurityResolver } from "maumau/plugin-sdk/channel-config-helpers";
import { createPairingPrefixStripper } from "maumau/plugin-sdk/channel-pairing";
import { createOpenProviderConfiguredRouteWarningCollector } from "maumau/plugin-sdk/channel-policy";
import { resolveTargetsWithOptionalToken } from "maumau/plugin-sdk/channel-targets";
...
function hasDiscordExecApprovalDmRoute(cfg: MaumauConfig): boolean {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). Maumau adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
