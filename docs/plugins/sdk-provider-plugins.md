...
summary: "Step-by-step guide to building a model provider plugin for Maumau"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Maumau
...
(LLM) to Maumau. By the end you will have a provider with a model catalog,
API key auth, and dynamic model resolution.

<Info>
  If you have not built any Maumau plugin before, read
...
<Steps>
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so Maumau can detect
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "maumau/plugin-sdk/provider-auth";
    ...
    `maumau onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "maumau/plugin-sdk/provider-entry";
    ...
    `maumau/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      Maumau calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Maumau classifies this as a **hybrid-capability** plugin. This is the
...
```
extensions/acme-ai/
├── package.json              # maumau.providers metadata
├── maumau.plugin.json      # Manifest with providerAuthEnvVars
