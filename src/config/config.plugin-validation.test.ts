...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "maumau.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      MAUMAU_STATE_DIR: path.join(suiteHome, ".maumau"),
      MAUMAU_PLUGIN_MANIFEST_CACHE_MS: "10000",
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-config-plugin-validation-"));
  ...
  it("does not fail validation for the implicit default memory slot when plugins config is explicit", async () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          MAUMAU_BUNDLED_PLUGINS_DIR: path.join(suiteHome, "missing-bundled-plugins"),
        },
      },
    );
