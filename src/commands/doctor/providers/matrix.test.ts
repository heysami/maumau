...
describe("doctor matrix provider helpers", () => {
  ...
  it("warns on stale custom Matrix plugin paths", async () => {
    const missingPath = path.join(tmpdir(), "maumau-matrix-missing-provider-test");
    ...
    expect(warnings[1]).toContain("maumau plugins install @maumau/matrix");
    expect(warnings[2]).toContain("maumau plugins install ./extensions/matrix");
  });
  ...
  it("removes stale Matrix plugin config when install path is missing", async () => {
    const missingPath = path.join(tmpdir(), "maumau-matrix-stale-cleanup-test-" + Date.now());
    await fs.rm(missingPath, { recursive: true, force: true });
