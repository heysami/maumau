...
type GatewaySecretDefaults = NonNullable<MaumauConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${MAUMAU_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.MAUMAU_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.MAUMAU_GATEWAY_PASSWORD);
}
