...
val androidStoreFile = providers.gradleProperty("MAUMAU_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("MAUMAU_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("MAUMAU_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("MAUMAU_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set MAUMAU_ANDROID_STORE_FILE, " +
            "MAUMAU_ANDROID_STORE_PASSWORD, MAUMAU_ANDROID_KEY_ALIAS, and " +
            "MAUMAU_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/MaumauKit/Sources/MaumauKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.maumau.app"
        minSdk = 31
        targetSdk = 36
        versionCode = 2026032902
        versionName = "2026.3.29-beta.2"
        ndk {
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "MAUMAU_ENABLE_SMS", "false")
            buildConfigField("boolean", "MAUMAU_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "MAUMAU_ENABLE_SMS", "true")
            buildConfigField("boolean", "MAUMAU_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "maumau-$versionName-$buildType.apk"
                    } else {
                        "maumau-$versionName-$flavorName-$buildType.apk"
                    }
