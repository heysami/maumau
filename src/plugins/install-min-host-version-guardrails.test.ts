...
describe("install minHostVersion guardrails", () => {
  it("requires published plugins that depend on new sdk subpaths to declare a host floor", () => {
    ...
    for (const relativePath of PLUGIN_MANIFEST_PATHS_REQUIRING_MIN_HOST_VERSION) {
      ...
      const requirement = parseMinHostVersionRequirement(manifest.maumau?.install?.minHostVersion);

      expect(
        requirement,
        `${relativePath} should declare maumau.install.minHostVersion`,
      ).not.toBeNull();
      if (!requirement) {
        continue;
      }
      ...
      expect(
        ...
        `${relativePath} should require at least Maumau ${MIN_HOST_VERSION_BASELINE}`,
      ).toBe(true);
    }
  });
});
