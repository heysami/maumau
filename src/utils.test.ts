...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("maumau-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("maumau-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("maumau-lid-a-", async (first) => {
      await withTempDir("maumau-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.maumau when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "maumau-config-dir-"));
    try {
  ...
  it("expands MAUMAU_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/maumau-home",
      MAUMAU_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/maumau-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers MAUMAU_HOME over HOME", () => {
    vi.stubEnv("MAUMAU_HOME", "/srv/maumau-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/maumau-home"));
...
describe("shortenHomePath", () => {
  it("uses $MAUMAU_HOME prefix when MAUMAU_HOME is set", () => {
    vi.stubEnv("MAUMAU_HOME", "/srv/maumau-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/maumau-home")}/.maumau/maumau.json`)).toBe(
      "$MAUMAU_HOME/.maumau/maumau.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $MAUMAU_HOME replacement when MAUMAU_HOME is set", () => {
    vi.stubEnv("MAUMAU_HOME", "/srv/maumau-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/maumau-home")}/.maumau/maumau.json`),
    ).toBe("config: $MAUMAU_HOME/.maumau/maumau.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/maumau", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "maumau"),
    );
  });
  ...
  it("prefers MAUMAU_HOME for tilde expansion", () => {
    vi.stubEnv("MAUMAU_HOME", "/srv/maumau-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/maumau")).toBe(path.resolve("/srv/maumau-home", "maumau"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/maumau", env)).toBe(path.resolve("/srv/maumau-home", "maumau"));
  });
