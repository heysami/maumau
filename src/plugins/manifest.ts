...
// package.json plugin metadata (used for setup/catalog)
...
export type MaumauPackageStartup = {
  /**
...
export type MaumauPackageManifest = {
  extensions?: string[];
  setupEntry?: string;
  channel?: PluginPackageChannel;
  install?: PluginPackageInstall;
  startup?: MaumauPackageStartup;
};
...
): MaumauPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
  for (const key of [MANIFEST_KEY, ...LEGACY_MANIFEST_KEYS] as const) {
    const value = manifest[key as ManifestKey];
    if (value) {
      return value;
    }
  }
