...
import { scheduleGatewaySigusr1Restart, triggerMaumauRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Maumau in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerMaumauRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Maumau via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
