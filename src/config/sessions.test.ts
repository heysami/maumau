...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ MAUMAU_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-maumau",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-maumau#general");
  });
  ...
  it("derives session transcripts dir from MAUMAU_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MAUMAU_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when MAUMAU_STATE_DIR differs from stored paths", () => {
