...
read_when:
  - You are building an Maumau plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (maumau.plugin.json)

This page is for the **native Maumau plugin manifest** only.
...
Maumau auto-detects those bundle layouts too, but they are not validated
against the `maumau.plugin.json` schema described here.

For compatible bundles, Maumau currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches Maumau runtime expectations.

Every native Maumau plugin **must** ship a `maumau.plugin.json` file in the
**plugin root**. Maumau uses this manifest to validate configuration
...
`maumau.plugin.json` is the metadata Maumau reads before it loads your
plugin code.
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Maumau can inspect without loading plugin code.                                        |
...
Maumau reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, Maumau falls back to `choiceId`.                                          |
...
| File                 | Use it for                                                                                                       |
| -------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `maumau.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs         |
| `package.json`       | npm metadata, dependency installation, and the `maumau` block used for entrypoints and setup or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Maumau must know it before loading plugin code, put it in `maumau.plugin.json`
...
- The manifest is **required for native Maumau plugins**, including local filesystem loads.
