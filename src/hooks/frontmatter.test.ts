...
`;
    ...
    expect(parsed.maumau.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.maumau.emoji).toBe("📝");
    expect(parsed.maumau.events).toEqual(["command"]);
    expect(parsed.maumau.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.maumau.install[0].kind).toBe("bundled");
  });
...
describe("resolveMaumauMetadata", () => {
  it("extracts maumau metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveMaumauMetadata(frontmatter);
  ...
  it("returns undefined when maumau key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        maumau: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Maumau" },
            { id: "npm", kind: "npm", package: "@maumau/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@maumau/hook");
  });
...
`;
    ...
    const maumau = resolveMaumauMetadata(frontmatter);
    expect(maumau).toBeDefined();
    expect(maumau?.emoji).toBe("💾");
    expect(maumau?.events).toEqual(["command:new", "command:reset"]);
    expect(maumau?.requires?.config).toEqual(["workspace.dir"]);
    expect(maumau?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const maumau = resolveMaumauMetadata(frontmatter);
    expect(maumau?.emoji).toBe("disk");
    expect(maumau?.events).toEqual(["command:new"]);
  });
});
