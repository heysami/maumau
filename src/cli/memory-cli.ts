    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["maumau memory status", "Show index and provider status."],
          ["maumau memory status --deep", "Probe embedding provider readiness."],
          ["maumau memory index --force", "Force a full reindex."],
          ['maumau memory search "meeting notes"', "Quick search using positional query."],
          [
            'maumau memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["maumau memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.maumau.ai/cli/memory")}\n`,
    );
