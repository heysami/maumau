...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.maumau.mac`) even if the shipped
    ...
    {
        if environment["MAUMAU_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "maumau.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "maumau.nixMode") { return true }
