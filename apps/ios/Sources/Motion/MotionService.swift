...
final class MotionService: MotionServicing {
    func activities(params: MaumauMotionActivityParams) async throws -> MaumauMotionActivityPayload {
        ...
        let mapped: [MaumauMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return MaumauMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: MaumauPedometerParams) async throws -> MaumauPedometerPayload {
        ...
        let payload: MaumauPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = MaumauPedometerPayload(
