...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `maumau.internal.`), **or**
...
Run `maumau devices list` again before approval.

4. Verify connection:

```bash
maumau nodes status
maumau gateway call node.list --params "{}"
```
...
- `MAUMAU_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Maumau iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Maumau builds.
...
```bash
export MAUMAU_APNS_TEAM_ID="TEAMID"
export MAUMAU_APNS_KEY_ID="KEYID"
export MAUMAU_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_maumau-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `maumau.internal.`) and Tailscale split DNS.
...
```bash
maumau nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__maumau__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__maumau__/canvas/` and `/__maumau__/a2ui/`.
...
```bash
maumau nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__maumau; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
maumau nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `maumau devices list` and approve manually.
