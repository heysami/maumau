...
vi.mock("../agents/workspace.js", () => {
  const resolveDefaultAgentWorkspaceDir = () => "/tmp/maumau-workspace";
  return {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "maumau-agent-" });
}
...
function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<MaumauConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<MaumauConfig["channels"]>["telegram"]>>,
...
async function withCrossAgentResumeFixture(
  run: (params: { sessionId: string; sessionKey: string; cfg: MaumauConfig }) => Promise<void>,
): Promise<void> {
