...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  const envSnapshot = captureEnv(["MAUMAU_STATE_DIR", "MAUMAU_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  let tmpDir: string;
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolveMaumauAgentDir() returns mainAgentDir
    process.env.MAUMAU_STATE_DIR = tmpDir;
    process.env.MAUMAU_AGENT_DIR = mainAgentDir;
