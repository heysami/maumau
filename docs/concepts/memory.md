---
title: "Memory"
summary: "How Maumau memory works (workspace files + automatic memory flush)"
read_when:
...
Maumau memory is **plain Markdown in the agent workspace**. The files are the
...
- `MEMORY.md` (optional)
  ...
  - If both `MEMORY.md` and `memory.md` exist at the workspace root, Maumau loads both (deduplicated by realpath so symlinks pointing to the same file are not injected twice).
  - **Only load in the main, private session** (never in group contexts).

These files live under the workspace (`agents.defaults.workspace`, default
`~/.maumau/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

Maumau exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, Maumau triggers a **silent,
...
Maumau can build a small vector index over `MEMORY.md` and `memory/*.md` so
