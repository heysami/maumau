---
summary: "Use OpenCode Zen and Go catalogs with Maumau"
read_when:
...
OpenCode exposes two hosted catalogs in Maumau:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Maumau keeps the runtime provider ids
...
maumau onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
maumau onboard --auth-choice opencode-go
maumau onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
