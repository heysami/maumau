...
Maumau supports **camera capture** for agent workflows:
...
```bash
maumau nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
maumau nodes camera snap --node <id> --facing front
maumau nodes camera clip --node <id> --duration 3000
maumau nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`maumau.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `maumau` CLI to invoke camera commands on the macOS node.

Examples:

```bash
maumau nodes camera list --node <id>            # list camera ids
maumau nodes camera snap --node <id>            # prints MEDIA:<path>
maumau nodes camera snap --node <id> --max-width 1280
maumau nodes camera snap --node <id> --delay-ms 2000
maumau nodes camera snap --node <id> --device-id <id>
maumau nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
maumau nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
maumau nodes camera clip --node <id> --device-id <id>
maumau nodes camera clip --node <id> --no-audio
```

Notes:

- `maumau nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
maumau nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
