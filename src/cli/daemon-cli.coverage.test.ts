...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      MAUMAU_GATEWAY_PORT: String(params.port),
      ...(params.token ? { MAUMAU_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.MAUMAU_STATE_DIR = "/tmp/maumau-cli-state";
    process.env.MAUMAU_CONFIG_PATH = "/tmp/maumau-cli-state/maumau.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        MAUMAU_PROFILE: "dev",
        MAUMAU_STATE_DIR: "/tmp/maumau-daemon-state",
        MAUMAU_CONFIG_PATH: "/tmp/maumau-daemon-state/maumau.json",
        MAUMAU_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.maumau.gateway.plist",
    });
