...
${formatHelpExamples([
  ['maumau agent --to +15555550123 --message "status update"', "Start a new session."],
  ['maumau agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'maumau agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'maumau agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['maumau agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'maumau agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.maumau.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.maumau.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "maumau agents set-identity --identity-file ~/.maumau/workspace/IDENTITY.md --agent main",
