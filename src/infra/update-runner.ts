...
const DEFAULT_PACKAGE_NAME = "maumau";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "maumau.mjs");
    ...
    const doctorStep = await runStep(
      step("maumau doctor", doctorArgv, gitRoot, { MAUMAU_UPDATE_IN_PROGRESS: "1" }),
    );
