...
read_when:
  ...
  - You are defining plugin config schemas or package.json maumau metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`maumau.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `maumau` field that tells the plugin system what
...
When enabled, Maumau loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `maumau.plugin.json` in the package root.
Maumau uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Maumau",
  "configSchema": {
    "type": "object",
...
Maumau loads when it only needs setup surfaces (onboarding, config repair,
...
**When Maumau uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `maumau/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Maumau validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "maumau/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `maumau onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "maumau/plugin-sdk/channel-setup";
...
helpers from `maumau/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`maumau/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `maumau/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "maumau/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/maumau-my-channel",
...
```bash
maumau plugins install @myorg/maumau-my-plugin
```

Maumau tries ClawHub first and falls back to npm automatically. You can also
force a specific source:

```bash
maumau plugins install clawhub:@myorg/maumau-my-plugin   # ClawHub only
maumau plugins install npm:@myorg/maumau-my-plugin       # npm only
```
...
```bash
maumau plugins search <query>
maumau plugins install <package-name>
```

<Info>
  For npm-sourced installs, `maumau plugins install` runs
