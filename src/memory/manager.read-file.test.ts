...
function createMemorySearchCfg(options: { workspaceDir: string; indexPath: string }): MaumauConfig {
  return {
    agents: {
      defaults: {
...
describe("MemoryIndexManager.readFile", () => {
  ...
  beforeAll(async () => {
    resetEmbeddingMocks();
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-mem-read-"));
