...
export function parseMaumauVersion(raw: string | null | undefined): MaumauVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeMaumauVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseMaumauVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameMaumauStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseMaumauVersion(a);
  const parsedB = parseMaumauVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareMaumauVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseMaumauVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  if (isSameMaumauStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareMaumauVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: MaumauVersion): number {
