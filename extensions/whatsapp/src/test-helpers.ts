...
const CONFIG_KEY = Symbol.for("maumau:testConfigMock");
...
vi.mock("maumau/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/media-runtime")>();
  const mockModule = Object.create(null) as Record<string, unknown>;
  Object.defineProperties(mockModule, Object.getOwnPropertyDescriptors(actual));
...
vi.mock("maumau/plugin-sdk/state-paths", async (importOriginal) => {
  const actual = await importOriginal<typeof import("maumau/plugin-sdk/state-paths")>();
  return {
    ...actual,
    resolveOAuthDir: () => "/tmp/maumau-oauth",
  };
});
...
vi.mock("@whiskeysockets/baileys", async (importOriginal) => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("maumau:lastSocket")] =
    created.lastSocket;
  return {
    ...actual,
    ...created.mod,
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("maumau:lastSocket")];
  if (typeof getter === "function") {
