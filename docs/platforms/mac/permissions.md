...
- Same path: run the app from a fixed location (for Maumau, `dist/Maumau.app`).
...
```bash
sudo tccutil reset Accessibility ai.maumau.mac
sudo tccutil reset ScreenCapture ai.maumau.mac
...
Workaround: move files into the Maumau workspace (`~/.maumau/workspace`) if you want to avoid per-folder grants.
