...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: MaumauConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: MaumauConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/maumau-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<MaumauConfig["bindings"]>[number];
