...
describe("openshell remote fs bridge", () => {
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("maumau-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("maumau-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("maumau-openshell-remote-agent-");
