...
beforeEach(() => {
  ...
  originalHideBanner = process.env.MAUMAU_HIDE_BANNER;
  ...
  delete process.env.MAUMAU_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.MAUMAU_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("maumau");
    program
      .command("status")
      .option("--json")
      .action(() => {});
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "maumau", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("maumau-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "maumau", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "maumau", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "maumau", "channels", "add"],
    });
  ...
  it("only allows invalid config for explicit Matrix reinstall requests", async () => {
    await runPreAction({
      parseArgv: ["plugins", "install", "@maumau/matrix"],
      processArgv: ["node", "maumau", "plugins", "install", "@maumau/matrix"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "maumau", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "maumau", "plugins", "install", "./extensions/matrix"],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@maumau/matrix", "--marketplace", "local/repo"],
      processArgv: [
        "node",
        "maumau",
        "plugins",
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "maumau", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.MAUMAU_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "maumau", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "maumau", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "maumau", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "maumau", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "maumau", "agents", "list", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "maumau", "config", "set", "gateway.auth.mode", "local", "--json"],
    });
    ...
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "maumau", "agents", "list"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "maumau", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "maumau", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "maumau", "backup", "create", "--json"],
    });
