...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredMaumauTmpDir: vi.fn(() => "/tmp/maumau"),
}));
vi.mock("../infra/tmp-maumau-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredMaumauTmpDir.mockReturnValue("/tmp/maumau");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".maumau-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredMaumauTmpDir.mockReturnValue("/tmp/maumau-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "maumau-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "maumau-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredMaumauTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "maumau-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "maumau-preferred", "downloads")}${path.sep}`),
    );
  });
