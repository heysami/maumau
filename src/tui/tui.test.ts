...
describe("resolveInitialTuiAgentId", () => {
  const cfg: MaumauConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/maumau" },
        { id: "ops", workspace: "/tmp/maumau/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/maumau/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run maumau devices list");
    expect(state.pairingHint).toContain("maumau devices list");
  });
