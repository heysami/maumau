...
read_when:
  - You need detailed behavior for maumau onboard
...
This page is the full reference for `maumau onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.maumau/maumau.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `maumau doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `maumau pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `maumau health`.
    - `maumau status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.maumau/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.maumau/agents/<agentId>/agent/auth-profiles.json`
...
`~/.maumau/credentials/oauth.json` (or `$MAUMAU_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`maumau onboard`](/cli/onboard)
