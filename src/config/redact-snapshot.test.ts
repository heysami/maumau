...
import type { ConfigFileSnapshot } from "./types.maumau.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.maumau/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/maumau/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/maumau/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/maumau/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/maumau/nickserv-password.txt");
