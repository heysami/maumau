#!/usr/bin/env bash
# ClawDock - Docker helpers for Maumau
# Inspired by Simon Willison's "Running Maumau in Docker"
# https://til.simonwillison.net/llms/maumau-docker
#
# Installation:
...
# Common paths to check for Maumau
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/maumau"
  "${HOME}/workspace/maumau"
  "${HOME}/projects/maumau"
  "${HOME}/dev/maumau"
  "${HOME}/code/maumau"
  "${HOME}/src/maumau"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Maumau at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/maumau"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Maumau not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/maumau/maumau.git ~/maumau"
    echo "  cd ~/maumau && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^MAUMAU_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d maumau-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart maumau-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f maumau-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec maumau-gateway \
    bash -c 'echo "alias maumau=\"./maumau.mjs\"" > /tmp/.bashrc_maumau && bash --rcfile /tmp/.bashrc_maumau'
}
...
clawdock-cli() {
  _clawdock_compose run --rm maumau-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build maumau-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "MAUMAU_GATEWAY_TOKEN=$token" maumau-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" maumau-gateway \
    bash -c './maumau.mjs config set gateway.remote.token "$TOKEN" && ./maumau.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec maumau-gateway \
    bash -c "./maumau.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart maumau-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm maumau-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec maumau-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'maumau config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Maumau${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (maumau alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to maumau project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.maumau)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'maumau channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'maumau status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.maumau.ai${_CLR_RESET}"
  echo ""
}
