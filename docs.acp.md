...
This document describes how the Maumau ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`maumau acp` exposes an ACP agent over stdio and forwards prompts to a running
Maumau Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Maumau Gateway
...
- ACP client spawns `maumau acp` and speaks ACP messages over stdio.
...
`maumau acp` resolves the Gateway URL and auth from CLI flags or config:
