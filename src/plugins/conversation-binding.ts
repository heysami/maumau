...
const APPROVALS_PATH = "~/.maumau/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "maumau-app-server:thread:",
  "maumau-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("maumau.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("maumau.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Maumau instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
