...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: MaumauConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: MaumauConfig) {
...
export function resolveHeartbeatPrompt(cfg: MaumauConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: MaumauConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: MaumauConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
