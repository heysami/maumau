...
Maumau integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Maumau owns lifecycle when `channels.signal.autoStart=true`.
...
- Maumau spawns `imsg rpc` as a child process (legacy iMessage integration).
