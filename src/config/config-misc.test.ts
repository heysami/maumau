...
import { buildWebSearchProviderConfig, withTempHome, writeMaumauConfig } from "./test-helpers.js";
import { MaumauSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = MaumauSchema.safeParse({
      $schema: "https://maumau.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://maumau.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = MaumauSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = MaumauSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeMaumauConfig(home, {
        gateway: { bind: "${MAUMAU_BIND}" },
      });

      const prev = process.env.MAUMAU_BIND;
      process.env.MAUMAU_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.MAUMAU_BIND;
        } else {
          process.env.MAUMAU_BIND = prev;
        }
      }
    });
  });
