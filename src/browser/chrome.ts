...
import {
  decorateMaumauProfile,
  ensureProfileCleanExit,
  isProfileDecorated,
} from "./chrome.profile-decoration.js";
import type { ResolvedBrowserConfig, ResolvedBrowserProfile } from "./config.js";
import { DEFAULT_MAUMAU_BROWSER_COLOR, DEFAULT_MAUMAU_BROWSER_PROFILE_NAME } from "./constants.js";
...
export function resolveMaumauUserDataDir(profileName = DEFAULT_MAUMAU_BROWSER_PROFILE_NAME) {
...
export function buildMaumauChromeLaunchArgs(params: {
...
export async function launchMaumauChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveMaumauUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_MAUMAU_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateMaumauProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`😼 maumau browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`maumau browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`maumau browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `😼 maumau browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopMaumauChrome(running: RunningChrome, timeoutMs = CHROME_STOP_TIMEOUT_MS) {
