...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [MaumauCapability.canvas.rawValue, MaumauCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(MaumauCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(MaumauCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = MaumauLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(MaumauCapability.location.rawValue) }

        caps.append(MaumauCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(MaumauCapability.watch.rawValue)
        }
        caps.append(MaumauCapability.photos.rawValue)
        caps.append(MaumauCapability.contacts.rawValue)
        caps.append(MaumauCapability.calendar.rawValue)
        caps.append(MaumauCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(MaumauCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            MaumauScreenCommand.record.rawValue,
            MaumauSystemCommand.notify.rawValue,
            MaumauChatCommand.push.rawValue,
        ...
        if caps.contains(MaumauCapability.camera.rawValue) {
        ...
        if caps.contains(MaumauCapability.location.rawValue) {
            commands.append(MaumauLocationCommand.get.rawValue)
        }
        if caps.contains(MaumauCapability.device.rawValue) {
            commands.append(MaumauDeviceCommand.status.rawValue)
            commands.append(MaumauDeviceCommand.info.rawValue)
        }
        if caps.contains(MaumauCapability.watch.rawValue) {
            commands.append(MaumauWatchCommand.status.rawValue)
            commands.append(MaumauWatchCommand.notify.rawValue)
        }
        if caps.contains(MaumauCapability.photos.rawValue) {
            commands.append(MaumauPhotosCommand.latest.rawValue)
        }
        if caps.contains(MaumauCapability.contacts.rawValue) {
            commands.append(MaumauContactsCommand.search.rawValue)
            commands.append(MaumauContactsCommand.add.rawValue)
        }
        if caps.contains(MaumauCapability.calendar.rawValue) {
            commands.append(MaumauCalendarCommand.events.rawValue)
            commands.append(MaumauCalendarCommand.add.rawValue)
        }
        if caps.contains(MaumauCapability.reminders.rawValue) {
            commands.append(MaumauRemindersCommand.list.rawValue)
            commands.append(MaumauRemindersCommand.add.rawValue)
        }
        if caps.contains(MaumauCapability.motion.rawValue) {
            commands.append(MaumauMotionCommand.activity.rawValue)
            commands.append(MaumauMotionCommand.pedometer.rawValue)
        }
