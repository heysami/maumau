...
read_when:
  - You want to use Groq with Maumau
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Maumau connects
...
media-understanding provider, Maumau uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.maumau/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `maumau models list | grep groq`
