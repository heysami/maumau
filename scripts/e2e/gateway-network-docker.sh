...
IMAGE_NAME="maumau-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="maumau-net-e2e-$$"
GW_NAME="maumau-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "MAUMAU_GATEWAY_TOKEN=$TOKEN" \
  -e "MAUMAU_SKIP_CHANNELS=1" \
  -e "MAUMAU_SKIP_GMAIL_WATCHER=1" \
  -e "MAUMAU_SKIP_CRON=1" \
  -e "MAUMAU_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
