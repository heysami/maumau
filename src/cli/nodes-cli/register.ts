    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["maumau nodes status", "List known nodes with live status."],
          ["maumau nodes pairing pending", "Show pending node pairing requests."],
          ['maumau nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["maumau nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.maumau.ai/cli/nodes")}\n`,
    );
