...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Maumau Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(maumau completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(maumau completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.maumau.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $MAUMAU_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "maumau") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
