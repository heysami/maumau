---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Maumau deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Maumau cron scheduling for periodic checks, or version status checks on a machine running Maumau (laptop, workstation, Pi, VPS).
---

# Maumau Host Hardening

## Overview

Assess and harden the host running Maumau, then align it to a user-defined risk tolerance without breaking access. Use Maumau security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Maumau changes the host firewall, SSH, or OS updates; it does not.
...
5. Maumau gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Maumau can access sensitive data.
...
### 2) Run Maumau security audits (read-only)

As part of the default read-only checks, run `maumau security audit --deep`. Only offer alternatives if the user requests them:

1. `maumau security audit` (faster, non-probing)
2. `maumau security audit --json` (structured output)

Offer to apply Maumau safe defaults (numbered):

1. `maumau security audit --fix`

Be explicit that `--fix` only tightens Maumau defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Maumau version/update status (read-only)

As part of the default read-only checks, run `maumau update status`.
...
- Credential hygiene notes (location of Maumau creds, prefer disk encryption)
...
After Maumau install or first hardening pass, run at least one baseline audit and version check:

- `maumau security audit`
- `maumau security audit --deep`
- `maumau update status`

Ongoing monitoring is recommended. Use the Maumau cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `maumau cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `maumau update status`
...
Before creating, `maumau cron list` and match on exact `name`. If found, `maumau cron edit <id> ...`.
If not found, `maumau cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `maumau update status` (preferred for source checkouts and channels)
2. `npm view maumau version` (published npm version)

## Maumau command accuracy

Use only supported commands and flags:

- `maumau security audit [--deep] [--fix] [--json]`
- `maumau status` / `maumau status --deep`
- `maumau health --json`
- `maumau update status`
- `maumau cron add|list|runs|run`

Do not invent CLI flags or imply Maumau enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Maumau compaction:
