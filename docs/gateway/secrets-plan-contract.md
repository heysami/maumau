...
read_when:
  - Generating or reviewing `maumau secrets apply` plans
...
This page defines the strict contract enforced by `maumau secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`maumau secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `secrets apply` writes supported `maumau.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `maumau secrets configure` or fix the target path to a supported shape above.
