...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("maumau-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "maumau-sandbox-browser-global" },
      agentBrowser: { network: "maumau-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("maumau-sandbox-browser-agent");
  });
