...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    ...
    process.env.MAUMAU_SKIP_CHANNELS = "0";
    delete process.env.MAUMAU_SKIP_GMAIL_WATCHER;
    delete process.env.MAUMAU_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.MAUMAU_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.MAUMAU_SKIP_GMAIL_WATCHER;
    } else {
      process.env.MAUMAU_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.MAUMAU_SKIP_PROVIDERS;
    } else {
      process.env.MAUMAU_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    const configPath = process.env.MAUMAU_CONFIG_PATH;
    if (!configPath) {
      throw new Error("MAUMAU_CONFIG_PATH is not set");
    }
  ...
  async function writeAuthProfileEnvRefStore() {
    ...
    if (!stateDir) {
      throw new Error("MAUMAU_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_MAUMAU_AUTH_REF" },
            },
          },
  ...
  it("fails startup when an active exec ref id contains traversal segments", async () => {
    ...
    const previousGatewayTokenEnv = process.env.MAUMAU_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.MAUMAU_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
    }
  });
  ...
  it("fails startup when auth-profile secret refs are unresolved", async () => {
    ...
    delete process.env.MISSING_MAUMAU_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_MAUMAU_AUTH_REF" is missing or empty.',
      );
    } finally {
