...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("maumau-bundled-");
  const workspaceDir = await createTempDir("maumau-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: MaumauConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.MAUMAU_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
