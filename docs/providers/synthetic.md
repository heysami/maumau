---
summary: "Use Synthetic's Anthropic-compatible API in Maumau"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Maumau registers it as the
...
```bash
maumau onboard --auth-choice synthetic-api-key
```
...
Note: Maumau's Anthropic client appends `/v1` to the base URL, so use
