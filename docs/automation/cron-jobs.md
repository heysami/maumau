...
- Jobs persist under `~/.maumau/cron/` so restarts don’t lose schedules.
...
- For upgrades, `maumau doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
maumau cron list
maumau cron run <job-id>
maumau cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.maumau/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `maumau cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Maumau applies a
...
- CLI equivalents: `maumau cron add --light-context ...` and `maumau cron edit --light-context`.
...
- `schedule.at` accepts ISO 8601. Tool/API values without a timezone are treated as UTC; the CLI also accepts `maumau cron add|edit --at "<offset-less-iso>" --tz <iana>` for local wall-clock one-shots.
...
- Job store: `~/.maumau/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.maumau/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Maumau classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Maumau archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `maumau cron runs` and adjust retention before logs become large
...
maumau cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
maumau cron edit <jobId> --agent ops
maumau cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
maumau cron run <jobId>
maumau cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `maumau cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
maumau cron edit <jobId> \
...
```bash
maumau system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`maumau system event`](/cli/system).

## Troubleshooting

### "Nothing runs"

- Check cron is enabled: `cron.enabled` and `MAUMAU_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Maumau applies exponential retry backoff for recurring jobs after consecutive errors:
