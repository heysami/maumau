...
function getChannelConfig(cfg: MaumauConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: MaumauConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: MaumauConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: MaumauConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Maumau uses to send replies back to Chat.",
      ],
