...
function resolveAgentName(cfg: MaumauConfig, agentId: string) {
...
function resolveAgentModel(cfg: MaumauConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: MaumauConfig): AgentSummary[] {
