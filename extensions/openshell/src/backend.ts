...
class OpenShellSandboxBackendImpl {
  ...
  private async syncWorkspaceFromRemote(): Promise<void> {
    const tmpDir = await fs.mkdtemp(path.join(resolveOpenShellTmpRoot(), "maumau-openshell-sync-"));
    try {
      const result = await runOpenShellCli({
        context: this.params.execContext,
        args: [
...
function buildOpenShellSandboxName(scopeKey: string): string {
  ...
  return `maumau-${safe || "session"}-${hash.toString(16).slice(0, 8)}`;
}

function resolveOpenShellTmpRoot(): string {
  return path.resolve(resolvePreferredMaumauTmpDir() ?? os.tmpdir());
}
