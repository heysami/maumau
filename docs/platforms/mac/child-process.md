...
service via the external `maumau` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.maumau.gateway`
  (or `ai.maumau.<profile>` when using `--profile`/`MAUMAU_PROFILE`; legacy `com.maumau.*` is supported).
