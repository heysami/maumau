...
Official Voice Call plugin for **Maumau**.
...
Docs: `https://docs.maumau.ai/plugins/voice-call`
Plugin system: `https://docs.maumau.ai/plugin`

## Install (local dev)

### Option A: install via Maumau (recommended)
...
```bash
mkdir -p ~/.maumau/extensions
cp -R extensions/voice-call ~/.maumau/extensions/voice-call
cd ~/.maumau/extensions/voice-call && pnpm install
```
...
- advanced webhook, streaming, and tunnel notes: `https://docs.maumau.ai/plugins/voice-call`

## Stale call reaper

See the plugin docs for recommended ranges and production examples:
`https://docs.maumau.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `messages.tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.maumau.ai/plugins/voice-call#tts-for-calls`

## CLI

```bash
maumau voicecall call --to "+15555550123" --message "Hello from Maumau"
maumau voicecall continue --call-id <id> --message "Any questions?"
maumau voicecall speak --call-id <id> --message "One moment"
maumau voicecall end --call-id <id>
maumau voicecall status --call-id <id>
maumau voicecall tail
