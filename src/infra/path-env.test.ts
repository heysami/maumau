...
let ensureMaumauCliOnPath: typeof import("./path-env.js").ensureMaumauCliOnPath;

describe("ensureMaumauCliOnPath", () => {
  const envKeys = [
    "PATH",
    "MAUMAU_PATH_BOOTSTRAPPED",
    "MAUMAU_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureMaumauCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/maumau-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "maumau");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureMaumauCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling maumau exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.MAUMAU_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.MAUMAU_PATH_BOOTSTRAPPED = "1";
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "maumau");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.MAUMAU_PATH_BOOTSTRAPPED;
      delete process.env.MAUMAU_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.MAUMAU_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/maumau-path/case-linuxbrew");
