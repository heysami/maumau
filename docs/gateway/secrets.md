...
Maumau supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `MAUMAU_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Maumau runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Maumau validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Maumau QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/maumau"],
...
- Maumau resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `maumau.json` refs)
...
When reload-time activation fails after a healthy state, Maumau enters degraded secrets state.
...
- Strict command paths (for example `maumau memory` remote-memory paths and `maumau qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `maumau status`, `maumau status --all`, `maumau channels status`, `maumau channels resolve`, `maumau security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `maumau secrets reload`.
...
```bash
maumau secrets audit --check
maumau secrets configure
maumau secrets audit --check
```
...
- plaintext values at rest (`maumau.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `maumau.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `maumau secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `maumau.json` plus `auth-profiles.json` for one agent scope
...
- `maumau secrets configure --providers-only`
- `maumau secrets configure --skip-provider-setup`
- `maumau secrets configure --agent <id>`
...
```bash
maumau secrets apply --from /tmp/maumau-secrets-plan.json
maumau secrets apply --from /tmp/maumau-secrets-plan.json --allow-exec
maumau secrets apply --from /tmp/maumau-secrets-plan.json --dry-run
maumau secrets apply --from /tmp/maumau-secrets-plan.json --dry-run --allow-exec
```
...
Maumau intentionally does not write rollback backups containing historical plaintext secret values.
