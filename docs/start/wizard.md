...
CLI onboarding is the **recommended** way to set up Maumau on macOS,
...
`maumau dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
2. **Workspace** — Location for agent files (default `~/.maumau/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `maumau doctor` first.
</Note>
...
Use `maumau agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.maumau/workspace-<agentId>`.
...
- CLI command reference: [`maumau onboard`](/cli/onboard)
