...
describe("entry root help fast path", () => {
  it("renders root help without importing the full program", () => {
    ...
    const handled = tryHandleRootHelpFastPath(["node", "maumau", "--help"], {
  ...
  it("ignores non-root help invocations", () => {
    ...
    const handled = tryHandleRootHelpFastPath(["node", "maumau", "status", "--help"], {
      outputRootHelp: outputRootHelpMock,
      env: {},
    });
  ...
  it("skips the host help fast path when a container target is active", () => {
    ...
    const handled = tryHandleRootHelpFastPath(["node", "maumau", "--container", "demo", "--help"], {
      outputRootHelp: outputRootHelpMock,
      env: {},
    });
