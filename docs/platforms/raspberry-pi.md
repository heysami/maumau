---
summary: "Maumau on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Maumau on a Raspberry Pi
  - Running Maumau on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# Maumau on Raspberry Pi

## Goal

Run a persistent, always-on Maumau Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
```bash
ssh user@gateway-host 'maumau dashboard --no-open'
```
...
(or `MAUMAU_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/maumau-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/maumau-compile-cache
mkdir -p /var/tmp/maumau-compile-cache
export MAUMAU_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `MAUMAU_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Maumau, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit maumau
```

```ini
[Service]
Environment=MAUMAU_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/maumau-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Maumau state/cache on SSD-backed storage to avoid SD-card
...
Most Maumau features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled maumau

# Enable if not
sudo systemctl enable maumau

# Start on boot
sudo systemctl start maumau
```
...
```bash
# Check logs
journalctl -u maumau --no-pager -n 100

# Common fix: rebuild
cd ~/maumau  # if using hackable install
npm run build
sudo systemctl restart maumau
```
