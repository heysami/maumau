---
summary: "Run Maumau Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Maumau running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Maumau on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Maumau Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Maumau 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Maumau Gateway in Docker
- Persist `~/.maumau` + `~/.maumau/workspace` on the host (survives restarts/rebuilds)
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-maumau-project --name="Maumau Gateway"
    gcloud config set project my-maumau-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create maumau-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh maumau-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Maumau repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    MAUMAU_IMAGE=maumau:latest
    MAUMAU_GATEWAY_TOKEN=change-me-now
    MAUMAU_GATEWAY_BIND=lan
    MAUMAU_GATEWAY_PORT=18789

    MAUMAU_CONFIG_DIR=/home/$USER/.maumau
    MAUMAU_WORKSPACE_DIR=/home/$USER/.maumau/workspace

    GOG_KEYRING_PASSWORD=change-me-now
    XDG_CONFIG_HOME=/home/node/.maumau
    ```
    ...
    services:
      maumau-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - MAUMAU_GATEWAY_BIND=${MAUMAU_GATEWAY_BIND}
          - MAUMAU_GATEWAY_PORT=${MAUMAU_GATEWAY_PORT}
          - MAUMAU_GATEWAY_TOKEN=${MAUMAU_GATEWAY_TOKEN}
          - GOG_KEYRING_PASSWORD=${GOG_KEYRING_PASSWORD}
          - XDG_CONFIG_HOME=${XDG_CONFIG_HOME}
          - PATH=/home/linuxbrew/.linuxbrew/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
        volumes:
          - ${MAUMAU_CONFIG_DIR}:/home/node/.maumau
          - ${MAUMAU_WORKSPACE_DIR}:/home/node/.maumau/workspace
        ports:
          # Recommended: keep the Gateway loopback-only on the VM; access via SSH tunnel.
          # To expose it publicly, remove the `127.0.0.1:` prefix and firewall accordingly.
          - "127.0.0.1:${MAUMAU_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`MAUMAU_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm maumau-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh maumau-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop maumau-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type maumau-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start maumau-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create maumau-deploy \
     --display-name="Maumau Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-maumau-project \
     --member="serviceAccount:maumau-deploy@my-maumau-project.iam.gserviceaccount.com" \
