...
private let chatUILogger = Logger(subsystem: "ai.maumau", category: "MaumauChatUI")

@MainActor
@Observable
public final class MaumauChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [MaumauChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [MaumauChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [MaumauPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [MaumauChatPendingToolCall] = []
    public private(set) var sessions: [MaumauChatSessionEntry] = []
    private let transport: any MaumauChatTransport
    private var sessionDefaults: MaumauChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: MaumauChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any MaumauChatTransport,
    ...
    public var sessionChoices: [MaumauChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [MaumauChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: MaumauPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [MaumauChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: MaumauChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: MaumauChatMessage) -> MaumauChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> MaumauChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return MaumauChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: MaumauChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: MaumauChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: MaumauChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [MaumauChatMessage],
        incoming: [MaumauChatMessage]) -> [MaumauChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [MaumauChatMessage]) -> [MaumauChatMessage] {
        var result: [MaumauChatMessage] = []
    ...
    private static func dedupeKey(for message: MaumauChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [MaumauChatMessageContent] = [
            MaumauChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> MaumauChatAttachmentPayload in
            MaumauChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> MaumauChatSessionEntry {
        MaumauChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = MaumauChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: MaumauChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: MaumauChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: MaumauAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = MaumauChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> MaumauPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
