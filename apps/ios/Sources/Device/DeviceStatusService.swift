...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> MaumauDeviceStatusPayload {
        ...
        return MaumauDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> MaumauDeviceInfoPayload {
        ...
        return MaumauDeviceInfoPayload(
    ...
    private func batteryStatus() -> MaumauBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: MaumauBatteryState = switch device.batteryState {
        ...
        return MaumauBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> MaumauThermalStatusPayload {
        let state: MaumauThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return MaumauThermalStatusPayload(state: state)
    }

    private func storageStatus() -> MaumauStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return MaumauStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
