...
const ENV_CATALOG_PATHS = ["MAUMAU_PLUGIN_CATALOG_PATHS", "MAUMAU_MPM_CATALOG_PATHS"];

function collectModelRefs(cfg: MaumauConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: MaumauConfig, providerId: string): boolean {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.maumau) || !isRecord(entry.maumau.channel)) {
      continue;
    }
    const channel = entry.maumau.channel;
...
function collectCandidateChannelIds(cfg: MaumauConfig): string[] {
...
function configMayNeedPluginManifestRegistry(cfg: MaumauConfig): boolean {
  const configuredChannels = cfg.channels as Record<string, unknown> | undefined;
  if (!configuredChannels || typeof configuredChannels !== "object") {
    return false;
  }
...
function isPluginExplicitlyDisabled(cfg: MaumauConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: MaumauConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: MaumauConfig, pluginId: string): MaumauConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
