...
const LIVE_MODEL_ID = process.env.MAUMAU_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.4-nano";
const LIVE_IMAGE_MODEL = process.env.MAUMAU_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-1";
const LIVE_VISION_MODEL = process.env.MAUMAU_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.MAUMAU_LIVE_TEST === "1";
...
function createLiveConfig(): MaumauConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await (speechProvider as any).synthesize({
      text: "Maumau integration test OK.",
      cfg,
      config: ttsConfig,
      target: "audio-file",
    });
