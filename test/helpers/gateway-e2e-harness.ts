...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `maumau-e2e-${name}-`));
  const configDir = path.join(homeDir, ".maumau");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          MAUMAU_SKIP_PROVIDERS: "1",
          MAUMAU_SKIP_GMAIL_WATCHER: "1",
          MAUMAU_SKIP_CRON: "1",
          MAUMAU_SKIP_BROWSER_CONTROL_SERVER: "1",
          MAUMAU_SKIP_CANVAS_HOST: "1",
          MAUMAU_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
