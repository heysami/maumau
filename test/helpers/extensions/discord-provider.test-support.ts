...
import type { MaumauConfig } from "../../../extensions/discord/src/runtime-api.js";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: MaumauConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): MaumauConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("maumau/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("maumau/plugin-sdk/acp-runtime")>(
    "maumau/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("maumau/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("maumau/plugin-sdk/command-auth")>(
...
vi.mock("maumau/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("maumau/plugin-sdk/reply-runtime")>(
    "maumau/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("maumau/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("maumau/plugin-sdk/config-runtime")>(
...
vi.mock("maumau/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("maumau/plugin-sdk/runtime-env")>(
    "maumau/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("maumau/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("maumau/plugin-sdk/infra-runtime")>(
