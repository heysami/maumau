...
@MainActor
final class MaumauAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.maumau.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.maumau.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.maumau.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "maumau.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "maumau.watch.promptId"
    static let sessionKeyKey = "maumau.watch.sessionKey"
    static let actionPrimaryIDKey = "maumau.watch.action.primary.id"
    static let actionPrimaryLabelKey = "maumau.watch.action.primary.label"
    static let actionSecondaryIDKey = "maumau.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "maumau.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "maumau.watch.action.primary"
    static let actionSecondaryIdentifier = "maumau.watch.action.secondary"
    static let actionIdentifierPrefix = "maumau.watch.action."
    static let actionIDKeyPrefix = "maumau.watch.action.id."
    static let actionLabelKeyPrefix = "maumau.watch.action.label."
    static let categoryPrefix = "maumau.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: MaumauWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> MaumauWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return MaumauWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Maumau" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [MaumauWatchAction]) -> [UNNotificationAction] {
...
@main
struct MaumauApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(MaumauAppDelegate.self) private var appDelegate
...
extension MaumauApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Maumau: installing uncaught exception handler")
