...
function createPluginSourceRoots() {
  ...
  const globalRoot = path.resolve(path.sep, "Users", "x", ".maumau", "extensions");
  const workspaceRoot = path.resolve(path.sep, "Users", "x", "ws", ".maumau", "extensions");
  return {
    stock: stockRoot,
    global: globalRoot,
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "maumau-home");
    ...
    expect(roots).toEqual({
      ...
      workspace: path.join(homeDir, "ws", ".maumau", "extensions"),
    });
  });
});
