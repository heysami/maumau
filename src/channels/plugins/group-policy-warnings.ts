...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: MaumauConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: MaumauConfig }>(
  collector: WarningCollector<{ cfg: MaumauConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: MaumauConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: MaumauConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: MaumauConfig },
>(
  projectCfg: (cfg: MaumauConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: MaumauConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: MaumauConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: MaumauConfig },
>(params: {
  providerConfigPresent: (cfg: MaumauConfig) => boolean;
