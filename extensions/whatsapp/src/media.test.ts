...
import { resolvePreferredMaumauTmpDir } from "../../../src/infra/tmp-maumau-dir.js";
...
vi.mock("maumau/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("maumau/plugin-sdk/media-runtime")>(
    "maumau/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
    convertHeicToJpeg: (...args: unknown[]) => convertHeicToJpegMock(...args),
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(resolvePreferredMaumauTmpDir(), "maumau-media-test-"));
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub MAUMAU_STATE_DIR.
    // Also keep it outside the Maumau temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["MAUMAU_STATE_DIR"]);
    process.env.MAUMAU_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "maumau-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredMaumauTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Maumau state workspace and sandbox roots", async () => {
  ...
  it("rejects default Maumau state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
