...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Maumau Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`maumau acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Maumau gateway or agent instead.                                                                                                                                       |
...
# Remote Gateway
maumau acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
maumau acp --url wss://gateway-host:18789 --token-file ~/.maumau/gateway.token

# Attach to an existing session key
maumau acp --session agent:main:main

# Attach by label (must already exist)
maumau acp --session-label "support inbox"

# Reset the session key before the first prompt
maumau acp --session agent:main:main --reset-session
```
...
maumau acp client --server-args --url wss://gateway-host:18789 --token-file ~/.maumau/gateway.token

# Override the server command (default: maumau)
maumau acp client --server "node" --server-args maumau.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an Maumau Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `maumau acp` over stdio.

Example config (persisted):

```bash
maumau config set gateway.remote.url wss://gateway-host:18789
maumau config set gateway.remote.token <token>
```
...
```bash
maumau acp --session agent:main:main
maumau acp --session agent:design:main
maumau acp --session agent:qa:bug-123
```
...
Maumau bot over ACP, use `acpx` with its built-in `maumau` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx maumau` at `maumau acp`.
3. Target the Maumau session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Maumau ACP session
acpx maumau exec "Summarize the active Maumau session state."

# Persistent named session for follow-up turns
acpx maumau sessions ensure --name codex-bridge
acpx maumau -s codex-bridge --cwd /path/to/repo \
  "Ask my Maumau work agent for recent context relevant to this repo."
```

If you want `acpx maumau` to target a specific Gateway and session key every
time, override the `maumau` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "maumau": {
      "command": "env MAUMAU_HIDE_BANNER=1 MAUMAU_SUPPRESS_NOTES=1 maumau acp --url ws://127.0.0.1:18789 --token-file ~/.maumau/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Maumau checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env MAUMAU_HIDE_BANNER=1 MAUMAU_SUPPRESS_NOTES=1 node maumau.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Maumau agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Maumau ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`MAUMAU_GATEWAY_TOKEN`, `MAUMAU_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`MAUMAU_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `MAUMAU_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `maumau acp client` sets `MAUMAU_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `maumau`).
