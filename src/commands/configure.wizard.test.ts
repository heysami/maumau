...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.maumau/maumau.json",
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.maumau/workspace",
  applyWizardMetadata: (cfg: MaumauConfig) => cfg,
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.applySearchProviderSelection.mockImplementation((cfg: MaumauConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by applySearchKey", async () => {
    ...
    mocks.hasExistingKey.mockReturnValue(false);
    mocks.hasKeyInEnv.mockReturnValue(false);
    mocks.applySearchKey.mockImplementation((cfg: MaumauConfig, provider: string, key: string) =>
  ...
  it("applies provider selection side effects when a key already exists via secret ref or env", async () => {
    ...
    mocks.hasKeyInEnv.mockReturnValue(false);
    mocks.applySearchProviderSelection.mockImplementation((cfg: MaumauConfig, provider: string) =>
      createEnabledWebSearchConfig(provider, {
        enabled: true,
      })(cfg),
    );
  ...
  it("skips the API key prompt for keyless web search providers", async () => {
    ...
    mocks.resolveSearchProviderOptions.mockReturnValue([
      createSearchProviderOption({
        ...
        docsUrl: "https://docs.maumau.ai/tools/web",
        credentialPath: "",
      }),
    ]);
    mocks.applySearchProviderSelection.mockImplementation((cfg: MaumauConfig, provider: string) =>
