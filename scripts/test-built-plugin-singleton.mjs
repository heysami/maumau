...
const warningFilterKey = Symbol.for("maumau.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadMaumauPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadMaumauPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "maumau-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@maumau/build-smoke-plugin",
      type: "module",
      maumau: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'maumau/plugin-sdk';",
...
const registry = loadMaumauPlugins({
  ...
  env: {
    ...process.env,
    MAUMAU_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
