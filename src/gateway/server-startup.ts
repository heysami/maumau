...
import type { loadMaumauPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureMaumauModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: ReturnType<typeof loadConfig>;
  pluginRegistry: ReturnType<typeof loadMaumauPlugins>;
  ...
  // Start Maumau browser control server (unless disabled via config).
  ...
  // Tests can opt out via MAUMAU_SKIP_CHANNELS (or legacy MAUMAU_SKIP_PROVIDERS).
  const skipChannels =
    isTruthyEnvValue(process.env.MAUMAU_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.MAUMAU_SKIP_PROVIDERS);
  if (!skipChannels) {
    try {
    ...
    params.logChannels.info(
      "skipping channel start (MAUMAU_SKIP_CHANNELS=1 or MAUMAU_SKIP_PROVIDERS=1)",
    );
  }
