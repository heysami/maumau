...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("maumau-daemon-install-int-");
    configPath = path.join(tempHome, "maumau.json");
    process.env.HOME = tempHome;
    process.env.MAUMAU_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.MAUMAU_GATEWAY_TOKEN).toBeUndefined();
  });
});
