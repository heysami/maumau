import type { AnyAgentTool, MaumauPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: MaumauPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<MaumauPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
