...
const { createMaumauTools } = await import("../agents/maumau-tools.js");
...
type SessionSendTool = ReturnType<typeof createMaumauTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createMaumauTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["MAUMAU_GATEWAY_PORT", "MAUMAU_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.MAUMAU_GATEWAY_PORT = String(gatewayPort);
  process.env.MAUMAU_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("MAUMAU_CONFIG_PATH missing in gateway test environment");
      }
