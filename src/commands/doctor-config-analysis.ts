...
import { MaumauSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: MaumauConfig): {
  config: MaumauConfig;
  removed: string[];
} {
  const parsed = MaumauSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: MaumauConfig): void {
