...
- `maumau gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `maumau doctor` reports no blocking config/service issues.
- `maumau channels status --probe` shows connected/ready channels.
...
maumau config get agents.defaults.models
```
...
maumau status
maumau channels status --probe
maumau pairing list --channel <channel> [--account <id>]
maumau config get channels
maumau logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `maumau config get gateway.auth.token` then paste into Control UI settings.                            |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `maumau devices list` then `maumau devices approve <requestId>`.                                                                            |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `maumau configure`). If you are running Maumau via Podman using the dedicated `maumau` user, the config lives at `~maumau/.maumau/maumau.json`.
...
maumau approvals get --node <idOrNameOrIp>
maumau logs --follow
maumau status
```
...
maumau browser start --browser-profile maumau
...
```bash
maumau gateway status
maumau config get gateway.mode
maumau config get gateway.remote.url
maumau config get gateway.auth.mode
```
...
```bash
maumau config get gateway.bind
maumau config get gateway.auth.token
maumau gateway status
maumau logs --follow
```
