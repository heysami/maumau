...
describe("doctor config flow", () => {
  ...
  it("previews Matrix legacy sync-store migration in read-only mode", async () => {
    ...
    try {
      ...
      expect(warning?.[0]).toContain('Run "maumau doctor --fix" to migrate this Matrix state now.');
    } finally {
      noteSpy.mockRestore();
    }
  });
  ...
  it("creates a Matrix migration snapshot before doctor repair mutates Matrix state", async () => {
    await withTempHome(async (home) => {
      ...
      const snapshotDir = path.join(home, "Backups", "maumau-migrations");
      ...
      expect(marker.archivePath).toContain(path.join("Backups", "maumau-migrations"));
    });
  });
  ...
  it("warns when Matrix is installed from a stale custom path", async () => {
    ...
    expect(
      doctorWarnings.some(
        (line) => line.includes("custom path") && line.includes("/tmp/maumau-matrix-missing"),
      ),
    ).toBe(true);
  });
