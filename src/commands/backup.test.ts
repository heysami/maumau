...
describe("backup commands", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("maumau-backup-test-");
  });
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-workspace-link-"));
    ...
    try {
      ...
      await fs.writeFile(
        path.join(stateDir, "maumau.json"),
        JSON.stringify({
          agents: {
            defaults: {
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    ...
    const configPath = path.join(tempHome.home, "custom-config.json");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-backups-"));
    try {
      process.env.MAUMAU_CONFIG_PATH = configPath;
      await fs.writeFile(
        configPath,
      ...
      const extractDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-backup-extract-"));
      try {
  ...
  it("optionally verifies the archive after writing it", async () => {
    const stateDir = path.join(tempHome.home, ".maumau");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-backup-verify-on-create-"));
    try {
      await fs.writeFile(path.join(stateDir, "maumau.json"), JSON.stringify({}), "utf8");
      await fs.writeFile(path.join(stateDir, "state.txt"), "state\n", "utf8");
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".maumau");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-backup-link-"));
  ...
  it("falls back to the home directory when cwd is a symlink into a backed-up source tree", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".maumau");
    const workspaceDir = path.join(stateDir, "workspace");
    const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "maumau-backup-cwd-link-"));
