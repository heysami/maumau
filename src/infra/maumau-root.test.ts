...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__maumau_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "maumau-root");
...
describe("resolveMaumauPackageRoot", () => {
  let resolveMaumauPackageRoot: typeof import("./maumau-root.js").resolveMaumauPackageRoot;
  let resolveMaumauPackageRootSync: typeof import("./maumau-root.js").resolveMaumauPackageRootSync;

  beforeEach(async () => {
    vi.resetModules();
    ({ resolveMaumauPackageRoot, resolveMaumauPackageRootSync } = await import("./maumau-root.js"));
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "maumau");
    const pkgRoot = path.join(project, "node_modules", "maumau");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "maumau" }));

    expect(resolveMaumauPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "maumau");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "maumau.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "maumau" }));

    expect(resolveMaumauPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveMaumauPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("falls through from a non-maumau moduleUrl candidate to cwd", async () => {
    const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
    const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
    setFile(path.join(wrongPkgRoot, "package.json"), JSON.stringify({ name: "not-maumau" }));
    setFile(path.join(cwdPkgRoot, "package.json"), JSON.stringify({ name: "maumau" }));
    const moduleUrl = pathToFileURL(path.join(wrongPkgRoot, "dist", "index.js")).toString();

    expect(resolveMaumauPackageRootSync({ moduleUrl, cwd: cwdPkgRoot })).toBe(cwdPkgRoot);
    await expect(resolveMaumauPackageRoot({ moduleUrl, cwd: cwdPkgRoot })).resolves.toBe(
      cwdPkgRoot,
    );
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveMaumauPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveMaumauPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-maumau package roots", async () => {
    const pkgRoot = fx("not-maumau");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-maumau" }));

    expect(resolveMaumauPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("falls back from a symlinked argv1 to the node_modules package root", () => {
    const project = fx("symlink-node-modules-fallback");
    const argv1 = path.join(project, "node_modules", ".bin", "maumau");
    state.realpaths.set(abs(argv1), abs(path.join(project, "versions", "current", "maumau.mjs")));
    const pkgRoot = path.join(project, "node_modules", "maumau");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "maumau" }));
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveMaumauPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveMaumauPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
