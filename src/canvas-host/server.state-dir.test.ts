...
describe("canvas host state dir defaults", () => {
  ...
  it("uses MAUMAU_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("maumau-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Maumau Canvas");
      } finally {
        await handler.close();
      }
    });
